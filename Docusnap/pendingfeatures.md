...
> **Source of truth for what is FLIPPED/SHIPPED = the switch ledger in `CLAUDE.md` (verified at the
> source) + `docs/oracle_log.md`. A 2026-09-02 cleanup pass removed entries confirmed complete against
> that ledger; the removed set is listed at the bottom under "✅ Cleared 2026-09-02".**
...
its cropped-image slices on toggle-OFF · C-4 client `ALLOW_SELF_SIGNED` gated to `!app.isPackaged` (MITM
close-out) · C-3 client app-level navigation lockdown · C-8 CLAUDE.md electron-builder version. **OPEN
(owner approval-class — do NOT auto-build):**
- **C-1 CODE-SIGN the installer/binary — the ONE release blocker with no documentation workaround on a
  direct/NSIS channel** (unsigned also makes the asar-integrity fuse + unpacked-`.node` protection
  meaningless). Oracle exception: if MS Store is the SOLE channel, the Store signs it → satisfied. → owner
  decides the channel.
- **R1 PLAINTEXT-DB disclosure.** The DB holds every doc's full OCR text in plaintext (encryption arc built
  but DARK). Oracle §2: **ship opt-in encryption + a LOUD BitLocker posture — NOT default-on** (default-on
  undrilled code-as-passphrase = unbounded silent DATA-LOSS > bounded exfil). Add a visible Security panel
  disclosing plaintext. The blocker is the *silent* form.
- **R2 honest binned-doc retention copy** (Oracle promoted to fix-before-release): a soft-deleted doc keeps
  its plaintext original in `%APPDATA%\ScanFinder\inbox` + its `ocr_text` row until Empty Bin, unsaid — fix
  the copy at least ("original kept until you Empty Bin"), optional age-purge.
- **D-1 `node-forge` 1.3.1 → `^1.4.0`** (Oracle de-escalated to free hygiene — NOT reachable, licence+TLS
  use Node native crypto; one-line, non-breaking, does not gate release).
- **D-3 add a CVE prebuild gate** (`npm audit --omit=dev --audit-level=high` + `pip-audit`/`osv-scanner` on
  `vendor/python`). **C-2 verify the fuses in the packaged artifact** (`npx @electron/fuses read`, owner box).
- Acceptable-with-doc (schedule): native PDF/image decoders (contained, keep vendor/python patched); the
  pre-existing TOFU-enrollment MITM on the detached client (pairing-integrity check before the CLIENT ships);
  filed-output plaintext by design (document BitLocker posture); verify WAL encryption once R1 ships.

## 2026-09-01 NIGHT — Chris sandboxed-round cards (`docs/CHRIS_FULL_APP_REVIEW_2026-09-01_NIGHT.md`; verdict YES; owner vet)
Implement nothing without the owner's go.
- **#1 (top harm) — teach-box clip → plausible-word issuer garble still stands up "Apply to N".** Diagnosed
  at source: Plan A's ripple guard (`offerIssuerRipple` renderer.js:7855-7857 → `checkIssuerRead` →
  `shouldOfferIssuerRipple`) works AS SCOPED (catches obvious garbles like "NOCUMENT") but
  `issuerReadLooksImplausible` is a "does this look like a company?" NAME-QUALITY gate, so a plausible-word
  clip ("lorthgate Textiles") passes and gets a green tick + one-click "Apply to 19". **New card = a
  LETTERHEAD-MATCH signal**: caution when the taught issuer doesn't match the company name the app can
  already read on that page's branding, before the green tick + "Apply to N". Needs design (reggie/eric →
  Oracle). NOT a Plan A bug.
- **#2 the "needs a quick check" banner goes stale** — doesn't refresh after a teach raises a field's
  confidence 69%→90% (Reprocess-Quick cleared it); it also appears to be what blocks those docs bulk-filing.
- **#3 the check summary double-counts** — "1 low confidence AND 1 formatting check" when both are on the
  SAME field; reword ("1 field needs a look — low confidence AND flagged").
- **#4 "ready" is overloaded** — activity "19 more ready to file" vs group head "2 more to file by itself";
  one word per concept ("waiting for you to confirm" vs "will file on its own once learned").
- **#6 binned-doc detail hides ref/date** when deciding whether to restore — show the known ref/date on
  binned items.
- (#5 diagnostic-log-on = dev-harness artifact, RESOLVED, no action — `dev-start.js` forces the env; the
  packaged default is OFF.)

## 2026-09-01 NIGHT — Quick Reprocess FLIP GATE (DARK `quick_reprocess_enabled`; `docs/QUICK_REPROCESS_GATE_2026-09-01.md`)
Built + pinned (merge 68, Python recipe 20, emit fixture 8/8 on real pixels), Oracle SIGN-OFF-W/COND; the
pixel-heal seam it caught is disclosed in the dialog (`2c25a6c`). **Remaining before flip (owner decision):
run the 5-arm gate (A OFF-identical / B Quick-vs-Full M=0 / C idempotence / D C3 self-heal / E C1 contested)
on a `db.backup()` of your REAL Castellan DB** — the only place the load-bearing pixel-heal→auto-file class
is measurable. Stays OFF until Arm B runs there. Slice 2 (retrofit `ocrCacheUsable` onto today's
unconditional cache reuse) still deferred (own switch + census).

---

## 2026-08-28 — WORKFLOW + STAMPING REDESIGN — ✅ BUILT 2026-08-28 NIGHT (`eba2ef2`…`86de7d7`)
**STATUS (2026-09-02 cleanup): BUILT + PINNED** — signed-grant `can_stamp` on the DPAPI audit chain,
append-only `stamp_events`, `/v1` + client parity; see `project_workflow_stamping_redesign_20260828.md` +
`HANDOVER_2026-08-28_NIGHT.md`. Full pre-build design + Oracle hard gates + the 5 owner decisions live in
the design doc + git (`eba2ef2`…`86de7d7`) — not duplicated here. **Only genuinely-open residual to
confirm:** whether any later gated slice (of the original §7 five) remains unbuilt; the core redesign
shipped DARK/add-on-gated. Verify against the design doc's §7 checklist before re-opening this.
...
## ✅ Cleared 2026-09-02 (cleanup pass — confirmed complete against the CLAUDE.md switch ledger)
Removed as done/live (git + oracle_log preserve the detail):
- **Manual-landmark boxes don't SNAP** — SHIPPED `9861d37` (the entry itself said "remove on the next sweep").
- **"corroboration clears a verification-doubt note" (class F)** ×2 (the BUILT-DARK entry + its audit
  companion) — `corrob_verification_doubt_clear` is FLIPPED ON (mig 91, per the CLAUDE.md ledger), so the
  "OWED before flip" note was stale.
- **Workflow + Stamping redesign** — marked BUILT in place (`eba2ef2`…`86de7d7`), not removed (a later slice
  may remain).

### Archival pass 2 (2026-09-02) — moved to `pendingfeatures-archive.md`
Section-by-section pass. Moved 7 CLOSED sections to the archive (decided/superseded/answered/refused):
BUYER-ISSUED SLICE 3 (MEASURED AND REFUSED) · the MACHINE-FEED (ruling record) · three "(superseded
original)" entries (MACHINE AUTO-FILES / IMPORT AUTO-FILE PRE-GATE / TYPE ELECTION) · NON-UK VAT NUMBERS
ARE NOW REFUSED · the ANSWERED template-LANDMARKS entry. Collapsed the WORKFLOW + STAMPING entry to a
BUILT stub (design detail is in `docs/designs/` + git; a possible later slice is the only residual).

**HONEST NOTE on this file:** it is long mostly because the work is genuinely still open, NOT because it is
full of removable completed items. Every remaining entry was checked header-by-header this pass: most are
either NOT BUILT (designed/deferred) or **BUILT DARK awaiting a flip gate** (the flip IS the pending work —
e.g. `deskew_corrob_autofile`, `watch_separate_enabled`, `template_locate_role_qualifier`,
`template_fragment_containment_yield`, `identifier_registry`, `quick_reprocess_enabled`, `autofile_gate_unify`
(flip-blocked), `confirmed_dominant_adopt` (census owed) — all still OFF per the ledger). The "SHIPPED"
...
## 2026-09-06 — log-review fix plan BUILT (HANDOVER_2026-09-06.md); the owed / deferred parts
- **Item 1 teach-side page-words caption sourcing** (Oracle-signed, NOT built): source the ⊕ caption from `region.py --page-words` (200 DPI, ONE pass — the wizard's `locateTypedValue` already does it) and feed `anchorLabel.js`'s row/cluster pickers with those words, instead of the two-pass 108-DPI LEFT strip (`region_core.py:151-153` upscales only <300 px; label text + box come from two Tesseract passes → `''` / `Denn` with NULL offset). This is the actual fix for the `''` label; the WARN + typed-caption input (`c09c16c`) is the safety net. Trade-off for the owner: one full-page read per draw (draw-tool latency memory).
- **`learning.clearAnchors` is document_type-BLIND** (`learning.js:673-686`: `field_key = ? AND (supplier_name = ? OR '__unknown__' OR NULL)`): a typed correction on a Meadowvale credit-note total also wipes a Meadowvale INVOICE total anchor. And the label-less currency class is self-limiting ONLY when the operator TYPES a correction — a re-teach re-plants the same label-less row (field_anchors id 17 did on 09-05). Own slice: scope the wipe by document_type (mig + pin), census the wipes in the audit log.
- **The live DB was RESET (541 docs) → the 605-paper `rr_ids.txt` selects 15 confirmed docs; the DB-based realdoc harness is vacuous for that corpus.** A WARM folder-corpus harness (score the Desktop corpus against `ground_truth.json` WITH the live learning snapshot) does not exist — `customer_corpus_score.js` is the COLD model. Decide: rebuild `rr_ids` from the current DB (`_dedup_ids.py` on a db.backup() copy) or build the warm folder harness.
- **doc 471 (Pelican 0023) confirmed date `26-01-6000` = POISONED GT** (an impossible year rubber-stamped from the old `PI/26/6000`-tail read). Learning Repair / correct; until then every arm shows it as a flagged "regression" on ref + date.
- **4(b) OCR re-detect** over the Desktop 605 corpus + Hard Set 600 (PDFs, no stored text) — a night-run arm; the live-copy text census MET (20/20 Ironclad, 521 unchanged).
- **5b seam gate**: OFF-vs-ON sibling run for templates 6/7 on a db.backup() copy (the heal now works → their siblings' Stage-0.5 compose changes next batch). Never re-darken. Templates 3/5/8/9 (NULL sample + NULL angle) = the open owner decision (~:1224 entry).
- **The 4 SILENT Print Tracker ref regressions** in every arm (`782923124N3M2`→`752…` 8↔5 unbacked; `1G25802868`→`1625802868` ×2 = the doc176 class; `HS73429209`→`H573429209`) = the leg-b / confusion-precedence 2a substrate — not today's scope; confirm pre-existing on the `old` arm.
- **Owner actions logged (no code):** tick the Statement + Service Worksheet presets · re-teach the Meadowvale total WITH "Total to Pay" · Learning Repair the three sign/wrong-row corrections rows · the `money_sign_capture` flip (both mints now re-attach) · flips of mig 122's arcs only on the arm numbers.
- **2B review-load seam (doc 453 class):** after the registration rung's caption-band MIRROR rejects a `USTOMER` read, the two-names demoter ("Two different names were read here … Kept the label read; please verify") still counts that REJECTED read as a name dissent and holds a doc whose label read was right (@78 → @69 + note). A `caption_band_read` (or `not_credible`) rejection should be a DISQUALIFIED dissent (the D1 "positive evidence the dissent was disqualified" leg exists for exactly this) — own slice + pin; price today = 1 of 20 Oakhaven docs.
- **Hard Set GT vs the reset live DB:** (a) the warm arm's `buyer_issued_po` docs are claimed by Bramblewood Joinery Ltd's REAL PO template (4 would-file as Bramblewood POs) — the 07-12 doctrine the 08-31 report already called a GT FLAW; fix the GT to dual-accept buyer-or-vendor, or accept. (b) `table_total`/`logo_siblings` lose their `statement` docs (Statement uninstalled on the reset DB) — the 4(a) preset tick restores them. Neither is a code regression (pre-today python reproduces both exactly).
- **2026-09-07 TEST BUILD `…-r20260907-0829-ef47b51.exe`:** mig 123 force-ON (three log-review arcs + `money_sign_capture`). The standing "revert the TEST force-ON migs before a customer build" list is now **108/110/112/114/116/118/123** (+ delete `test_migration123_test_force_on.js`).
- **Review-window pin hygiene (pre-existing reds, 2026-09-07):** `test_activity_strip.js` ("put back by you" after undo; bulk-approval strip line), `test_deskew_session.js` (C4 runReprocessBatch {deskewAll, deskewMinAngle}), `test_teach_speaks.js`, `test_settings_wiring.js` (stamp/dbenc ids). All red BEFORE today's renderer change (verified on the stashed renderer). A hygiene pass: re-pin or fix.
- **2026-09-07 TEST BUILD 3 `ScanFinder Setup 2.0.0-r20260907-0920-7e8f1c4.exe`:** mig 124 force-ON of the 09-04 arcs. Revert list before a customer build: **108/110/112/114/116/118/123/124** + the two test pins. Their flip gates (Oracle C9 for leg-b/leg-a, G1-G3 for 2a, the join census) remain owed.
- **One-confirm convention (2026-09-07, built `d514a15`): follow-ups** — (a) slice 2, the quiet-lane 'convention' re-read of queued siblings, only with a record usage >= 2 arming threshold (Oracle C6); (b) the REVERSE rule ("file this company's POs under the vendor") = a value-rewriting arc, Oracle-only; (c) the canonical-suggestion accept (`identity_suggest_canonical`) makes confirmed != display → no record → the note recurs on a fragment-reading scope (documented non-fire); (d) the `/v1` client single confirm can write the record on a note the client may not render — verify the client renders the note or gate the hook to desktop; (e) revert list before a customer build: 108/110/112/114/116/118/123/124/126.
- **Per-mapping teach angle (Oracle C3, 2026-09-07):** `templates.sample_deskew_angle` is ONE angle per template, but a REUSED template re-pins its sample at every promote (`review/handler.js promote-to-template`) without resetting the angle, so a box drawn on a NEW tilted sample is composed with the OLD sample's angle (the same hole for `link-document-to-template` + graduation). Built today: the commit-time write is guarded `IS NULL` and a ≥0.3° mismatch logs a WARN naming both angles. The fix = a per-mapping `teach_angle` column (NULL → the template angle) written by the wizard for every box it saves; the compose reads the mapping's own angle first. Also pre-existing: the angle is page-0 only while the compose uses `crop_pages[0].size` for a mapping on any page (multi-page teach).
- **Wizard snap left pad (007 + Oracle 2026-09-07):** `shared/boxSnap.js` left pad `min(0.004, 0.15·h)` ≈ 3 px vs a 17 px glyph while the caption gap measures 21–24 px on this layout; paper placement alone scatters a value's left edge by ~57 px across 10 siblings, so even the full compose leaves 10/90 first-glyph cuts ≥ 8 px. Follow-up: scale the left pad to a fraction of the MEASURED caption gap (capped, never past the caption's right edge) — its own census before it ships (every new teach is in the blast radius).
- **E3 identity slip on the RAW pass (Oracle P5 SEND BACK, 2026-09-07):** a 2-token supplier name has zero slip tolerance under `identity_present_on_page`'s ≥60% whole-word floor (1/2 = 50%), so one glyph slip ('Lronbridge') on the raw pass throws the frozen `template_fixed` identity away and forces the straighten retry + its "confirm once" note. The deskew identity-slip release was refused (a memory stamp with ONE page family is not a fact; cosmetic while the customer first-fill note also holds). Right layer = a slip-tolerant token match INSIDE `identity_present_on_page` for the FILL/veto path (edit-distance ≤1 on tokens ≥6 chars, floor unchanged) — its own Oracle round with a cross-supplier collision census ('Ironbridge' vs 'Ironbridges'; a young shared-logo sibling).
- **Case-only prefix reads (`ws-55718` sweep-filed; Oracle P6 WRONG LAYER as framed):** case is invisible to every extraction gate for hyphenated codes (`shape_signature` folds letters, `classify_single` → ALPHANUM_SEP either way) and Learning Repair's `alphaPrefix` uppercases. The fold belongs as a CASE-ONLY leg in the prefix-confusable adopt lane (`engine.py` `_PREFIX_CONFUSE_CLASSES` has no W/w pair) gated on a unanimous confirmed prefix (≥5 docs, ≥0.9 share) + the lane's page-witness rule; plus a case-aware prefix outlier in Learning Repair (drop the `.toUpperCase()` behind its own flag) to surface the filed rows.
- **Import-speed slice 3 (Oracle C9-C11, NOT built):** pool the independent locates (`_fit_page_transform` two-phase with single-flight per cache key; the mapper pre-pass; the light-text levels consumed in order with a whole-pass serial belt on any raise) behind `DS_OCR_PARALLEL_LOCATES`, every pool honouring `DS_OCR_POOL_WORKERS`. Slice 4 (per-mapping pool) = DO NOTHING (the first-mapping-wins short-circuit; unmeasured reward). The owner's own switches remain the big cost (light-text −5.5 s of 26 s on a scanned page; `deskew_review_retry` 2× on a tilted review-bound doc).
- **Shared locate cache honest yield:** −1 spawn on a tilted page — the scan compose shifts the Stage-0.5 landmark keys so the Stage-2 refit (un-composed landmarks) misses; the ~5-spawn saving appears on level pages. A later slice could fit Stage 2 against the SAME composed landmarks (a fit-input change — its own gate).
- **`teach_angle_compose_null_abstain` (mig 129) is NOT a flip candidate on its own:** the arm shows abstain trades the year-garble regression for two new first-digit clips (stationary read class) — the fix is the KNOWN angle (P1a + backfill); keep OFF unless a NULL-angled template is measured worse under the half compose than stationary.
