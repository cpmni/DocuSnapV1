/** ANCHOR_INLINE_TAUGHT_OFFSET_VETO A/B over the Pelican Office Interiors batch.
 *
 *  WHY THIS EXISTS (2026-08-08). The live DB holds 9 Pelican documents whose customer_name is junk
 *  ('sui' / 'sup' / 'sum' / 'sup to', conf 70-82, all needs_review) committed by the ⊕-taught anchor
 *  (field_anchors id 73, label 'CUSTOMER', direction below, taught 18:52). Measured cause: the label
 *  and the right-hand block's heading share ONE OCR row — 'CUSTOMER' at x=0.0677 and 'SHIP' at
 *  x=0.5173, y=0.2332 both — and _locate_for_relocation searches a FULL-PAGE-WIDTH strip at the
 *  label's row by design. cluster_value_words then splits the post-label words into gap-runs and
 *  returns the run nearest the label's right edge; 'SHIP TO' is the ONLY run, and a single run is
 *  returned UNCHANGED (template_mapper.py:2229). The persisted ledger proves the cost: every one of
 *  those docs also carries {'Bramblewood Joinery Ltd', template_mapping, conf 90} and the junk still
 *  wins, because a ⊕ anchor outranks the Stage-0.5 mapping.
 *
 *  The fix compares the HARVESTED value's position to the anchor's own taught offset (the sibling
 *  _located_at_taught_position only ever verified the LABEL). Dropping the harvest falls through to
 *  the crop read seated AT that offset, which lands on the name — so the expected result is a HEAL,
 *  not merely a fail-to-review.
 *
 *  THE NUMBER THAT DECIDES THE FLIP is REGRESSED: the 87 documents that read correctly today must
 *  not move, and collateral on delivery_number / delivery_date / supplier_name must be 0.
 *
 ...
 *
 *    ELECTRON_RUN_AS_NODE=1 node_modules/electron/dist/electron.exe stress_test/inline_offset_veto_ab.js
 */
...
const ARMS = [
  ...
  { name: 'B offset-veto ON    ', delta: { ANCHOR_INLINE_TAUGHT_OFFSET_VETO: '1' } },
];
...
// `onDoc` (optional) is called with each file_done AS IT ARRIVES rather than at process close, so the
// arm can report progress every 10% instead of going silent for ~18 minutes (owner request
// 2026-08-08). Returns a kill handle on the promise for the opt-in early abort.
function runP(folder, args, files, manifest, extraEnv, onDoc, ctl) {
  return new Promise(resolve => {
    ...
    if (ctl) ctl.kill = () => { try { p.kill(); } catch {} };
    let out = '', tail = '';
    p.stdout.on('data', d => {
      out += d;
      if (!onDoc) return;
      // Line-buffered: a message split across two chunks is held in `tail` until its newline
      // arrives, so it is never parsed half-formed. `out` remains the source of truth below.
      tail += d;
...
(async () => {
  ...
  console.log(`INLINE TAUGHT-OFFSET VETO A/B — ${files.length} Pelican docs (${missing} skipped: file missing), 2 arms`);
  console.log(`live env: ${Object.keys(BASE).sort().join(', ') || '(none)'}`);
  console.log(`ANCHOR_INLINE_TAUGHT_OFFSET_VETO in baseline: ${BASE.ANCHOR_INLINE_TAUGHT_OFFSET_VETO ? 'ON (!!)' : 'off (correct)'}`);
  console.log(`ground truth: "Bramblewood Joinery Ltd"\n`);

  // ── PROGRESS CHECKPOINTS every 10% of documents (owner request 2026-08-08) ───────────────────
  // On the SECOND arm the first arm is already complete, so each document can be compared against
  // its own baseline the moment it lands: a REGRESSION is printed immediately, because that is the
  // number that decides this flip, not the heal count. AB_ABORT_ON=<n> stops an arm after n
  // regressions; the run then reports what it has and says plainly that it is partial.
  const ABORT_ON = Number(process.env.AB_ABORT_ON || 0);
  const res = {};
  let partial = false;
  for (const a of ARMS) {
    const t0 = Date.now();
    const prior = res[ARMS[0].name] && a !== ARMS[0] ? res[ARMS[0].name] : null;
    const STEP = Math.max(1, Math.ceil(files.length / 10));
    const ctl = {};
    let done = 0, next = STEP, good = 0, healed = 0, regressed = 0, aborted = false;
    let fresh = [];
    const onDoc = (m) => {
      done++;
      const v = ef(m, 'customer_name').value;
      const ok = isGood(v);
      if (ok) good++;
      if (prior) {
        const pv = ef(prior[m.original_filename] || {}, 'customer_name').value;
        const pOk = isGood(pv);
        if (!pOk && ok) healed++;
        else if (pOk && !ok) {
          regressed++;
          fresh.push(`    REGRESSED #${idOf[m.original_filename]}  ${sv(pv)} -> ${sv(v)}`);
        }
      }
      if (done < next && done !== files.length) return;
      const tail = prior ? ` · healed ${healed} · REGRESSED ${regressed}` : '';
      const line = `  [${String(Math.round(100 * done / files.length)).padStart(3)}%] ${done}/${files.length}` +
                   ` · correct ${good} · wrong ${done - good}${tail}`;
      console.log(line);
      for (const s of fresh) console.log(s);
      fresh = [];
      while (next <= done) next += STEP;
      if (ABORT_ON && regressed >= ABORT_ON && !aborted) {
        aborted = true; partial = true;
        console.log(`\n!! AB_ABORT_ON=${ABORT_ON} reached (${regressed} regressions) — stopping this arm.`);
        if (ctl.kill) ctl.kill();
      }
    };
    console.log(`\narm ${a.name.trim()}:`);
    res[a.name] = await runP(RR, args, files, manifest, { ...BASE, ...a.delta }, onDoc, ctl);
  ...
  if (partial) {
    console.log(`\n⛔ PARTIAL RUN — an arm was stopped early by AB_ABORT_ON. The counts above are over`);
    console.log(`   the documents that finished; the rest are ABSENT, not correct. Not a gate result.`);
  }
