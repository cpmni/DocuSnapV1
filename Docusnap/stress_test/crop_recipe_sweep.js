...
(async () => {
  ...
  for (const rec of RECIPES) {
    ...
    results[rec.name] = per;
    const okN = Object.values(per).filter(p => p.ok).length;
    console.log(`  ${rec.name.padEnd(38)} ${okN}/${files.length} correct`);
  }
  ...
  for (const rec of RECIPES.slice(1)) {
    const per = results[rec.name];
    const recovered = baseWrong.filter(id => per[id].ok);
    const regressed = ids.filter(id => base[id].ok && !per[id].ok);
    console.log(`  ${rec.name.padEnd(38)} recovered ${recovered.length}/${baseWrong.length} [${recovered.map(i => '#' + i).join(',')}]  regressed ${regressed.length} [${regressed.map(i => '#' + i).join(',')}]`);
  }
