...
const report = path.join(fs.mkdtempSync(path.join(os.tmpdir(), 'sf-search-fn-')), 'report.json');
const env = { ...process.env }; delete env.ELECTRON_RUN_AS_NODE;   // a WINDOW needs Electron proper
const r = spawnSync(exe, [path.join(ROOT, 'scripts', 'search-window-harness.js'), '--report', report], { cwd: ROOT, env, encoding: 'utf8', timeout: 120000, windowsHide: true });
