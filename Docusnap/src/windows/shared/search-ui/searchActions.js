...
//
// Capability gating (client parity): each action that a transport may not be able to back is ALSO gated
// on window.SearchTransport.caps.* (absent = true) — see _cap() in searchState.js. Desktop-local actions
// (Open in Explorer / Open File / Edit in Review / Print) simply hide on a transport that can't do them.
...
function renderActions(doc) {
  ...
  if (doc.status === 'deleted') {
    ...
    if (canEdit && _cap('bin')) _btn(docSection, 'Restore', () => _afterChange(window.SearchTransport.restoreDocument(doc.id)), true);
    if (isAdmin && _cap('bin')) _btn(docSection, 'Delete permanently', async () => {
      const suffix = await window.SearchState.purgeSuffix();
      if (confirm('Permanently delete this document and its file? This cannot be undone.' + suffix)) _afterChange(window.SearchTransport.purgeDocument(doc.id));
    });
  } else {
    if (doc.status === 'confirmed') {
      ...
      if (isAdmin && _cap('sendBack')) _btn(docSection, '↩ Send back to Review', () => {
        if (confirm('Send this document back to the Review queue? It stays filed until you re-confirm it.'))
          _afterChange(window.SearchTransport.repairDeconfirm(doc.id, { source: 'search' }));   // card 8: the note names Search, not Learning Repair
      }, true);
      ...
      if (doc.has_file && canEdit && _cap('localFile')) {
        _btn(docSection, 'Open in Explorer', _openVia(window.SearchTransport.showDocumentInExplorer));
        _btn(docSection, 'Open File',        _openVia(window.SearchTransport.openDocumentFile));
      }
    } else {
      // Edit in Review: admin/edit only — enforced in main.js open-review-window-at handler.
      if (_cap('review')) _btn(docSection, 'Edit in Review', () => window.SearchTransport.openReviewWindowAt(doc.id), true);
    }
    ...
    if (doc.has_file && window.SearchState && window.SearchState.printAvailable && _cap('print')) {
      _btn(docSection, '🖨 Print', async () => {
        let r = null;
        try { r = await window.SearchTransport.printDocument({ docId: doc.id, source: 'original' }); }
    ...
    if (canEdit && _cap('bin')) _btn(docSection, 'Delete', () => {
      if (confirm('Move this document to the recycle bin? You can restore it later.')) _afterChange(window.SearchTransport.deleteDocument(doc.id));
    });
  }
