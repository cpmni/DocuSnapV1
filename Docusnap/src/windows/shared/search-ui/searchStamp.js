'use strict';
/*
 * searchStamp.js — the "Send or stamp…" popup + click-to-place + the stamped/original toggle.
 * Workflow+Stamping redesign 2026-08-28 (slice 2b). SHARED SEARCH UI — IO only through
 * window.SearchTransport.{stamp,workflow}.* (S4 2026-09-13: moved from the core-only search-stamp.js so
 * the client's search pop-out stamps and sends the same way). Self-contained: injects its own CSS + popup
 * DOM. The renderer only ever sends coords + a stamp type; the host (core main process / the core's /v1)
 * resolves the source, gates permission + access, and writes the immutable record.
 * Capability gates (absent = true): stampCreate ("+ New stamp"), workflowHistory (the decision history rows).
 */
(function () {
  ...
  async function _renderWaiting() {
    ...
    let mine = [];
    try { mine = (await T().workflow.inbox() || []).filter(r => r.document_id === _doc.id && (r.state === 'pending' || r.state === 'claimed')); } catch { return; }
    ...
    const resolve = async (decision) => {
      ...
      try {
        await T().workflow.resolve(r.id, decision, note, r.version);
  ...
  async function _renderStamp() {
    ...
    if (!_types.length) { try { _types = await T().stamp.types(); } catch { _types = []; } }
    const canCreate = _cap('stampCreate');   // a transport without a stamp-type write (the client) hides "+ New stamp"
    host.innerHTML = `
      <div class="sp-lead">Add a stamp to record a decision or a status.</div>
      <div class="stamp-chips">${_types.map(t => `<button class="stamp-chip" data-id="${t.id}" style="--sc:${esc(t.color)}">${esc(t.label)}</button>`).join('')}${canCreate ? '<button class="stamp-chip new" data-new="1">+ New stamp</button>' : ''}</div>
  ...
  function _newStampInline(host) {
    ...
    go.addEventListener('click', async () => {
      err.hidden = true;
      try {
        await T().stamp.typeCreate({ label: word.value.trim(), color });
        _types = await T().stamp.types();
  ...
  function _beginPlacement(type, note) {
    ...
    go.addEventListener('click', async () => {
      ...
      try {
        await T().stamp.place({ documentId: _doc.id, stampTypeId: type.id, box, page, note });
  ...
  async function onDocShown(doc) {
    ...
    let stamps = [];
    try { stamps = await T().stamp.list(doc.id); } catch { stamps = []; }
  ...
  async function _setView(doc, which) {
    ...
    try {
      const r = await T().stamp.currentPages(doc.id);
  ...
  async function _renderSend() {
    ...
    let recips = [];
    try { recips = await T().workflow.recipients(); } catch { recips = []; }
    let stampers = [];
    ...
    host.querySelector('#sp-send-go').addEventListener('click', async () => {
      ...
      try {
        await T().workflow.assign(_doc.id, toId, why.value, host.querySelector('#sp-send-note').value);
