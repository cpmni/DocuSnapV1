...
// in the recycle bin → Restore / Delete permanently. All role-gated (+ capability-gated).
...
function _resultItem(doc) {
  ...
  `;
  if (window.SearchThumbs) {
    // DE-PATHED: the thumbnail handler resolves the file server-side by docId; rows no
    // longer carry paths at all. (searchThumbs.js = the transport-fed lazy loader.)
    window.SearchThumbs.lazy(el.querySelector('.result-thumb-img'), { id: doc.id });
  }
...
function _renderToolbar() {
  ...
  if (!head) return;
  const n = _sel().size;
  if (!n || !_canEdit() || !_cap('bin')) { head.classList.add('hidden'); head.innerHTML = ''; return; }
...
// A transport without a bin / send-back hides those tools (caps; absent = shown as today).
function _renderRail() {
  ...
  const n = _sel().size, canEdit = _canEdit() && _cap('bin'), isAdmin = _isAdmin() && _cap('bin'), bin = _isBin();
...
// Wire the rail once (called from SearchUI.init after the DOM is ready). Delete dispatches
...
function _showMenu(x, y) {
  _closeMenu();
  if (!_cap('bin')) return;   // nothing to offer on a transport without a bin
  const n = _sel().size;
  const sfx = n > 1 ? ` (${n})` : '';
...
async function _act(kind) {
  ...
  const call = kind === 'delete' ? window.SearchTransport.deleteDocument
             : kind === 'restore' ? window.SearchTransport.restoreDocument
