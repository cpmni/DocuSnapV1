...
async function doSearch() {
  ...
  try {
    if (bin) {
      const deleted = await window.SearchTransport.getDeletedQueue();
      window.SearchResults.renderResults({ confirmed: [], uncommitted: [], deleted: deleted || [] });
    } else {
      const results = await window.SearchTransport.searchDocuments(getParams());
...
async function _binRows() {
  try { return (await window.SearchTransport.getDeletedQueue()) || []; }
...
function initInputs() {
  ...
  // IPC per id, so N events coalesce into one repaint. A transport without a push channel (the
  // client) simply lacks onBinChanged — the focus belt above still covers it.
  if (window.SearchTransport && typeof window.SearchTransport.onBinChanged === 'function') {
    let binTimer = null;
    window.SearchTransport.onBinChanged(() => {
  ...
  if (restoreAllBtn) restoreAllBtn.addEventListener('click', async () => {
    ...
    try { await window.SearchTransport.restoreAllDeleted(); } catch (e) { console.error('restore all:', e); }
  ...
  if (emptyBtn) emptyBtn.addEventListener('click', async () => {
    ...
    try { await window.SearchTransport.purgeAllDeleted(); } catch (e) { console.error('empty bin:', e); }
...
// A transport without a bulk restore (caps.restoreAll === false) never shows "Restore all".
function _setBin(on) {
  ...
  if (r) r.style.display = (on && _cap('restoreAll') && window.SearchState && ['admin', 'edit'].includes(window.SearchState.role)) ? '' : 'none';
}
