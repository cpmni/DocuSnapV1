    ...
    try {
      ...
      if (req.method === 'POST' && confirmMatch) {
        ...
        if (!r.ok) {
          ...
          // `nearMatch` rides along ADDITIVELY on an ISSUER_NEAR_MATCH refusal (Oracle C4, 2026-09-25): the client can
          // then show both spellings + the count, and a future client Use/Keep affordance has its payload. Never a
          // path; the existing {error, code} shape is unchanged for every other refusal.
          return sendJson(res, status, { error: r.error, code: r.code || null, ...(r.confirmedBy ? { confirmedBy: r.confirmedBy } : {}),
                                         ...(r.code === 'ISSUER_NEAR_MATCH' && r.nearMatch ? { nearMatch: r.nearMatch } : {}) });
        }
