...
contextBridge.exposeInMainWorld('docusnap', {
  ...
  discardStuckDocs:            (ids)     => ipcRenderer.invoke('discard-stuck-docs', ids),
  ...
  generateFilingSlips:        (count, opts)  => ipcRenderer.invoke('generate-filing-slips', count, opts),
  ...
  batchAuditSendBack:    (eventId, docId) => ipcRenderer.invoke('batch-audit-send-back', { eventId, docId }), // Quick check → send one doc back to Review
