...
## 2026-09-04 — FILING_SANITY_REF_HISTORY_SOFTEN (Gate-C absent-note soft path on HISTORY, no live corroboration; doc238 RFH0738865/RFHO738865; extends the mig-111 live soften) — SIGN OFF WITH CONDITIONS
Extension: when the live soften (clause 1, >=2 live page families) does NOT fire but rv is an EXACT confirmed literal + the page's nearest token is a BACKED one-glyph confusable (`_near`, `_one_confusable_diff`) + no clip container, write the truthful soft note instead of "doesn't appear". Auto-file-NEUTRAL BY CONSTRUCTION: Gate C only runs on a NOTELESS field and writes a note either way; trust.js blocks on any note. So the ENTIRE risk surface is the note-MARK ripple, not filing.
Premise verified: live path CANNOT fire this class (winner 'anchor' NOT in _CORROB_PAGE_FAMILIES, agree empty -> clause 1 fails) -> history path is the only route.
Seam (three absent-mark consumers): engine P-lane (:4012) refuses anyway (interior confusable -> adopted==val; strongly-confirmed rv -> both-forms refusal); classFixService CLEARABLE (js:64) never targets an already-correct value; renderer _neitherOnPage (:4238) is THE seam — fine for doc238 (corrected_to==val, isApplied) but a softened sibling with corrected_to!=val (off-page) loses the "neither on page" button-suppression (Chris r6 card5). Compound with FORMAT_VARIANCE_RELAX_REF safe (soften note-preserving).
Conditions: C1 leg-b unambiguity (SHIP-BLOCKING: _near not itself confirmed; rv the ONLY confirmed literal within one backed-glyph of _near — closes two-confirmed-neighbours) · C2 own kill switch DARK, mig 117 seed-OFF + 118 force-ON · C3 keep BACKED (_one_confusable_diff, not _same_length_one_glyph; unbacked 5<->8 stays scary/live-path) · C4 bilingual mark hygiene (reuse live-soften note OR distinct mark; escape all 3 consumers + _is_verification_doubt_note) · C5 renderer: only fire when value settled (corrected_to empty or ==rv) so _neitherOnPage stays moot · C6 isAutoFileEligible False (auto-file byte-identical) · C7 RED-first pins (doc238 heal + ABSENT_MARK not in note / unbacked stays scary / non-literal stays scary / TWO-NEIGHBOURS fixture reproduces the ambiguity & refuses / non-sweepable / not-auto-file-eligible) + WARM-DB census (softened value == on-page-labelled, 0 new auto-files) + realdoc M=0.

## 2026-09-04 — CORRECTIONS->CONFUSION PRECEDENCE (mine corrections into per-scope OCR-confusion facts; reggie+gary) — 2a: SIGN OFF WITH CONDITIONS (review-bound, DARK); HIGH: CONDITIONS LOCKED, NOT A BUILD APPROVAL
Generalises signed leg-b (unambiguous_near_miss) from "needs the exact confirmed literal" to "needs a backed confusion FACT + singleton-ball" so it corrects a NEVER-SEEN serial. 2a DARK/review-bound (note+corrected_to+conf<=70, held by existing gates, no trust.js change) -> OFF==ON byte-identical, blast radius contained.
SEAM 1 (2a): the B7 reducer exclusion "exclude corrections backed by +confusion_resolved" is UNIMPLEMENTABLE — corrections has NO method column (learning.js:370-377); the only join (extractions.extraction_method) SURVIVES a human override (updateExtractionValue :385-388 never touches method), so it would tag a genuine counter-edit as arc-backed and DROP the self-heal. And B7 is ALREADY closed by no-row-on-accept (renderer :4388/:4407: data-original==display -> accept writes zero rows). -> DROP the exclusion; PIN the no-row-on-accept invariant instead.
SEAM 2 (HIGH, Q-HIGH-2 = YES): _critFieldCorrobRelax clears the 88 floor inside the weak-critical-field block (trust.js:1119-1127); the '+confusion_resolved' hold must be a TOP-LEVEL unconditional leg, not nested/short-circuitable, or a doubly-weakened path auto-files.
SEAM 3 (HIGH): trust.js "re-derive the bar" is a JS<->Python parity surface — must re-count support/counter/singleton-ball from LIVE corrections+value_counts (SQL/getFieldFormats), use resolution_meta only for {from,to,pos}, never trust note-absence or the engine's self-reported tier. If you won't own the parity port, KEEP the <=70 cap (don't ship HIGH).
'' FALLBACK (verified engine.py:10123-10124, existing _xsupplier=_sup_fmt is None :10134): cross-supplier facts -> wrong-but-held pre-fills (2a) / cross-supplier MISFILE (HIGH). Supplier-scope required for HIGH (ship-blocking), conditioned for 2a (nearly free — reuse _sup_fmt).
2a conditions: A1 supplier-scoped-only · A2 min support N (>=3 docs, >=2 distinct corrected-values, counter==0; not N=1) · A3 both-forms honest note · A4 drop method-exclusion + pin no-row-on-accept. Gate: RED-first (heal / genuine-serial-with-to-glyph not corrupted / cross-supplier refused / N-floor / one-counter demotes / OFF==ON) + realdoc M=0. Mig 119 (max applied = 118, verified).
HIGH conditions (NOT approval): H1 unconditional hold leg · H2 live re-derivation (parity pin) · H3 S_AUTO>=10 DISTINCT docs AND values + session spread + >=2 pixel sources as a SEPARATE counter from corroboration · H4 supplier-scoped-only · H5 census (independent GT; 0 misfiles on 752/782 + cross-supplier + doubly-weakened + stale-meta-after-reprocess + genuine-real-serial + rubber-stamp-no-row + reducer-keeps-counter-edit) + M=0 AND zero per-field accuracy regression, prod provenance on.

## 2026-09-04 (late) — CONFUSION PRECEDENCE 2a RELOCATION (the signed leg-b site is unreachable for the ref role; gary redesign) — SIGN OFF WITH CONDITIONS O1-O9; HIGH tier still NOT approved (+H6)
PREMISE CORRECTION (verified): the leg-b/leg-a site (engine.py text branch `if key in text_field_keys:`) EXCLUDES every ref-role field of a ref-NAMED type (`text_field_keys` = text-typed AND not `_is_ref_field`); a `reference`-typed `reference_number` never enters it. RESOLVE_REF_NEAR_MISS / RESOLVE_REF_POSITIONAL never executed on the exhibit (predicate pins green = the dead-guard class); doc138's heal came from Stage 2.5b. 2a is the family's FIRST live arc — its safety case stands alone (R5: never say "extends signed leg-b").
SEAM 1 (ship-blocking): gary's post-4.5/pre-Gate-C site would note the field BEFORE every page-witness gate (Gate C :6731, Fix A/G1, caption/S-C/prefix/D1 chain), which all abstain on a noted field — silencing the only modality that can falsify an extrapolated value. SEAM 2 (ship-blocking): `corrected_to == value` renders the green "auto-corrected" badge with a false tooltip (renderer :4223/:4318). SEAM 3: `corrected_to`/`was_corrected` are read as a HUMAN act by processing/handler.js:1441 (reprocess carry) and batchAuditService:147.
A1 `_needs_review` write is DEAD (:10713 assigns unconditionally; validator.needs_review never reads a note; 70 is not < 70). A2 the machine-confirm exclusion hides ~90% of the confirmed substrate — the "never-seen" ball on value_counts alone is not the stated guarantee. A3 the ball was case-sensitive. A4 precise-typed fields reached 2a un-noted. A5 `+confusion_resolved` broke the crosscheck-outlier equality test.
O1 call AFTER `_flag_digit_disagreement`, BEFORE the learned-agreement boost (every gate judges the RAW read; 2a fires only on a value that passed them all) + a NEW source-order pin (the CLAUDE.md-cited tests/test_validation_pass_order.py does not exist). O2 write NEITHER corrected_to NOR was_corrected (raw_value + method + note carry provenance). O3a `confusion_literals` = keys(value_counts) ∪ keys(machine_value_counts) built ONCE in processing/handler.js (Python never reads the channel); refusal-side only, value_counts stays the licensing precondition. O3b from-glyph ATTESTATION: any known literal of that length carrying `from` at `pos` kills the fact (protects the UNSEEN sibling W2S9999999 the per-value ball cannot reach). O3c casefold. O3d consumer pin 2->3 with the justification + a licensing-precondition source assertion. O4 return a bool, OR into review_needed. O5 the house deny set (`_reconcile_clipped_suffix._skip_types`) + list/barcode/name-like/identity + method == anchor_crop_crosscheck + user-set methods + already-noted. O6 overriding a label-confirmed read is a PINNED decision; O6b fix the false leg-b/leg-a comments + reachability assertions in the relocation commit. O7 the mark stays out of every clearer set (pinned bilingually). O8 the note names both forms, the evidence and its size, and says the value has NOT been seen before. O9 flip gate: G1 OFF==ON + realdoc M=0 with the arm asserted ARMED (proves inertness, not safety); G2 constructed census on a db.backup() copy vs INDEPENDENT GT (heal / F5 machine-confirmed AND its unseen sibling refused / 752-782 / cross-supplier / N-1 / counter / taught read overridden review-bound / reprocess carry / accept-no-row / edit-back kills); G3 zero per-field regression with prod provenance ON; G4 the pin set.
REFUSED: R1 the HIGH auto-file tier (+H6: 2a must run WARM with a measured false-correction rate first); R2 corrected_to/was_corrected; R3 adding the mark to any clearer set; R4 relocating leg-b/leg-a in this commit; R5 "extends leg-b" claims. Log Stage 2.5b's silent @95 rewrite (invisible to trust.js) as its own arc.
BUILT per the conditions (same session): engine._apply_confusion_precedence + the `_cp_fired` OR; confusion_literals union in handler.js; attestation + casefold in confusion_correct; consumer pin 3; pins test_confusion_precedence.py 34 / test_confusion_precedence_wiring.py 41 / test_field_confusions.js 17 / test_confusion_no_row_on_accept.js 7 / test_confusion_autofile_hold.js 11. E2E on copies: the over-floor S->5 poison fact REFUSED on machine-confirmed W2S8745899 (doc55, attestation); doc176 OFF==ON byte-identical. NEW LIVE EXHIBIT for the leg-b relocation: doc176's fresh read `1625802868` @95, no note, needs_review false, overall 100 — one BACKED glyph (6<->G) from the human-confirmed `1G25802868` = leg-b's singleton-ball case, unresolved at the dead site -> an auto-file candidate carrying a misread serial.

## 2026-09-04 (late) — FORMAT_CLASS_JOIN (a MIXED-code scope loses its whole format entry; gary design) — SIGN OFF WITH CONDITIONS C1-C11
EXHIBIT: Print Tracker doc262 `752923124N3M2` @70 with the edge-cut "fuller reading could not be verified" note although the page prints it. PREMISE (verified): the mapper's `_edge_cut_relocate` HAD re-read the value correctly; consent failed because the scope had NO entry in format_class_index — `classify_format` takes the 3 NEWEST distinct values and requires unanimity of class, else FREETEXT, and `build_format_class_index` DROPS a FREETEXT non-name entry. Today's confirms put a pure-digit serial beside alnum ones → FREETEXT → entry gone → EVERY confirmed-literal arc (RELAX_REF/_INLINE, both Gate-C softens, leg-b, 2a, _has_no_usual_format) and the consent ladder silently inert for the sender; the morning pins bake a 19-value order that still classifies upper_alphanum (vacuous pins). Live: N=39, thr=4, 5 folded shapes clear the bar, 6/23 confirmed serials sit BELOW it; join over the distinct set = upper_alphanum; 0 charset violations; census: 1 scope joins (PT ref; the '' twin refused).
gary: A′+D under one DARK flag (join class; shapes kept but no veto; positive-evidence consent; no boost/fc_delta). ORACLE AMENDMENTS: (P1) the admission belongs in the INDEX BUILDER (classify_format byte-identical; `is_name_like_field` is free there; test over the DISTINCT set — a name scope whose 3 newest are single tokens could otherwise join); (P2) separators over the distinct set (the 5-newest pool would let a historic '/' value fail its own charset → hard null). SEAMS gary missed: class F (`_try_verification_doubt_clear`, ON since mig 91) treats "no shape entry → refuse" as its only hold on the edge-cut note and LIFTS to 90 on a clear; deskew `_shape_ok` and `shape_match_score` read `shapes` as a POSITIVE licence; the folded set is length-blind (`#`/`@#` accept every digit/letter-digit string of any length). SHIP-BLOCKER (Q8): the pad-window C5 two-sided swap + S4's 'confirmed' tier would commit `782923124N3M2` at 90 with NO note when the tight read is a clip of a confusable twin both of whose forms are confirmed literals — a silent wrong serial on a scope whose history demonstrably contains rubber-stamped misreads (1625802868 ×2, 782 ×1).
CONDITIONS: C1 admission in build_format_class_index over the distinct set + name-field exclusion · C2 separators over the distinct set · C3 a joined entry carries NO `shapes` key (count-gated set = the admission test only); attach length-aware `shape_families` · C4 class F explicitly refuses a joined entry (pinned) · C5 consent on a joined entry is REVIEW-BOUND at most (a distinct 'joined' token; every caller testing `in ('confirmed','provisional')` treats it as not clean — `_edge_cut_relocate` keeps the cap, the pad-window never clean-swaps) · C6 the consent evidence is length-aware (exact literal OR a >=3-doc `shape_families` variant; never the folded `shapes`) · C7 a joined scope flags, never blanks (`_xsupplier or joined` → keep-and-flag) · C8 supplier-scoped only, the '' twin never joins · C9 no `support`, joined keys out of `supported_keys` · C10 the truthful note ("read again from the label — matches a value you have confirmed before — please confirm it once"), not in any clearer set · C11 sequencing: revert the TEST force-ON migs BEFORE the corpus census; the join lands before 2a is trusted (2a's shape-sanity refusal is knowingly dead on joined entries); flags stay independent.
GATE before FLIP: `python_backend/tools/format_join_census.py` over the live copy AND the 605-doc corpus (hard: 0 name/whitespace joins, 0 '' joins, 0 charset violations incl. separators; the C6 coverage number per scope) · realdoc M=0 + zero per-field drop + a BLANKED-FIELD delta (must be 0) + a FLAG delta · pins (below) · e2e doc262 + a no-dissent sibling.
BUILT per the conditions (same session): `join_code_class` / `_joined_separators` / `_count_gated_shapes` / `join_format_entry` + the admission in build_format_class_index (no shapes, families attached, no support); `template_mapper._joined_consent` + the 'joined' tier in `_shape_consents`; `_JOINED_LITERAL_NOTE` at `_edge_cut_relocate` + the grow path; engine C4 / C7 / S5 + the 2a comment; env bridge `format_class_join`; mig 120 seed OFF (no force-ON twin). Pins: `test_format_class_join.py` 51 (RED-first on the LIVE 20-value order; lattice/refusals/admission guard/order-invariance/C5-C6 consent/C10/C4/C7/S5/OFF==ON); every existing format/mapper pin green unedited incl. `test_format_anomaly_checker.py` §2. Census on the live payload: 1 scope joins, hard lines clean, coverage 19/23. E2E: doc262 → `752923124N3M2` @85 with the TRUTHFUL Gate-C soften note (review-bound); sibling doc146 (no page dissent) unchanged vs OFF.
DEFERRED (own flag + census): the UNANIMOUS-by-accident twin (three digits-only newest ⇒ DIGITS_ONLY ⇒ lettered serials hard-null) — derive the class from the values backing the count-gated shapes. LOGGED (pendingfeatures): Stage 4.5 hard-nulls an exact confirmed literal on any non-joined shaped scope (no literal exemption, unlike `_gate_value`).

## 2026-09-04 (late) — RELOCATE leg-b/leg-a (RESOLVE_REF_NEAR_MISS / RESOLVE_REF_POSITIONAL) — SEND BACK → narrowed → SIGN OFF WITH CONDITIONS C1-C11
PREMISE (verified): the old site is FIVE-ways dead for the ref role — `text_field_keys`, the label-confirmed exemption, the already-noted skip, the FORMAT_VARIANCE_RELAX∧_has_no_usual_format conjunction, AND `if anomaly:` (leg-b could only ever judge a shape-FAILING read; the new site judges every clean read — a fourth widening axis that must be declared, and is required to heal the exhibit). Latent defect: the old `''` doc-type fallback could have proposed ANOTHER supplier's literal. F5 (gary): `raw_value` never persists on the import path (handler.js builds raw and display from `value`) → 2a's "searchable" claim false; the both-forms NOTE is the only durable record. C1 (ship-blocking, done first): the page of docs 176/210 RENDERED — it prints `1G25802868`; the human-confirmed `1625802868` is a rubber-stamped 6<->G misread of the same device (identical IP/MAC) — the exhibit is a genuine heal.
SEAMS: S1 leg-b × FORMAT_CLASS_JOIN (DARK-on-DARK — the join was signed as an EVIDENCE channel; leg-b promotes it to a value authority; re-run the join census with the leg armed); S2 a SUGGESTION accepted via "Use L" writes `corrections[refKey]` = the `classFixService.applyForConfirm` trigger (up to 25 siblings; bounded — `deriveClassFix` needs an alpha head; DARK) — name it, census it; S3 the method suffix (`+ref_resolved`, also 2a's `+confusion_resolved`) breaks `_crosscheck_witness_bucket`'s EXACT family match → the winner's own crop read is recorded as a disagreeing family (conservative for filing, corrupts the record) → NO method suffix; S4 a PRE-FILL starves 2a's substrate (an accept writes no corrections row) — a SUGGESTION accepted writes exactly the fact 2a mines; S5 under SUGGEST the page-witness modality is NOT spent (the stored value is still the raw read).
CONDITIONS: C2 v1 of BOTH legs = SUGGESTION (value/display/raw unchanged, corrected_to = L, conf<=70, note; never was_corrected; no method suffix) — PRE-FILL is a Phase-2 promotion behind the census · C3 port the from-glyph attestation to leg-b (any known literal of the same length carrying the READ's glyph at the diff position → refuse) · C4 attach `confusion_literals` independently of `confusions` (else D6's union is dead) · C5 `unambiguous_near_miss` in place: casefolded union ball → singleton → resolve to value_counts in original case (0 = machine-only refuse; >1 = case-ambiguous refuse) · C6 no support floor, but the note states the count + glyphs; marks out of every clearer; never "one character differs" / "doesn't appear on this page as written" (renderer triggers) · C7 do NOT persist raw_value on the import path (handler.js:1481 class-fix survival + sweepPredicate/exportService read it); correct the false claim now · C8 mig 121 UPSERTs both settings to 'false' (a relocated feature is born DARK; discharges TEST migs 114/116; the owner arms by an explicit settings write) · C9 flip gate: arms A/B/C × join ON/OFF after reverting the TEST migs: realdoc M=0 + zero per-field drop with the arm ARMED; the AUTO-FILE ELIGIBILITY DELTA (docs eligible OFF but ineligible ON — the review-load price, per leg); leg-a fire count + wall-clock; a constructed census on a db.backup() copy vs the RENDERED PAGE (heal-or-refuse doc176, 752/782 ball-2, machine-only neighbour, cross-supplier, unbacked/short/no-counts, attestation, taught-mapping read overridden, accept-writes-a-corrections-row, `ref_class_fix_enabled` composition, OFF==ON) · C10 pins incl. reachability, the suggestion shape, three independent auto-file gates each alone, the machine-only dead zone + no-floor + no-suffix + accepted-suggestion-writes-a-row as deliberate · C11 leg-a stays DARK behind its own flag with its own flip (may ask, never answer).
BUILT per the conditions (same evening, `f76e8d1`): `_apply_ref_resolvers` before `_apply_confusion_precedence`; old blocks deleted; `unambiguous_near_miss` union + human licensing + casefold + attestation; union attached on every supplier group under either flag (handler.js) and independently of facts (index); mig 121; docs corrected. Pins: `test_ref_resolvers_wiring.py` 53 · `test_resolve_ref_near_miss.py` 24 · `test_resolve_ref_positional_wiring.py` 13 · `test_ref_resolver_autofile_hold.js` 10; every existing pin green. E2E on the purged copy from the FILED PDF: the fresh winner was the anchor family's double-slip `I625802868` and Gate C noted it first → the pass deferred (O1 ordering proven); the heal class is carried by the wiring pin on the real index. OWED before FLIP: C9 in full (the eligibility delta is the number the owner must see).

## 2026-09-05 — LOG-REVIEW FIX PLAN (5 detection items + 2 main-process bugs; gary/007/reggie/eric/herald consensus) — per-item verdicts; design `docs/designs/LOG_REVIEW_FIX_PLAN_2026-09-05.md`
PREMISE (verified per item): 1 Meadowvale label-less authoritative currency anchor = pure absolute crop on a floating totals block (`_skip_rigid` :504-507 needs a label; every relocation road needs one; not-located → 50, no note; `_is_weak_read` text-only → the registration trigger is exactly `not value`). 2 `_is_bare_label` token-EXACT (:2426-2430); `_caption_top_limit` on the WIDENED box (:1159 before :1167); registration rung has zero caption defences; Guard B / `_RELOCATE_METHODS` INERT on the exhibit (`dq<0.6`, `USTOMER` scores 1.0). 3 date pattern 2 lookbehind digit-only; MDY fallthrough. 4 Statement bucket has no bare name; only installed names fold; `DETECTED_SLUG_FALLBACK` re-arms `_type_refuse` for same-supplier templates. 5a `templatesDir` absent from the register() destructure — product effect NIL (Python reads `--templates-file`); the :62 regex pin literally pins the buggy text. 5b `pythonArgs()` with no script → `['-3.12', undefined]`.
CORRECTIONS: the sign lever is the PATTERN (no shipped currency pattern admits `-`, :703-704) — `_clean_text_fallback` :2947 is a second amputation; fix both under `MONEY_SIGN_CAPTURE`. Gary's "value None + note" has NO emission path in anchor.py and a note on a keyword incumbent would hold every Meadowvale INVOICE.
SEAMS: S1 refuse the rigid read alone → registration becomes the Tier-A committer (guard both, one commit); S2 the Stage-0.5 mapper calls `_crop_is_credible` WITHOUT a label (template_mapper.py:1244,1248) — passing one = new EXACT behaviour → own slice; S3 clamp threading + registration band mirror must land together; S4 5b REVIVES a dead arm (templates 6/7 compose changes) → sibling run, never re-darken; S5 item 4(b) re-arms `_type_refuse` on same-supplier templates (the designed docket road) — HYPOTHESIS until the diff.
VERDICTS: 1 SIGN OFF W/COND (both layers; read-side refuse+registration guard one commit, DARK `anchor_labelless_currency_refuse` mig 122; note ONLY when the field would end EMPTY (mirror `issuer_band_withheld` :9582-9589); teach-side = page-words caption + WARNING, refuse nothing) · 2 SIGN OFF W/COND (slice A DARK `anchor_bare_label_fuzzy` per caption TOKEN, skip digit tokens; slice B reorder :1167 safe for the :1944-1946 pin + clamp at :574/:1250 for authoritative+labelled+below + band mirror, one commit) · 3 SIGN OFF W/COND (instrument `_crop_is_credible`'s date branch :2667 before the census) · 4 (a) DO IT (owner presets) · (b) SIGN OFF W/COND DARK `type_uninstalled_heading_fold` seg0-only top-band · (i) WRONG LAYER → copy fix · (ii)/(iii) SEND BACK (census / prove additive) · 5a SIGN OFF (executing pin + negative control + a bare-helper lint) · 5b SIGN OFF W/COND (sibling run; do NOT re-darken).
ORDER: 5a → 3 (+instrument) → 2A → 2B → 1 read → 1 teach → sign lever → 5b → 4 presets/(b) DARK/(i) copy. MEASURE FIRST: label-less currency `field_anchors` census + planted corrections; Meadowvale eligibility delta; reorder band count; date crop-site census; 4(b) re-detect diff. NOT BUILT.


## 2026-09-06 — LOG-REVIEW FIX PLAN BUILT (autonomous, owner order "start 5a then the others in the recommended order") — per-slice record + gate numbers
BUILT in the signed ORDER, every slice its own commit + pin, sibling pins green, DARK arcs seeded OFF by mig 122 (UPSERT): 5a `dfc3cc4` (executing pin + negative control 7 reds + the bare-helper lint) · 3 `70bfe3a` (alnum lookbehind on BOTH numeric patterns; `_crop_is_credible` date branch census-hooked per the condition; harness `RR_CFG`/`RR_PY_ROOT` for frozen-snapshot arms) · 2A `c912c09` `anchor_bare_label_fuzzy` (per TOKEN, digit tokens skipped, <5/<4 never compared, joined + per-token roads; the mapper site untouched per C2) · 2B `03be597` ONE commit (clamp on the UN-widened box at BOTH relocate rungs; clamp threaded to rigid + registration; band reject mirrored at the registration commit; rides RELOCATE_CAPTION_EXCLUDE + CAPTION_BAND_REJECT; the reorder-band census instrumented) · 1 read `ba454bf` `anchor_labelless_currency_refuse` (rigid + registration refused under ONE predicate; the note road = engine `_apply_labelless_withheld_notes` running LAST, note ONLY when the field would end EMPTY, mirror of issuer_band_withheld) · 1 teach `c09c16c` (WARN + typed-caption input for currency-without-caption, refuses nothing; page-words sourcing NOT built — owed) · sign lever `799c212` (MONEY_SIGN_CAPTURE re-attach at the anchor mint; the FLIP stays the owner's) · 5b `8a26f6f` (argv fixed + stderr; sibling run owed; never re-darken) · 4b `25a0cfe` `type_uninstalled_heading_fold` (heading-only phrases: top band + seg0 == phrase, never a mention, strictly additive) · 4i `cc41c9f` (copy only). 4a = owner action (both presets already in the catalog with the right aliases).
MEASURED: (1) field_anchors census (agent, db.backup() copy): label-less authoritative currency anchors = 0 TODAY — the Meadowvale row was deleted by `learning.clearAnchors` on the owner's TYPED sign corrections (document_type-BLIND predicate `learning.js:673-686`; a re-teach re-plants the class) → gate (b) runs on a COPY with the row re-planted at the diagnostic geometry. (2) Item 3 census over 2,570 date-site records (crop 2,091 / keyword 479): A old-accepted-new-rejects = 2, BOTH Pelican `P1/26/…` code windows; B window-moved = 0 → the affected population is exactly the exhibit class. (3) Item 4(b) re-detect over the live copy's 541 stored texts OFF vs ON: changed = 20, ALL Invoice@80 → Statement@95 heading=True (the Ironclad 20, all needs_review); 521 unchanged — MET on the live corpus (Desktop 605 + Hard Set = PDFs, owed with OCR). (4) Item 3 realdoc arm `new` (ALL 453 confirmed docs of the RESET live DB; the 605-corpus RR_IDS list now selects 15 — vacuous): type 453/453 · supplier 453/453 · ref 448/453 · date 452/453; regressions 6 / 4 SILENT, ALL refs of the leg-b/2a class (Print Tracker 8↔5, 1G↔16 ×2, HS↔H5) + doc 471 Pelican 0023 flagged both ways — its confirmed date `26-01-6000` is POISONED GT (the new read `05-01-2026` is the page). `old` arm + `b_only` / `arcs_on` / `mv_off` / `mv_on` arms queued (see HANDOVER_2026-09-06.md; numbers to be appended below when they land).
OPEN SEAMS FOUND: `clearAnchors` document_type-blind + self-limiting only on typed corrections (pendingfeatures) · the live-DB reset makes the 605-corpus DB harness vacuous (a folder-corpus WARM harness does not exist; `customer_corpus_score.js` is the COLD model) · doc 471 GT poison.
ARMS LANDED (14:45): Item 3 `old`→`new` 453 docs — identical accuracy + identical 6 regressions, wouldFile delta 0; the only diffs are the deskew-retry note ("was '26-01-6000'" → "was '(empty)'") = the pattern heals the FIRST read. 2B alone — M unchanged, fill 20/20 + 60/60 unchanged, 9 Oakhaven `USTOMER`-class registration commits → `Bramblewood Joinery Ltd`; eligibility −4 (3 = wrong-name auto-files stopped, 1 = doc 453 right-but-held by the two-names demoter counting a caption-band-REJECTED read as a dissent — seam logged); reorder band 25 records, 17/17 value changes (12 heals, 5 address-garbles refused downstream). Three DARK arcs ON — 0 diffs, wouldFile identical (INERT live). Item 1 Meadowvale copy — 18 wrong-row @50 totals → 20 truthful re-teach notes; eligibility −2 (426 the teach doc, 433 a lucky right read) = the pinned trade-off; no doc newly eligible with a wrong value → MET. Flips remain the owner's.
FOLLOW-ON (15:05): 5b sibling run (templates 6/7 NULL vs healed angle, 30 siblings) 0 diffs, wouldFile identical → MET. Hard Set 3 arms: wrong+would-file 0 in cold arms; the two moved classes (table_total, logo_siblings) + the warm buyer_large 4 wrong+would-file are REPRODUCED EXACTLY by the pre-today python/config (`e68430e`) on the same DB copy → the DB reset (Statement uninstalled; Bramblewood's real PO template claims the buyer-issued POs = the 07-12 doctrine / GT flaw), ZERO delta from today's commits. `score_hard_set.js` gained RR_CFG/RR_PY_ROOT.

## 2026-09-07 — ONE-CONFIRM BUYER-ISSUED CONVENTION LICENCE (owner ask; gary design → Oracle) — SIGN OFF WITH CONDITIONS C1-C8; BUILT `d514a15`
PREMISE (verified): the convention note (mig 98 ON) clears only on a same-type `supplier_name` hint usage >= 3 — C4 (2026-08-31) was "honest arithmetic", not a safety margin: a generic bump cannot tell a NOTED confirm from any other. A `buyer_issued_convention` record written at the one moment the system knows the human answered THIS question is better evidence per unit; the value is never rewritten, so the rubber-stamp cost on doc #1 = the doctrine default (= today after 3). The vendor-caption read is already dropped on POs (2026-07-12). `accept-issuer` is a broader type-blind one-act road that already exists.
SEAM (Oracle): the undo road was ASYMMETRIC — `retractConfirmHints` walks field keys, so a Learning Repair send-back would have left the licence standing → C2 (blocking): retract through the planting confirm's audit row, never by value. Sibling-fill keys on `letterhead` methods, the note on template_fixed/hint_text_match/logo → mutually exclusive. A stored-row note-clear on the 8 siblings is REJECTED (the note is their ONLY hold at @95; clearing it makes them Tier-1 eligible off one click, and the sweep the same confirm schedules would file them with no re-read).
CONDITIONS: C1 both wordings license (the bare TO:-block case is the same question) · C2 retract via the audit row; replant never replants · C3 write only when the confirmed type == the type the doc was read as · C4 bridged `env == null && setting === 'true'`, engine `!= "0"`, harness OFF arms explicit '0'; the leg lives inside the parent's block · C5 told with undo through reviewEvents (kind `convention`, undo deletes the exact record, audited) · C6 no quiet-lane 'convention' reason now; a post-confirm hint (the `_positionHint` shape) offers "Re-check N other …" via the EXISTING reprocess-from-sender; slice 2 only with a usage >= 2 arming threshold · C7 memory readers label the pseudo key · C8 mig seed OFF, no force-ON twin (the owner's explicit build order overrides for the VM test: mig 126, in the revert list).
GATE: Python truth table (one noted answer licenses; supplier_name usage 1-2 do NOT) + JS record/retract + reviewService H1/H2 N1-N6 C3 + the undo road + the realdoc OFF==ON (vacuous: no records live) + the load-bearing copy-DB warm arm: noted → one confirm → re-run → 0 noted, values unchanged; bulk control still noted; retract arm restores the note. Flip gate = that census + a sandboxed Chris run on a VM-shaped fresh DB (9-PO batch, confirm #1, take the nudge, expect the consent bar with 8 and zero notes; negative: edit #1 to the vendor → the other 8 stay noted).
RESULT (off 7/7 noted, wouldFile 0 · on (record x1) 0/7 noted, values unchanged, wouldFile 4 (= the licensed baseline) · ctrl (switch ON, no record) 7/7 noted — the record is load-bearing. Three Hard Set scan). Residual named: a colliding logo silences the note for the wrong company after one answer instead of three (the identity-conflict arm still catches it).


## 2026-09-07 (afternoon) — IMPORT SPEED (oscar + gary consensus; owner "import is notably slower") — per-slice: SIGN OFF W/COND slices 1+3 · SEND BACK slice 2 (→ conditions met, built) · DO NOTHING slice 4
PREMISE: "slower than it used to be" is true and mostly self-inflicted — the owner's own switches (light-text OFF alone −5.5 s of 26 s; `deskew_review_retry` doubles a tilted review-bound doc); the plan recovers ~6 s. The profile ran uncapped (no OMP_THREAD_LIMIT) so the −4.1 s for slice 2 conflated pool speed-up with an OMP regime change. `RR_DUMP` carries no per-field tuples — the gate must diff `RR_CONSENSUS`.
SEAMS: S1 a memoised OCR FAILURE (`_ocr_lines` → `[]`) served by a shared cache to every Stage-2 relocation/label-lock = the 07-17 "one silent-wrong path" relocated (→ C1 None-never-cached); S2 the pools LOWER an exported parent cap (`OMP_THREAD_LIMIT='1'` unconditional) — the 08-11 'ACC-2291'/'ACC-229]' class reintroduced for cap≥2 boxes (→ C6, own commit); S3 slice 2's `threadCap===1` predicate is a vacuity trap (a "very fast PC" may be cap 2-3) → measure (C5: this PC = 16 cores/conc 10 = cap 1) and predicate on "a cap is exported"; S4 the parity pin §E is blind to call-site env (→ C8 §F).
CONDITIONS: C1 `_ocr_lines` None on exception, never cached · C2 cache stamped with its fn · C3 all three anchor sites · C4 pins (composed landmarks miss/hit, raising fn, identical Transform) · C5 report cores/conc/cap · C6 OMP-inherit first · C7 own key `ocr_parallel_import_enabled`, NOT in ALL_ON_DEFAULTS · C8 parity §F both ways · C9 single-flight per key in the pooled locates · C10 light pass belt = whole pass serial on any raise · C11 every pool honours DS_OCR_POOL_WORKERS. Slice 4: do not build (the `:950` first-mapping-wins short-circuit; unmeasured reward).
BUILT: `8b8b095` slice 1 (gate BYTE-IDENTICAL 147 docs; honest yield −1 spawn on a tilted page — the compose shifts the landmark keys) · `5e40fa2` OMP inherit (cap-2 pools on/off IDENTICAL) · `e9f67a4` slice 2 (mig 127 seed OFF + 128 TEST force-ON; gate BYTE-IDENTICAL + 10× determinism 10/10) · `a0e3795` the thumbnail UX slice (the Oracle's zero-blast-radius suggestion). Slice 3 NOT built.

## 2026-09-07 (afternoon) — PLACEMENT ROOT CAUSE + HOLDS (007 + gary; four owner exhibits E1-E4) — SIGN OFF W/COND on P1(a)+P1(b)+P2+P3+P4 as an ORDERED set · SEND BACK P5 · WRONG LAYER P6
PREMISE (verified): `engine.py` reads a NULL `sample_deskew_angle` as 0.0 and composes by −θ_scan; every template on the install is NULL because the packaged detector script was compiled away (installed log ×7) and the commit-time write spawns the same script; the wizard stores boxes in the RAW frame via `_teachBackBox`, so the right θ_teach is `state.deskewImgAngle`. Strongest argument the specialists missed: the DESKEW sibling branch ALREADY abstains on NULL — P1(b) aligns the two. OVERSTATED: for a LEVEL sample with NULL today's half-compose is exactly right, so abstain alone swaps one wrong class for another (its own arm, DARK). Stationary-garbles-fail-the-gate is NOT a class property (this supplier's date sits far right; a centred date gets confident cut dates too). E2's shape-warn premise WRONG as framed — case is invisible to the format model for hyphenated codes (`shape_signature` folds letters; `classify_single` puts both in ALPHANUM_SEP); the installed-build mechanism = Gate A "not printed on the page as a whole token" on the raw pass + the derived rung's flag on the straightened pass (read from the installed log after the ruling). Registration is DEAD on this supplier (2/5 landmarks) — the stationary fallback leans on abs + inline only.
...
## 2026-09-07 (evening) — INLINE_DISAGREE_CORROB_SOFTEN (class G: reword the box-clip value-doubt note when 2 families corroborate; gary design) — SIGN OFF WITH CONDITIONS G1-G7
PREMISE (verified at source): `_INLINE_DISAGREE_NOTE` (template_mapper.py:502-503, "…the box may be clipping the first character; please check which is printed.") is minted at _pick_fuller_code:1770-1775 via `_mapping_result(shape_warn=True)` -> method `template_mapping_shapewarn`, conf capped 70. It is NOT in `_verification_doubt_note_marks()` (engine.py:1894) so F never sees it — the gap gary names is real. Reword>current: the note's real defect is a drifted taught box (re-teach), not value doubt — a system fix. CORRECTION: the lift-to-90 gary imported from F is a CATEGORY ERROR — F lifts because F CLEARS the note to AUTO-FILE (engine.py:4283-4287); G stays review-bound (note retained -> `_any_note` True at engine.py:11592-11596 -> `_needs_review` stays True; trust.js:869/:1077 blocks auto-file on the note independently), so the lift buys nothing G needs and REMOVES the 70<88 weak-critical backstop against a future note-stripper.
SEAM: `_SHAPE_WARN_NOTE` (template_mapper.py:114) shares the SAME method `template_mapping_shapewarn` — the MARK, not the method, is the sole discriminator; assert the mark absent from _SHAPE_WARN_NOTE + the raw-witness note ("one character differs") + F's allowlist. G MUST be its own class — folding the mark into F's allowlist -> F clears+lifts+auto-files on a same-length glyph-swap common-mode. Winner family = mapping-crop (engine.py:2276); a crop witness is a 2nd crop of the overlapping region (weak independence) -> restrict the witness to KEYWORD family (the canonical independent pair, :2278-2279).
CONDITIONS: G1 cap conf <=70 (never 90; pin <88) · G2 own class, mark-absent + mark-is-discriminator pins · G3 keyword-family witness only + soft check_value/fail-open shape belt · G4 reword must not assert correctness — keep "confirm once" (confirm already mandatory) · G5 pin the coupling (soft belt safe ONLY while conf<88 + note retained) · G6 `== '1'` idiom (empty reads OFF), not `!= '0'` · G7 seed OFF mig 135 (HEAD=134), force-ON mig 136, both on the revert list (…/128/134 +136). GATE: OFF==ON realdoc M=0 + per-field accuracy delta 0 (G edits no value) + fire census {stays needs_review, value byte-identical, note reworded, conf<88}; anti-regression pins raw-witness/money/identity stay flagged, mark not in F's allowlist, recompute returns needs_review True. Slice-2 (full clear+auto-file, F's strict rail restored) DEFERRED, separately gated.
BUILT: template_mapper `_INLINE_DISAGREE_MARK` + `_INLINE_DISAGREE_SOFTENED_NOTE` (mark factored out, shared write+resolve); engine class G `_try_inline_disagree_corrob_soften` (env `INLINE_DISAGREE_CORROB_SOFTEN` == '1', keyword-only witness, soft belt fail-open, conf min(conf0,70), reword-not-clear, provenance `inline_disagree_softened`); mig 135 seed OFF + mig 136 TEST force-ON; setting->env wired in processing/handler.js. Pin test_inline_disagree_corrob_soften.py 22/22 (exhibit heals reworded+conf70; raw-witness/crop-only/unlicensed/money/identity refuse; OFF==ON; fail-open on no-entry; mark-discriminator + not-in-F-allowlist). Neighboring test_corrob_note_resolve 51/51, test_verification_doubt_clear unchanged (its 1 pre-existing fail is unrelated). OWED before flip: OFF==ON realdoc M=0 on the 44-doc inline_disagree cohort + the fire census.

## 2026-09-07 (night) — PRE-DEPLOYMENT AUDIT vet (eric/gary/oscar/main-Claude consolidated; docs/PRE_DEPLOYMENT_AUDIT_2026-09-07.md) — SIGN OFF WITH CORRECTIONS
VERDICT: SIGN OFF WITH CORRECTIONS. Audit sound + actionable; no load-bearing finding WRONG. Traced P0/P1/P2 at source.
CONFIRMED P0-1 mechanism: 12 UPSERT force-ON blocks (108/110/112/114/116/118/123/124/126/128/134/136) in database/index.js, each `ON CONFLICT DO UPDATE SET value='true'` + migration-stamped (revert code leaves setting ON, mig won't re-run, no down-migration); build (package.json:9) has NO gate; scripts/check-release-migrations.js absent. "Silent wrong auto-file" REAL: template_mapper.py:150-152/:167-168 — mig 108/110 clearing shape_warn drops cap+note, "docTrustGate _codeish does NOT re-catch a code-class ref bleed"; trust.js:1097-1144 88-floor only bites when field conf<88, but dropping the cap restores native (high) conf → 88-floor + flagged + docTrustGate all pass. Residual safety = each arc's own precondition (exact-literal / non-credible-rigid / >=2-family), not a shared gate. P0-2 correctly scoped (client perMachine:false = sharper injection; unpacked .node outside asar integrity). P0-3 confirmed (HARDEN_JS unset by default). P1-1 LOW caveat correct (apiClient.js:62 pinned CA; forge = cert-gen only). P2-1 premise CORRECTED IN AUDIT'S FAVOUR: customer default is genuinely 300 (tesseract.py:45) → 300 is un-corpus-validated + OOM-risky, 200 aligns default<->calibration(reslice.py:288)<->validated point; under-tiered as "efficiency". DPI change fails safe (ocrCache.js:87 dpi-changed → full re-read); learned formats value-level/DPI-agnostic.
SEAM (cross-lane, unowned): (a) CENSUS CONTAMINATION — grep-gate stops future tainted builds but the owner's reference DB has 108…136 already applied+stamped, no down-migration + deleteAppDataOnUninstall:false → ON forever → every future OFF==ON / M=0 census reads a DB already ON = vacuous (the "!= '0' reads EMPTY as ON" / "rr_ids harness vacuous" trap class). So the down-migration is CO-EQUAL with the gate, not "Secondary". (b) COMBINATION-NOT-VALIDATED — {12 OFF, DPI 200, parallel ON} on a COLD DB never run as one config; seed-OFF baseline + parallel byte-identical, probably valid, but must be PINNED.
CORRECTIONS: (1) elevate P0-1 down-migration Secondary→REQUIRED; (2) re-tier P2-1 up (validated-config+OOM); (3) add realdoc M=0 gate to P2-3 (read-path change, audit names none); (4) P0-5 + P1-8 gate ONLY the LAN add-on → deferrable core-desktop-only.
TOP-3 BLOCKERS: (1) P0-1 gate + reference-DB reconciliation; (2) P0-3 HARDEN_JS release build; (3) P2-1 DPI200 + P2-4 RAM. P0-2 signing = business-gated on incorporation, unsigned NSIS is accepted interim → #4.
GATE: cold-DB corpus run at {DPI 200, parallel ON, 12 revert-list switches OFF} → M=0 + zero per-field accuracy delta vs 200-DPI serial baseline; + prebuild test that FAILS on an un-sentinelled TEST-BUILD force-ON marker (would fail today: 12 markers, no gate).

## 2026-09-08 — AUDIT FIX PLAN vet (gary P0-1 + eric P0-3/P1 + oscar P2 bundle consolidated; docs/designs/AUDIT_FIX_PLAN_2026-09-08.md) — SIGN OFF WITH CONDITIONS C1-C10
2026-09-08 — AUDIT_FIX_PLAN vet — SIGN OFF WITH CONDITIONS C1-C10: 13 blocks/26 keys confirmed (mig 106 was missing from every revert list); exclude ocr_parallel_import_enabled from TEST_SWITCH_KEYS (1.4 disarm would un-promote mig 139); interim arming must write the marker; gate belt must be KEY-based not SQL-shape; hardened boot smoke BEFORE the build flip; cold gate must spawn with buildWorkerCommand env + a warm 200-vs-300 arm; 1.3 metric = wouldFile(137)⊆wouldFile(136) with value-diffs 0; F8 is a hypothesis, drop header-CSP on value not mechanism. Per item: 1 SIGN OFF W/COND · 2 SEND BACK → C4 · 3 SIGN OFF W/COND → C5 · 4 SIGN OFF W/COND · 5 SEND BACK → C6/C7 · 6 reorder · 7 F8 over-claimed. Conditions folded into the plan §9.

## 2026-09-08 — C7 RULING on the warm arm (mig 138 ocr_dpi=200; 300→200 on the post-137 copy, 147 docs) — SIGN OFF WITH CONDITIONS C7a-C7e
2026-09-08 C7 ruling — SIGN OFF W/COND: mig 138 ships 200 for fresh installs only (guarded seed: templates=0 AND confirmed=0, else an explicit '300' row); the warm arm measured a DPI FLIP under 300-taught geometry, not 200-vs-300 (the 7 "heals" and 8 of 15 holds are the same clip class re-rolled + 5 Ironbridge type-refuse holds + 1 silent one-line-low relocate #117 = anchor.py relocate-seat/fixed-px-pad layer, post-launch DARK arc — NOT value_quality, which has no address veto); comparer compared arrays not values (fix, C7c); M=4 poisoned GT must be repaired (C7d, owner); A↔B strict is OMP=1-only (the product exports ≥2 threads on 16+-core boxes — pre-existing LSTM non-reproducibility, log not claim), A↔C per lane per rendition with A's M=0 absolute (C7 cold reading); Settings DPI helper text warns of the re-read wave (C7b).

## 2026-09-08 — KEYWORD-SUPERSTRING NAME GROW (owner ask: "extend the mapped read until it corroborates the keyword"; 007 geometry + gary engineering → Oracle) — SEND BACK, redirected (docs/designs/KEYWORD_SUPERSTRING_GROW_2026-09-08.md)
2026-09-08 keyword-superstring name grow — SEND BACK: F2 falsified (`abs_expanded` needs an EMPTY tight read), Fix 0 "heals ungrown" via R8's hidden hpad, gary's pass is inert on the exhibit (same failing crop read) and a third owner; ship the mapper leg's entry census + in-band grown re-read + name defer_cap, a zero-OCR keyword-superstring NOTE (corrected_to, after Stage 4), v2 = ledger witness through `_corrobLicensedKeyword`. F3 = two clipped customer names AUTO-FILED today on the owner's fresh install (docs 19/21; `customer_name` is non-role, only a note blocks) — fix-now class.


## 2026-09-08 — PRE-DEPLOYMENT RE-AUDIT vet (oscar/eric/gary/backend re-run vs the fix build; docs/PRE_DEPLOYMENT_AUDIT_2026-09-08_RERUN.md) — SIGN OFF WITH CORRECTIONS
2026-09-08 re-audit vet — SIGN OFF W/CORRECTIONS: every FIXED-VERIFIED traced (harden-js keeps src/windows + package.json external; verifier html belt + REFUSED rename; composeEnv scrub + applyFlags; backup exclusion on export AND restore; mig 137/138/139/140 + arming inside open()→runMigrations; 7 belts; smoke userData before the lock) — all hold; renderer-plaintext is the plan's scope, not a regression; no uncovered re-arm road. THE MISSED SEAM: the smoke's identity assert was SKIPPED when the identity line was absent (lock-lost quit / temp-dir fall-through both exit 0 with no line) and had never run on a real artifact — made REQUIRED the same evening (file channel smoke-identity.json in a verifier-owned userData + stdout fallback; exit 4 on a userData failure; RED-first pins). Both hardened installers in dist/ (2017-dd9a790, 1355-5b6c226) carried the P0 yet five docs called them "current/verified" — renamed .REFUSED + pointers fixed before the owner's click-through. "Whole class" over-claimed (static tags only; safe because the tree is no longer deleted). The BUILD_REV claim FALSIFIED at the source (composeEnv strips -TEST; pinned). Ranked: RR_ALLOW_ARMED + RR_APP_ENV default = before the NEXT flip census, not the build; name-band pin + tautological pins before the mig-140 flip; register hand-run pins now; owner = signing/DB-at-rest/DocumentRoot+2FA/Learning Repair/Python re-provision with hashes.

## 2026-09-09 — CONFUSABLE-GLYPH RE-READ WITNESS → suppress Gate-C "absent" + allow auto-file (owner ask "remove the message and autofile"; docs/designs/CONFUSABLE_REREAD_ARC_2026-09-09.md) — SEND BACK
2026-09-09 confusable-reread arc — SEND BACK (ship the note half review-bound; DO NOT ship the auto-file-via-reread). Premise REJECTED: a K-recipe re-read of the SAME 200-DPI bitmap is NOT an independent witness for an O/0-class glyph — the committed `template_mapping` read is `_ocr_crop_laddered` (anchor.py:3518, rungs light/heavy×PSM7/6, 2× upscale+autocontrast+sharpen), and 2 of the 3 proposed witness recipes OVERLAP those rungs = circularity; the thing that ACTUALLY resolved O/0 in the multi-res experiment was the SCALE axis (fresh 300/400/600 renders) + human read, which the engine has no plumbing for (HANDOVER_2026-09-04: higher-DPI re-render needs process_docs render-plumbing, unbuilt). M is not the knob; independence-of-AXIS is. The real crop(O)-vs-page(0) difference = tight-crop framing vs whole-page PSM-3 line context (segmentation axis) → a tight re-read shares the committed read's bias and reproduces O INCLUDING the mirror case (true value P0-, biased crop reads O, K correlated same-pixel votes → silent wrong auto-file into a wrong folder/filename = the M=1 class). O/0 is the worst pilot pair (roundest, least discriminable) and doc #45 is 83<90 so the auto-file half is INERT on the very exhibit — risk lives only on graduated sibling scopes. VERIFIED: note→autofile mechanism (trust.js:922,1156 non-empty validation_note→flagged→eligible:false; clearing _absent removes exactly that); both existing softeners review-bound; Gate-C LIVE in owner DB (FILING_VALUE_SANITY_FLAGS=true) → phantom card is a real customer defect worth fixing. CONDITIONS — C1 SHIP NOW: note-only, review-bound — in the Gate-C else arm (engine.py:7355) when _near is a one-glyph _CONFUSE_TO_DIGIT digit↔letter confusable (exclude the ca.upper()==cb.upper() case-fold branch), emit the existing _FILING_SANITY_SOFTEN_NOTE WITHOUT requiring confirmed history (auto-file byte-identical, blast radius = note text; new DARK flag seed OFF in TEST_SWITCH_KEYS). C2 auto-file lever must come from a DIFFERENT AXIS than the committed read: (a) confirmed supplier PREFIX history — the already-Oracle-signed _prefix_dominant_backed (engine.py:2125) + _prefix_confusable_adopt (:2138); PO refs unique but the "PO" prefix is not, so this needs NO OCR — WRONG LAYER: wire auto-file into (a), don't invent a re-read; or (b) a genuine higher-DPI render from source (needs the unbuilt plumbing; only helps if source carries >200 DPI real info). C3 any re-read witness may only be a NOTE softener, never an auto-file lever, until a decorrelation argument stronger than "different threshold/PSM" is shown by census. C4 hard build-order: may not build/arm until oscar fix #1 (per-call timeout+SetErrorMode) AND #2 (crop-conditioning) land; witness abort/garbage→abstain (keep note), pinned; honour the pad-window flag-producer seam. Confusable trust rank for any future auto-file use: I/1,|/1,]/1,[/1 > S/5,B/8,Z/2 > O/0 (never OCR-only). GATE: census over 605+Copperfield — count Gate-C confusable-absent notes, note-fix must show 0 wouldFile change; for the auto-file lever RESTRICT to graduated scopes where clearing would file + hand-check every flip where the page-form could be true (M=0 := zero wrong-value flips + zero per-field accuracy drop); adversarial mirror set must be CONSTRUCTED (page-form digit correct, crop biased to letter) — gate FAILS if arc files any. Pins: mirror / circularity(same-bitmap O/0 doesn't count toward auto-file) / abstain-on-abort / note-stays-review-bound. SIDE LEAD: doc #45 ocr_recipe bd:true,bd_used:false — born-digital DETECTED but text layer NOT used; if used, exact text read, no O/0 at all (separate thread, check born_digital.py).

## 2026-09-09 — CONFUSABLE-PREFIX AUTO-FILE (the auto-file half redirected here by the 2026-09-09 re-read SEND BACK; docs/designs/CONFUSABLE_PREFIX_AUTOFILE_2026-09-09.md) — SIGN OFF WITH CONDITIONS C1-C7
2026-09-09 confusable-prefix auto-file — SIGN OFF WITH CONDITIONS (GO build DARK; do NOT flip until C1/C2/C7). The design does exactly what Condition 2a required: the auto-file lever is confirmed PREFIX history (`_prefix_dominant_backed` ≥5/≥5/≥0.90 + `_prefix_confusable_adopt`), an axis independent of the crop read, NO OCR (so NOT tesseract-dependent), inert until a scope earns ≥5 confirmed refs. Premises verified: suffix-safe (rv.startswith(dom) + adopt==rv forces the one diff inside [0,L), a suffix confusable → adopt subs==0 → None → witness False); NOT a duplicate of `_try_prefix_confusable_adopt` (on the exhibit rv already IS dom → that lane no-ops, Gate-C still flags the whole-page); note→autofile at trust.js:922 (softOptionalNonblock exception is role-gated, doesn't apply to a ref role); admitted class = `_CONFUSE_TO_DIGIT` ∩ `_PREFIX_CONFUSE_CLASSES` (narrower gate wins). Q RULINGS: Q1 dominance bar alone NOT enough (a genuine P0-form second convention is INVISIBLE to the prefix index — code_prefix truncates a digit-leading prefix — so ≥0.90 doesn't bound it; sufficient ONLY with C2); Q2 ADMIT O/0 for the prefix region (the re-read "O/0 last" ranking does NOT transfer — history axis, and a real P0 barely registers as a prefix, making O/0 among the SAFEST here); Q3 crop-vs-history match is ALREADY required by rv.startswith(dom) → a mismatch already forces review → convention-consistency is the correct arbiter for the prefix region. TWO CATCHES (binding): C1 SEAM — `_pageFamilyDisagrees` (trust.js:591, `trust_role_disagreement_refuse` mig 93 ON) can independently HOLD the doc even after the note is removed if a STRUCTURED page-family candidate read the confusable form (`_corrob_values_agree`/`_cmp_norm` does NOT fold O↔0), so the note-free claim is not the whole story — MEASURE the decomposition (arc-inert-held-by-disagree vs arc-live) and pin that a confusable case WITH a structured disagreeing keyword candidate stays HELD; do NOT fold O/0 into `_corrob_values_agree` to "fix" it (WRONG-LAYER, reopens the mirror everywhere — rejected in advance). C2 MISSING MIRROR GUARD — add the both-forms refusal the sibling lane carries (engine.py:4681-4691), but over the PAGE-FORM head `near[:len(dom)]` with COUNTER==0 (stricter than both_forms_established's ≥3/≥10%) via `confirmed_counts_index`, because a genuine P0- series is invisible to the dominance bar; without it a scope with 5 PO + 3 real P0 auto-misfiles a real P0 as PO (M=1). Residual accepted: a first-ever P0 in a counter==0 PO-only scope is admissible for the PREFIX region (a letter-then-digit class prefix is essentially never a real convention; the adversarial mirror set is the gate). CONDITIONS: C1 measure+pin the disagree seam; C2 the counter==0 page-form mirror guard; C3 fail-safe try/except→abstain→soft note, every failed clause → soft note, pinned; C4 DARK seed OFF in TEST_SWITCH_KEYS, HARD dep filing_sanity_confusable_soften ON, no tesseract dep, not in any customer-default mig until C1/C2/C7; C5 the 6 spec pins + the C1 held-by-disagree pin + the C2 second-convention pin + a `_CONFUSE_TO_DIGIT ⊆ _PREFIX_CONFUSE_CLASSES` coverage pin; C6 NON-VACUOUS census (live install inert at 2 confirms → run the 605 corpus + a CONSTRUCTED ≥5-confirm graduated scope, report the C1 decomposition); C7 M=0 (zero wrong-value auto-files + zero per-field accuracy drop) + the CONSTRUCTED adversarial mirror set (a genuine P0- doc in a ≥90% PO scope must NOT auto-file), hand-check every census bucket-(ii) doc where the page-form could be true, realdoc M=0. SIDE LEAD (separate thread): doc #45 ocr_recipe bd:true,bd_used:false — born-digital detected, text layer not used; if used, exact text read, no O/0 at all — higher-value than either half for born-digital PDFs; check born_digital.py.

## 2026-09-09 NIGHT — IMPORT-AFTER-TEACH STREAMLINE (barry experience + gary mechanism → Oracle; owner authorised gated implementation) — SIGN OFF WITH CONDITIONS
2026-09-09 import-after-teach streamline — SIGN OFF WITH CONDITIONS. PREMISE (verified on the install DB copy): the propagation the owner wants ALREADY EXISTS + is ON — the quiet re-read lane re-reads a taught sender's queued siblings, and at auto_file_threshold=90 auto-files the clean ones; barry's "quiet_reread_on_layout is DARK" was STALE 2026-08-26 comments predating mig 93's ALL_ON default. So "no automatic re-read" = a RELIABILITY+VISIBILITY failure of an ON mechanism. PRIMARY ROOT CAUSE = the un-timed tesseract fan-out wedge (a libstdc++ abort hangs the child on the WER modal → pytesseract's default timeout=0 waits forever → shard never returns → siblings never merge → the per-file watchdog os._exit ORPHANS the still-blocked child, process_docs.py:307-324). SLICE 0 (tesseract timeout + SetErrorMode) = GO tonight, S0-1..S0-4: fail-toward-review (abort→per-field miss via the crash-path except at anchor.py:3285/template_mapper.py:2273,2425/tesseract.py:679-704), byte-identical on a completed read. Bonus: pytesseract kills the child BEFORE raising (:151) so Slice 0 also largely closes the ORPHAN class — oscar's Job Object #5 becomes defense-in-depth, not primary. Correction: the wedge is bounded by the per-file daemon watchdog's os._exit (which IS the orphan mechanism), not only the 30-min JS watchdog. NO seam for Slice 0 with the flag-producer arcs (oscar's crop-conditioning #2 is the one with the seam — a crashing read made to SUCCEED; Slice 0 makes it MISS — do NOT fold #2 in tonight). Conditions: S0-1 every timeout'd call has an enclosing except returning the empty/None (guard the unwrapped: region_core.py:167, ocr_region.py:43/45, ocr_image); S0-2 crop timeouts GENEROUS (≥ full-page value — SetErrorMode is the speed mechanism, the timeout is only the floor; document so a future dev can't tighten it); S0-3 gate = realdoc M=0 + byte-identical across 605 serial + Hard Set 0/0 + a monkeypatch pin (image_to_data raises TimeoutExpired/TesseractError → each guarded site returns empty, never raises) + a source-shape pin (every production image_to_* call carries timeout=); S0-4 owner-gated — crash-FIX efficacy provable ONLY by the -TEST-vs-release CONCURRENT repro, S0-3 green ≠ "crash fixed". SLICE 1 (one-click foreground re-read) — design SOUND but has a GENUINE SEAM it uniquely introduces: reprocess-batch hardcodes via:'manual' (release-if-reliable), which DROPS the quiet lane's unconditional first-fill hold (quietLane.js:456-458, the Copperfield-447 guard) — a single clean layout-population read would be released→eligible→offered. Build DARK tonight ONLY if S1-1..S1-4 clean, else LOG: S1-1 teach_reread_now_enabled seed OFF in TEST_SWITCH_KEYS, shard=1 with _reprocessThreadCap UNCHANGED (byte-identical verified — the cap is from configured concurrency not live shard count, handler.js:2346/3190/4720); S1-2 population = quietLane._candidates exact mirror + identity-neutral staging (no known_supplier/pin injection → a wrong-supplier sibling is never force-overwritten); S1-3 carry per-doc via, apply the via-SPECIFIC first-fill hold (layout/ready first-fills get the UNCONDITIONAL hold, NOT collapsed to via:'manual'); S1-4 files ONLY via the consent-offer bar, NEVER scheduleScopeAutoAccept/onJobDone (with threshold 90 + scope_sweep_auto_accept that would file without a click). DO-LESS ruling: Slice 0 + surface the lane's existing unshown events (job_start/doc_done/job_deferred) as a visible "N re-reading" signal is MOST of the win; Slice 1 only additionally helps continuous active use — real but not urgent. IMPLEMENTED tonight: Slice 0 (SetErrorMode + generous timeout on the crash-path calls, guarded; pin test_ocr_timeout_backstop.py 12/12). Slice 1 LOGGED for the owner (the S1-3 via-seam not rushed). Peripheral OCR sites (heading_reread/landmarks/orientation) timeout deferred — not the crash path, covered by SetErrorMode + their guards.

## 2026-09-10 — TAUGHT-BOX EDGE-CLIP HEAL (007 root cause + fix design; owner north-star "reliable autofiles via clean slices"; docs/designs/EDGE_CLIP_HEAL_2026-09-10.md) — SIGN OFF WITH CONDITIONS C1-C7
2026-09-10 edge-clip heal — SIGN OFF WITH CONDITIONS (GO build DARK; flip owner-gated). Traced end-to-end: proposal unbuilt (no `template_edge_clip_heal`/`_edgeclipheal` in python_backend), the pad readers + `_snap_union_witness` exist as described, insert point template_mapper.py:2864/2882 accurate, `_read_pad_window_name` genuinely new, `_PAD_CODE_PROVISIONAL_CAP=87 < CRITICAL_FIELD_FLOOR=88`. PREMISE HOLDS with two corrections: (1) NEW class, not a `_padunclip` variant — `_maybe_pad_code` heals a DROPPED glyph (suffix-containment, length change) and is two-sided-consent-gated (2573-2576: both `YN-#####` and `DN-#####` shape-confirmed → consent_tight="confirmed" → swap BLOCKED by design), while the exhibit is a same-length SUBSTITUTION at the cut edge → the gap is real; (2) "cap<88 ⇒ never cold-auto-files" is TRUE only for REF/DATE (critKeys=[ref,date] at trust.js:1176; supplier_name NOT in it, and docTrustGate is bypassed at overall==100, 1226-1234) → a healed ISSUER at 87 is NOT blocked (C6). All three "why every defense misses" claims verified true at the mechanism (edge-cut overhang<~8px→(None,None); widen bails on shape-valid; pad margin+15 defeated by high-conf clip). FORK: READ-window is the right layer (compose can't kill the 3-9px residual; the belt recovers no values); KEEP BOTH `_edgeclipheal` + the M=2 overall==100 belt (complementary). The certification is largely CIRCULAR except the PLACEMENT geometry (un-cut-edge anchor + occupancy) — that IS the legitimate independent signal (located-at-taught-slot), proves PLACEMENT not READING; reading-correctness rests on clip-containment (only the cut-edge glyph differs) — sound for a bisected glyph, shaky for a genuinely-ambiguous pair, so the ADOPT class is narrow. CONDITIONS (binding before flip): C1 NOTE-FIRST — never lift a value already carrying validation_note/`_edge_suspect`/raw-witness/`_edge_healed` (mirror the 2535/2881 guards; preserves the name defer-cap belt doc 67); C2 ONE decision point / ONE pad read (replace the `_PAD_DISAGREE_MARGIN` gate for the certified-clip class only, else fall through; no double-mutation, no triple-OCR); C3 DO NOT add a third date mutator — scope `_edgeclipheal` to CODE-substitution + NON-issuer-NAME completion, dates stay on the mig-143 `_pad_date_witness` route; C4 convert pad-window crop-pixel word boxes to the page-normalized frame of target_box before `_snap_union_witness` (pin a crop-offset mis-conversion fixture); C5 an adopted `_edgeclipheal` ref whose 88-floor relaxes must require `_corrobLicensedKeyword` (page-text, genuinely independent) NOT the box-crop common-mode `_corrobLicensed` — scoped to the method family via `extraction_method`, do NOT widen the 08-15 relax globally (the dominant-shape leg does NOT distinguish `DN-38627` vs `DN-38626`); C6 ISSUER (`supplier_name`) → FLAG tier only, never silent adopt, while the overall==100 docTrustGate bypass stands (or land the M=2 Complementary-B "capped field drags overall<100" first); C7 keep the M=2 belt (run role-disagreement-refuse at overall==100 — the lowest-blast-radius safety of the lot; land it regardless of this arc). VERIFICATION GATE: unit pins (i certified L-cut sub→adopt@87 method `_edgeclipheal` doc140; ii certified R-cut name completion→adopt doc139; iii garbled-both-sides→FLAG cap≤70+note+corrected_to; iv not-certified wrong-row/two-equidistant/mid-glyph/LR-both-cut→byte-identical; v note-first NEVER lifted; vi C4 frame fixture; vii ref adopt held@87 unless `_corrobLicensedKeyword`) each proven to FAIL with the guard removed; realdoc OFF==ON outside the fire set + M=0 (zero new silent wrong commits AND zero new wrong auto-files) + wouldAutoFile(ON)⊆wouldAutoFile(OFF)∪{keyword-corroborated-to-truth} + zero per-field accuracy drop; adversarial fire census (log every adopt (field,tight,recovered,edges,occupancy,corrob families), hand-verify recovered==page truth, ZERO corroboration-lifted on box-crop-only families under C5); throughput budget on a teach-heavy template. Exclude currency (totals `_skip_rigid` regime). Scope: code/ref (adopt) + non-issuer name (adopt) + issuer (flag) + garbled (flag); date excluded (C3).

## 2026-09-10 — UNIVERSAL CORROBORATION-ADOPT over a taught box (owner "2 reads agree → autofile"; Thornbury delivery_docket_10 exhibit; gary+007 → docs/designs/TAUGHT_CORROB_ADOPT_2026-09-10.md) — SIGN OFF WITH CONDITIONS C1-C4 + gate G1-G3
2026-09-10 template_taught_corrob_adopt — SIGN OFF WITH CONDITIONS (build DARK as designed; flip gated). PREMISE HOLDS (traced): no arc adopts an on-page corroborated alternative over a FLAGGED Stage-0.5 taught winner — `_crosscheck_corroborated_alternative` fires only for winner.method=="anchor_crop_crosscheck" (engine.py:2357); `_universal_postmerge_verify` double-excludes via `_override_eligible`==False for any Stage-0.5 method (7760, `_is_stage05_located` 1033) AND the validation_note guard (7768); the emit same-family-skip (5294) genuinely drops the deposed taught value (tight + pad-widened crop both `mapping`). NOT wrong-layer; the arc fills a real gap (the pendingfeatures.md ANCHOR-WINS-OVER-CORROB class). SEAM (mig-152 M=2 belt) SOUND + honestly named: today delivery_number's record is winner=IN-64470(mapping), disagree=[{keyword,DN-64472}] → `_pageFamilyDisagrees` fires; after adopt winner=DN-64472(mapping), agree=[keyword], disagree=[] → gate correctly silent — it RESOLVES the disagreement (the only cross-family voice genuinely agrees), does not launder it; keeping `_corrobadopt` in the mapping family so the deposed value is honestly same-family (not a fake cross-family disagree) is the CORRECT independence accounting. BUT be clear-eyed: 007's "mig-152 belt is the backstop" is FALSE for the ADOPTED value — the arc removes that backstop for the field it rewrites, so the whole safety case collapses to one question: can keyword + pad-crop both be wrong-for-this-field on the same value? GUARDRAIL closes common-mode (two taught-box crops → fams={mapping} size 1, no keyword → rejected) and closes the digit-confusable garble (verified `digit_substitution_diff("IN-64470","DN-64472")`==-1 — leading I/D is a non-digit-position mismatch so `_uv_restore_demotion` does NOT block, arc is non-vacuous on its own exhibit; a pure ≤2-digit slip IN-64470↔IN-64478 →1 → demoted → no adopt). But the GUARDRAIL does NOT close CROSS-FIELD NEIGHBOUR BLEED: the full-page keyword regex has ZERO positional binding, so if the pad over-reads a spatially-adjacent same-shape code X and keyword independently latches X, slot[X]={mapping,keyword} crop+page-present, demotion None → a genuine two-family agreement on a WRONG-FOR-THIS-FIELD value passes. The crop-leg-is-taught-region argument bounds the pad side only. 87-vs-90 discrepancy RECONCILED by staging: the 88 critical floor blocks c<88 and is UNCHANGED by band-88, so at 87 the field cannot auto-file (review-bound by construction); at 90 it clears + auto-file reachable. CONDITIONS: C1 (FLIP-blocking) positional guard — the keyword witness must locate within/near the taught target box OR share its row-band (ledger `box` engine.py:3711) before adopt; if no reliable keyword box, the flip can't proceed on census alone. C2 STAGE — Phase 1 adopt review-bound (cap ≤88, softened note "corrected from the taught box — please confirm", no auto-file, note KEPT so a bleed still surfaces for review), Phase 2 (@90 + band-88 auto-file) only after C1 + fire census. C3 fail-safe pin: no `_pad_witness` OR no keyword slot ⇒ byte-identical (flag+note intact, held). C4 pin Part A `_pad_witness` reaches Part B AND is inert without `template_pad_window_code`. GATE: G1 605-corpus OFF-vs-ON M=0 (RR_APP_ENV=1, OCR_RENDER_DPI=200; proves NO REGRESSION — corpus rarely fires Stage-0.5, not efficacy); G2 direct integration fixture on a taught clipping template — (a) DN-64472 adopts, agree:[keyword] disagree:[] both `_corrobLicensed`+`_corrobLicensedKeyword` True; (b) THE ADVERSARIAL CENSUS — a same-shape-neighbour layout where pad over-reads the neighbour + keyword grabs it MUST NOT adopt (C1's positional guard exercised; a green fixture lacking this case is worse than none); (c) a pure-digit-slip alt demoted, no adopt; (d) C3 fail-safe byte-identical; G3 pins — the digit_substitution -1 on the I/D case (pin the exhibit verdict either way so a future loosening OR tightening can't silently start blocking it), the same-family skip of the deposed value, mig-152 silent-after-adopt-but-firing-on-a-bleed. Auto-file today = ZERO (triple-DARK conjunction mig153+mig145+corrob master, all seed-OFF) → the build signs off; risk lives at the flip. `_docFullyCorroborated` bounds harm (all of supplier/ref/date must license) but the value REWRITE+note-drop degrades REVIEW presentation on a bleed even without auto-file → Phase-1 MUST keep the softened note, never a clean commit. Route the neighbour-bleed positional-guard detail through 007/reggie before the flip. SEPARATE (S1 sweep): the residual "File up to N" consent offer for VIEWED eligible docs — SUPPRESS THE CLICK, KEEP THE SIGNAL: replace the offer with the in-view countdown (`sweep_inview_countdown` mig 103) — visible, cancellable/pausable, and NEVER fires while a field editor is dirty; do NOT let a viewed doc auto-file with zero signal the instant it's eligible (the presence guard exists for the doc-changing-under-the-operator's-eyes case, incl. losing an in-progress edit). Correctness unchanged (same isAutoFileEligible as the silent non-viewed siblings) — purely a consent/UX decision.

## 2026-09-10 — REF-ROLE SHAPE VERIFY (ref role verified by learned shape, not 'constant' set-membership; reggie+gary → Oracle; docs/designs/REF_ROLE_SHAPE_2026-09-10.md) — SIGN OFF WITH CONDITIONS
2026-09-10 trust_ref_role_shape (mig 154) — SIGN OFF WITH CONDITIONS (build DARK as designed; flip owner-gated). Right layer confirmed — scope to `ref_field_key`; do NOT touch `classifyLearnedShape` (a classifier-level fix would widen the date role + company key = WRONG LAYER). Premise traced at the source: `classifyLearnedShape` short-circuits `distinct<=2 → 'constant'` (trust.js:274-275) before the every()-tests; the sub-100 role branch (1042-1044) reads `_effectiveClass(f,true)` which only rescues 'freetext' → 'constant' set-membership refuses a genuinely-new `INV-45152` (`unverifiable-value:invoice_number`); the at100 arm already EXCLUDES 'constant' (1005) so a ref at 100% already files — bug is graduated-floor(95)/sub-100 ONLY; every built-in/preset ref field is type:'text' (document_types.js) → systemic. Correction 1 (label): the LIVE sub-100 backstop is `trust_role_disagreement_refuse` (mig 93, ON — trust.js:956-958), NOT mig 152 (the at100 belt, DARK). Correction 2 (safety understated): a scope with a genuinely mixed/unstructured ref history now returns 'freetext' → even a REPEAT value routes to Review (was: filed under 'constant' membership) — strictly fail-safer, pinned as intended. SEAMS HOLD: company/identity key keeps 'constant' set-membership (override keys on e.field_key===ref_field_key; supplier_name is COMPANY_KEYS, never the ref key — Ironbridge N2 preserved, pinned); date role returns at the STRICT 'date' arm (982) before the role branch, override never keys on date_field_key (the 09-09 Copperfield silent-wrong-DATE M class NOT widened, pinned); empty-ref caught by the gate-unify missing-required refusal. RESIDUAL (parity, not new): a wrong-but-code-shaped ref surviving the note check + 88 floor + a COMMON-MODE misread (no page-family disagreement — `_pageFamilyDisagrees` is only a partial net) can file silently — identical to what every 'digits' ref role + the at100 path already accept; the finer per-scope prefix/digit-run skeleton (reggie) narrows it below parity → DEFERRED behind its own gate, not a ship-blocker. Sibling site trust.js:1301-1302 (crit-floor <88 corrob relax still uses fmt.cls) — DEFER (fail-safe when entered, inert on Thornbury @98) + comment + pin so nobody folds it into the corrob-relax/_edgeclipheal/_corrobadopt keyword-witness seam without its own vet. PLUMBING (binding): switch hoisted ONCE in docTrustGate (925) AND threaded through autoFileEligibleIds (1317-1322) like roleDominant — else the renderer "will auto-file" badge disagrees with what the backend does, or an N×row perf regression. teachFollowup over-promise = SEPARATE Slice B, switch-coupled; do NOT ship the "will auto-file" copy ahead of the switch ON. CONDITIONS: (1) mig 154 seed 'false', in TEST_SWITCH_KEYS, no numbered force-ON twin, OFF byte-identical (all pinned + GREEN); (2) classifyRefShape contract pin — structured only on unanimous every()-test, else 'freetext', never 'constant' (GREEN); (3) seam pins — new supplier_name blocks, date role invalid-date blocks, 'Information' garble blocks (GREEN); (4) sibling 1301 pin + comment (comment DONE; the corrob-relax path stays fmt.cls); (5) corpus realdoc_regression OFF-vs-ON M=0 + zero per-field accuracy drop AND inspect every NEW would-file's ref == confirmed GT (count-only green forbidden) — owner-owed; (6) live-DB re-judge of the 20 held Thornbury docs → all flip, ref==read, scan whole install for any new would-file ref != correct read — owner-owed (the real efficacy proof; the 605-corpus likely can't reach the 2-distinct-duplicate case). Auto-file today = ZERO (DARK, seed OFF). Note: this same fix files the WS-68687 batch-leftover exhibit at 97% and lets the armed sweep_inview_recheck (mig 150) light the countdown on open — no Reprocess needed.

## 2026-09-10 — ANCHOR_AXIS_LOCK (additive width-invariant label-column read for below/right free-text; 007+reggie+gary → Oracle; docs/designs/ANCHOR_AXIS_LOCK_2026-09-10.md) — SIGN OFF WITH CONDITIONS C1-C6
2026-09-10 anchor_axis_lock (mig 155) — SIGN OFF WITH CONDITIONS (build DARK as designed; flip owner-gated, likely stays DARK per C6). RIGHT LAYER (not WRONG LAYER): width-invariant left-edge reconstruction off the LOCATED label is the genuine fix for the centre-pin class (the free-text twin of mig-141); a display overlay is cosmetic-on-the-sample. Root cause verified: the anchor stores the value box CENTRE, offset_dx_norm = value-centre − label-left ≈ ½·taught-width, so a wider teach sample pins the read-box off to the side on a narrower doc (Vellum & Crane customer_name drifting right of "Deliver To"). Owner constraint honoured — ADDITIVE candidate, every existing rung byte-identical. Premise traced: `_override_eligible` (engine.py:3775, takes a dict) excludes authoritative/`_is_stage05_located`/keyword_override → the exhibit's authoritative anchor_crop is popped+discarded = INERT (heals the PASSIVE/empty clip, not the authoritative one — that's mig-151 territory); no callable merge helper (Tier-A is inline engine.py:9758-9818) so the transient-stash + post-merge reconciler channel is the right pattern; `direction NOT NULL DEFAULT 'right'` → axis dispatch always defined, no migration; `_label_left_limit` (anchor.py:4238) is gated to authoritative+right+structured so the arc REIMPLEMENTS the formula for free-text below (C5); `cluster_value_words` med_h*1.2 split is the SOLE precision defence (validation can't catch a column-merge). THE SHIP-BLOCKER (C1): on the target class (customer_name, optional non-role) the ≤87 cap is INERT (88 floor is ref/date only; overall scored from required fields) → the note is the sole checkpoint, and optional_soft_flag_autofile (mig 142) dissolves exactly that soft-note class → both-ON = a possibly-wrong axis-lock read auto-files silently. A pin can't fix it (independent flips leave the seam live) and the corpus M=0 gate runs mig-142 OFF so it is STRUCTURALLY BLIND to the seam — the fix must be CODE and the both-ON pins are the only gate that reproduces it. SHIPPED: trust.js `isAxisLockNoteRow` keys on the `anchor_axis_locked` extraction_method (a STRUCTURED sentinel, not the copy — a copy edit must not re-arm the seam) → the note is NEVER soft-cleared at BOTH `_flaggedSoftAware` and docTrustGate; pinned in test_scope_trust.js §26 (control soft-clears; axis-lock blocks under softOptionalNonblock, both-ON). Other conditions built: C2 `_override_eligible`-gated replace / empty-fill (pinned exhibit-inert + keyword/Stage-0.5 discarded); C3 kept OUT of `_field_candidates` (not-in-ledger pin) + reconciler in the post-merge tail after all `_remember_candidates`; C4 emit-gate = strict superset/longer OR empty-fill (a shorter multi-line recipient read → no emit, pinned) so it never offers a truncated value; C5 reimplemented the width-invariant formula for free-text below/above (pins exercise the new path). REVIEW-ASSIST not auto-file heal: always-note + ≤87 (≤69 on a longer disagreeing read). HONEST LIMITATIONS (bound efficacy, no regression): INERT on the motivating exhibit (authoritative wins); ANCHOR-FAMILY ONLY (the candidate rides the anchor result dict → dropped if a keyword incumbent won — safe/narrower than the full "beat a weak keyword" design). C6 WORTH-IT GATE: the arc is inert on the exhibit and may be VACUOUS on arm137/605 (the read-widen trap) → run a fire-census on a REAL docket/free-text corpus BEFORE flip; ZERO real fires ⇒ STAY DARK (do-nothing-in-practice, not worth the merge-tail blast radius); include a constructed mis-resolved-supplier / abutting-column (gap ≤ med_h*1.2) adversarial. VERIFICATION GATE: pins GREEN (test_anchor_axis_lock.py 26, test_scope_trust.js §25+§26, test_migration155; OFF byte-identical on test_anchor_name_lock_guard.py + test_anchor_drift_guard.py); corpus realdoc_regression RR_APP_ENV=1 OCR_RENDER_DPI=200 OFF-vs-ON G1 byte-identical-outside-class + M=0 (wouldAutoFile ON⊆OFF + ON-only==GT) + placement accuracy |box_centre−glyph_centre| up + zero regression on money/inline/totals-reflow/registration — owner-owed. Auto-file today = ZERO (DARK, seed OFF).

## 2026-09-10 — NAME_ROLE_NONNAME_FLAG (deterministic non-name guard for name-role fields; reggie+gary → Oracle; docs/designs/NAME_ROLE_NONNAME_FLAG_2026-09-10.md) — SIGN OFF WITH CONDITIONS C1-C4
2026-09-10 name_role_nonname_flag (mig 156) — SIGN OFF WITH CONDITIONS (build DARK; flip owner-gated). Defect verified live: Vellum & Crane sales orders auto-filed `customer_name = CH1 2HU` (a UK postcode) @92-94% no flag — a Stage-0.5 template_mapping zone drifted onto the postcode line of the multi-line "Customer" block; customer_name is optional (COMPANY_KEYS=['supplier_name'] since mig 44) so it never gates; required fields read clean → overall 100 → gate-free auto-file → wrong optional value rides silently. Premise traced: a Stage-0.5 read is authoritative/label-confirmed → exempt from every name-quality gate; the structured-type flag loops SKIP name fields; the wordness gate self-disables (word_like-gated, not-authoritative-gated, soft, caps-not-blocks) → a postcode-as-whole-value in a name field was checked by NOTHING. Posture B (flag+hold, keep the value) not A (reject-to-empty still auto-files + destroys the evidence). RULINGS: (1) LAYER is NOT WRONG — this deterministic subset does NOT re-trip the Larkspur C7 ruling (which sent the WORD-LIKE wrong-line placement fix to anchor.py's below-relocate seat); this is the proper-subset deterministic belt (whole-value postcode/email/vat/iban), a Stage-4.5 flag in the content-nature family (date-in-ref/prefix-outlier class), no address-line veto attempted — log that the word-like line stays parked. (2) The `flagged` check in isAutoFileEligible (trust.js:1218-1250, via _flaggedSoftAware :1240) is the load-bearing gate for this exhibit — at overall==100 docTrustGate is never called; the note reaches _flaggedSoftAware. (3) The ≤69 cap does NOT hold an optional field (overall scored from required only) — the NOTE is the sole hold; pinned. CONDITIONS built: C1 pattern set = postcode_uk + email + vat_gb + iban via re.fullmatch on value.strip() (DROP reggie's bare-number/currency leg — a bare number is a legitimate recurring name-like value + the currency patterns are un-anchored → batch-stall; exclude reference_code/alphanumeric/percentage/date). C2 (STRONGEST) the accepted_names / dominant-confirmed exemption + curated-method exemption (template_fixed/override/fixed/manual/+confirmed_adopt/+name_snap) so a supplier's legitimate recurring value (Ship-To depot postcode, contact email) never stalls its batch — do NOT exempt template_mapping/anchor_* (that non-exemption IS the fix). C3 append `+nonname_flag` to the `method` key (never replace, never a new key → the template_mapping prefix survives for _method_family prefix-match) + isNonNameFlagRow UNCONDITIONALLY at both trust.js:576 and :1006 (the mig-142 forward-defense; inert OFF because no row carries the sentinel → byte-identical). C4 pin the hold to the note not the cap. Both name keys (a postcode as supplier_name misfiles the FOLDER — higher stakes; its note already blocks as a role key, the sentinel matters for the optional customer_name). VERIFICATION GATE: predicate pins (positives postcode/email/vat/iban; negatives Studio54/Plan9/3M/B&Q/A1 Storage Ltd/Meadowbrook Vets/"Beaumont Care CH1 2HU"; excluded loose shapes) + test_scope_trust.js §27 (both-ON mig-142 held; control soft-clears) + test_migration156 + LIVE CONFIRMATION on the actual CH1 2HU doc (OFF @94 no note auto-files; ON value kept, +nonname_flag, conf 69, note, held) — ALL GREEN; realdoc M=0 + accepted_names batch-stall check owner-owed. Efficacy hypothesis: the 605-corpus likely can't reach the live template drift → M=0 proves SAFETY not efficacy; the live-doc confirmation carries efficacy. Auto-file today = ZERO (DARK, seed OFF). This is the SAFETY BELT; the placement root cause (zone drifts to the postcode line) is the separate parked Larkspur below-relocate arc.

## 2026-09-10 — DRIFT-OVERRIDE MATCH GUARD (a Stage-0.5 drift-relocate may only discard the absolute read on a credible label match; 007 → Oracle; docs/designs/DRIFT_OVERRIDE_GUARD_2026-09-10.md) — SIGN OFF WITH CONDITIONS C1-C3
2026-09-10 template_drift_override_guard (mig 157) — SIGN OFF WITH CONDITIONS (build DARK; flip owner-gated). RIGHT LAYER (gating the drift-OVERRIDE on match quality); raising the global _FUZZY_MATCH_THRESHOLD would be WRONG LAYER (rejected). Root cause (007, reproduced live): template_mapper._extract_one's drift guard (:2841) DISCARDS the credible absolute read and relocates when _label_drifted; on Vellum & Crane #243 the crop OCR failed to segment "Customer", so _locate_anchor fell to the address word "Chester" (match_score 0.667 vs "customer", above _FUZZY_MATCH_THRESHOLD=0.6) ~2.5 lines below the taught anchor → phantom drift → relocate onto the postcode line → customer_name="CH1 2HU"; the absolute read of the taught box was "Larch & Hollow Cafe Co" @95, thrown away. Intermittent = the label-word OCR segmentation lottery (#245/#249 read "Customer" cleanly @1.0 → absolute read stands). Premise VERIFIED at source: the branch returns _relocate_and_read discarding abs_text (:2841-2850); _locate_anchor returns match_score (:4212); _label_is_the_taught_one (:1651) — ("Customer","Chester") False so it falls to the score floor; the 0.667/0.824 gap computed against _label_score; byte-identical OFF. THE SEAM (Oracle #1 job — 007's naive version has a real bug): 007's version, on the weak-match case, falls to `elif drift_located: anchor_stable=True` which KILLS the registration arbiter (an INDEPENDENT global page-transform drift signal that does NOT depend on the label OCR lottery, its contract "fires when the anchor is not a usable local signal" — a sub-floor fuzzy match IS exactly that). Counter-case: a GENUINELY drifted scan whose label garbles to 0.6-0.8 while the stationary box covers a wrong neighbour → 007's version auto-files the wrong neighbour @high-conf; C1 (leave anchor_stable=False) lets the arbiter recover it. C1 does NOT weaken the #243 heal (#243 not genuinely drifted → box_divergence≈0 → arbiter won't fire → absolute name stands) — strictly-better-or-neutral. CONDITIONS built: C1 (LOAD-BEARING) the weak-match case falls through with anchor_stable=False (relocate/return inside `if _do_strong:`, the `elif` keeps its anchor_stable=True) — pinned by the P3 SEAM pin (fails on 007's literal, passes on C1). C2 extend _exactness_census to log match_score so the 0.8 floor is PLACED from a census, not a 2-point gap. C3 verification gate: unit pins (P1 heal Chester 0.667 refused/abs stands; P2 garble 0.82/0.90 override taken; P3 seam reg-arbiter-stays-live; P4 OFF byte-identical) + realdoc M=0 on 605 (NO-REGRESSION ONLY — the class is a live-DB doc, not in the corpus) + the LIVE #243 re-run (the ONLY heal proof). Kill switch TEMPLATE_DRIFT_OVERRIDE_GUARD / _DRIFT_OVERRIDE_MATCH_FLOOR=0.8, mig 157, TEST_SWITCH_KEYS→41, DARK. No overlap with mig-155 axis-lock (DOWNSTREAM — seat) or Larkspur below-relocate (Stage-2). LIVE-CONFIRMED: #243 OFF="CH1 2HU"@94 / ON="Larch & Hollow Cafe Co"@94; #245/#249 byte-identical. This is the placement ROOT-CAUSE fix — it makes the mig-156 non-name guard a near-vacuous last-resort backstop (owner: "minimise warnings, maximise correct detections"). Auto-file today = ZERO (DARK, seed OFF).

## 2026-09-11 — BELOW-RELOCATE ARC (value crop lands one line low on a multi-line block; the parked Larkspur residual; 007 design → Oracle; docs/designs/BELOW_RELOCATE_ARC_2026-09-11.md) — **SEND BACK**
2026-09-11 below-relocate arc — SEND BACK (layer stands, re-shape not re-layer). **Added value: YES — two ship-blockers traced that 007 missed.** (1) PREMISE FALSE: the design's load-bearing claim "the fix is robust to either H-a or H-b" is wrong at the source — the chooser's frame origin `cap_bottom = label_box.y_norm + label_box.h_norm` is the IDENTICAL quantity `_place_from_located` uses (anchor.py:1875 ≡ 2496-2497), so under H-a(ii) an inflated `label_box.h_norm` puts `cap_bottom` BELOW line 1 and "topmost credible under cap_bottom" returns line 2 (inherits the mislocate, doesn't heal); under H-b line 1 is sub-credible @200 (already only @89 @300) and no crop geometry conjures an unreadable glyph → the "line 1 at 200 AND 300" pin is unsatisfiable. So H-a/H-b is a PRE-BUILD measurement, not a census line, and it decides whether the row-1 heal can heal at all or only degrade to flag. (2) FAIL-SAFE HOLE (ship-blocker): the design asserts chooser→None ⇒ needs_review/no-auto-file, but `results[field_key]` is built only inside `if value:` (anchor.py:1457, entry 1652) and the guard-note attaches only `elif field_key in results` (1735) — so a None + weak/blank rigid read leaves the OPTIONAL field ABSENT + un-flagged → STILL auto-files silently empty. Trades silent-wrong-value for silent-empty-field (the exact check-5 failure the arc claims to prevent). Also: (3) SEAM — `_is_caption_band_read(q,…,relo,…)` at 1246 would judge the chooser's caption-anchored crop `q` against the OLD ballooned `relo` box = coordinate-frame mismatch; re-parameterise or bypass. (4) BLAST — `anchor_pad_hnorm`'s `max(h_norm,pitch_norm)` collapses to the tiny `h_norm` for structured/single-line/right crops (no pitch computed) → starves the +20px headroom at 300, breaking the byte-identity pin for the heavily-shifted-scan class; scope the pad conversion to free-text below/above ONLY. `_DRIFT_FLOOR`→pitch correctly deferred (different rung, not a precondition). REDIRECT (bring back for a clean sign-off): (a) resolve H-a vs H-b as a PRE-BUILD measurement on the real Larkspur docket @200/@300 (dump label_box, cap_bottom, cy, `_read_lines_full` per-line); (b) make v1 FLAG-FIRST — abstain→review on ≥2 credible lines or a not-credible row-1 (provably correct under both hypotheses; never commits a line); heal-to-row-1 = later separately-gated optimisation for only the measured-healable sub-case; (c) CLOSE THE FAIL-SAFE HOLE (mandatory) — the flag forces needs_review + blocks auto-file even with NO committed value; RED-first pin must FAIL on today's code; (d) scope the pad conversion to free-text below/above; (e) re-parameterise/bypass the downstream caption checks to the chooser's crop; (f) gate = the measurement recorded + realdoc M=0 + census differ = heal-or-flag with zero new wrong commits AND zero new silent-empty auto-files + the empty-field-blocks-auto-file pin + the scoped 300-crop pin + a Larkspur pin asserting the MEASURED mechanism. NOT BUILT. Design doc updated to v2 (flag-first) with the two pre-build blockers flagged.

## 2026-09-11 — REF-ROLE CONFUSABLE FLAG (flag a class-outlier letter/digit confusable on the ref role; Chris Card 1 SO→S0; reggie predicate + gary gate → Oracle; docs/designs/REF_CONFUSABLE_FLAG_2026-09-11.md) — **SIGN OFF WITH CONDITIONS C1-C5**
2026-09-11 ref_confusable_flag (mig 159) — SIGN OFF WITH CONDITIONS (build DARK; flip owner-gated). **Added value: YES — caught a wrong seam rationale + a length-blind disarm.** PREMISE sound + traced: the class is genuinely uncovered (Gate C `engine.py:7441 if _absent:` — the mig-147/148/149 soften/prefix arcs live entirely inside `_absent`; a self-consistent whole-page O→0 misread → `_absent=False` → none run; near_miss/confusion_correct need value_counts, empty for a new supplier; `S0-47966` is a valid reference_code shape → no format anomaly). FLAG-ONLY is the right layer, NOT a re-OCR (same-pixel read-agreement = the rejected weak corroboration; O/0 unknowable from pixels on a non-slashed-zero font). Ref-role-ONLY is correct — do NOT extend to optional strict reference_code in this arc. THE SEAM VERIFIED TRUE: a validation_note on ref_field_key forces `{eligible:false,reason:'flagged'}` in BOTH isAutoFileEligible (trust.js:1250-1261 via _flaggedSoftAware :577-590, roleKeys includes ref_field_key, isSoftAdvisory :540-544 false for a role key) AND docTrustGate (:1011-1020), EVEN with mig-142 ON, with ZERO trust.js change + NO sentinel; the note PERSISTS (handler.js:3421/:6488 write validation_note; the analogous below-relocate empty-field hole is NOT present because this SETS a note on the row); the boost (:11542) skips noted fields so running before it (:11527) can't be re-lifted. Born-digital signal IS readable at the site (`page_provenance` local in extract(), :8517 `=='ocr'`). CONDITIONS: **C1 (SHIP-BLOCKER in the design) — the mig-149 ordering rationale is WRONG:** `_confusable_prefix_backed` (engine.py:4707-4738, call :7503) clears a confusable to auto-file with NO note/corrected_to, so the new flag's "skip if note/corrected_to set" protects NOTHING; the real guards are (a) the mig-149 committed value uses the LETTER prefix `SO-` (dom-is-letters → Rule B needs a digit-for-letter → doesn't trip) and (b) for a genuine-digit-prefix supplier (`S0-` dom) the ATTESTATION DISARM — which as designed (mirror mig-156 `_confusion_from_attested`, format_anomaly_checker.py:807) is LENGTH-BLIND-WRONG (it filters same-total-length; a prefix is length-agnostic → a variable-length genuine-digit-prefix supplier keeps false-flagging). FIX: Rule B's disarm MUST reuse `any_confirmed_shares_head(value_counts, read_head)` (ocr_corrector.py:533, length-agnostic + counter==0-strict — the SAME primitive mig-149's mirror guard uses at :4734) → the two-arc seam closes by construction. Rule A (interior) may use a position-specific attestation, pinned separately. **C2** — the census must add a cell mig-149 ON + ref_confusable_flag ON over a genuine-digit-prefix REPEAT supplier (synthetic fixture if the 605/Demo have none) proving `wouldFile` unchanged — else the census is vacuous on the seam. **C3** — born-digital gating is PER-PAGE (gate on the provenance of the page the ref came from; fallback = do not flag if the ref's page can't be confirmed OCR; never flag a born-digital-read value; do not suppress a pure all-'ocr' scan). **C4 pins** (each fails with the guard removed): the 7 python cases + the JS anti-silent-file pin + (i) the C1 head-based disarm pin with the current ref a DIFFERENT length than the confirmed siblings (the case _confusion_from_attested gets wrong) + (ii) the C2 mig-149-cleared genuine-digit-prefix doc stays auto-file-eligible both arcs ON. **C5 flip gate** — newly-flagged dominated by TRUE confusables adjudicated AT THE PIXELS (corpus GT may rubber-stamp the wrong glyph), near-zero false alarms overall + near-zero on born-digital AND repeat suppliers (the latter proves the disarm releases), realdoc M=0, zero per-field accuracy drop, wouldFile(ON)⊆wouldFile(OFF); repeat-supplier false alarms ⇒ disarm still mis-scoped, back to C1. MINOR: drop "reuses _PREFIX_CONFUSE_CLASSES verbatim/no new map" (there is a tier/case filter + it's cross-module engine.py↔fac, mind the verbatim pin); confirm the mig-158 note-topic signature matches when both flip. Auto-file today = ZERO (DARK, seed OFF). BUILDING DARK with C1-C4 folded in.

## 2026-09-11 — REF ARBITER: page-absent clip — upstream heal (A) + reinstatement fallback (B) (Ridgeway VS-72672 vs page WS-73673; 007+reggie+gary → Oracle; docs/designs/REF_ARBITER_REINSTATE_2026-09-11.md) — **SIGN OFF WITH CONDITIONS on B (build DARK) · SEND BACK A (re-scope + measure)**
2026-09-11 ref-arbiter reinstate — **Added value: YES.** Fork ruling: the RUNTIME composition (A@Stage-0.5, B@GateC) is right but the BUILD ORDER is backwards — B is the safe sufficient-for-Phase-1 fix, ship it first; A is NOT a mig-151 trigger flip. Traced: (merge authority) engine.py:9532-9634 keeps a `_is_stage05_located` winner regardless of confidence (yield legs date-invalid/format-fail/fragment-contain all False for the shape-valid VS-72672), `_override_eligible`:3781 False for stage-05 → a keyword@99 can NEVER displace it → B is the ONLY reselection path, must override authority → review-bound. (Gate C) :7424/:7478/:7590 absent branch is flag-only, never reselects; `_field_candidates` retains the keyword read. (Fail-safe) reference_number ∈ roleKeys (trust.js:579), isSoftAdvisory False for a role key (:541) → the reinstate note ALWAYS blocks + mig-142 can't soft-clear → `wouldFile(ON)==wouldFile(OFF)` is true SET-EQUALITY. **SEND BACK A (ship-blocker):** the exhibit (VS72672 vs WS73673, 3 scattered diffs) DEFEATS `_clip_contained` (template_mapper.py:2544, `_EDGE_CLIP_MIN_INTACT=3` needs ≥3 intact un-cut-side glyphs + ≤2 differing edge glyphs; here 0 intact BOTH edges) → arming mig-151 does NOT heal it; a real A must adopt on `_snap_union_witness`+whole-token-on-page+shape-valid while BYPASSING `_clip_contained` = a materially bigger change to the every-doc Stage-0.5 crop path than "widen the trigger". A2 gate the A build on 007's 3 measurements as PRECONDITIONS (box-left-x vs W-ink-left-x = crop-bounds clip vs source-pixel severance; a wider row-bounded psm6 re-read recovers WS cleanly incl. digits; `fields.type` for reference_number ∈ {alphanumeric,reference_code} else the pad-code family `_CODE_CROSSCHECK_TYPES` template_mapper.py:135 never runs → A dead, B immune). If (1)=source-pixel or (3)=text → DO-NOTHING on A, B-only. **CONDITIONS on B (build DARK):** B1 use the STRICTER whole-token page test symmetric with Gate C (engine.py:7491-7492 `rv.casefold() not in toks`), NOT the looser separator-tolerant `_page_presence_corroborated`:2280 (correct the design's "same whole-token test" wording); B2 primary neighbour-bleed guard = dominant-prefix + `shape_match_score==1.0` + exactly-one-qualifying + not-another-field's-committed-value (edit-distance ≤3 is tuned-to-sample, SECONDARY only); ship the adversarial same-shape-neighbour fixture (a different field's on-page code, matching prefix+shape → must return None), each guard proven to fail with it removed, #1 the regression PIN; B3 NO corrected_to / NO corrections-learning row (a corrected_to re-arms renderer.js:4339-4347 affordances), method → the reinstated candidate's method, cap ≤69, JS pin (reinstated ref role note not auto-file-eligible + not soft-cleared by mig-142); **B4 the NEW mig-158 seam** — B's soften note (rank-1 REF_ADVISORY) merged vs a rank-2 lane-hold under note_topic_dedup-ON drops B's note TEXT but the doc stays held (hold keys on note PRESENCE + value equality, handler.js:1663; value set by Python not the merge) — PIN it + B byte-identical with mig-158 OFF, with filing_value_sanity_flags OFF, and on a miss; B5 mig 160 seed 'false', TEST_SWITCH_KEYS→44, SFDEV toggle, HARD dep filing_value_sanity_flags ON. GATE: realdoc 605+Demo OFF-vs-ON (RR_APP_ENV=1 OCR_RENDER_DPI=200 FILING_VALUE_SANITY_FLAGS=1) → M=0 AND wouldFile(ON)==wouldFile(OFF) set-equality, adjudicated AT THE PIXELS (corpus GT may rubber-stamp the off-page garble → a GT "regression" is likely the pipeline being RIGHT), ref accuracy rises. Other seams: B↔P-lane (engine.py:4335, Stage 2.5 before Gate C) no conflict; A↔B runtime compose without double-fire. Auto-file today = ZERO (both DARK). BUILDING B DARK with B1-B5; A PARKED pending measurements.

## 2026-09-11 — ARC A RE-SCOPE (Stage-0.5 page-absent code-clip: placement-certified wider re-read; 007+gary → Oracle; docs/designs/CODE_LEFT_GROW_2026-09-11.md) — **SIGN OFF WITH CONDITIONS (build Phase 1 only, DARK; census-gate the flip)**
2026-09-11 template_code_left_grow (mig 161) — the re-scope of the Arc A previously SENT BACK; the demanded measurement is DONE and favours A (240-DPI + product-200-DPI wide read = WS-73673 clean; trim left 14% → /S-73673; full-page reads WS- on all 20 → crop-BOUNDS clip, not source-pixel). **Added value: YES — caught a circular-witness copy-paste trap + a filing-visible representation gap.** FORK: A is NOT over-reach vs B-only, NOT wrong-layer — B needs an independent-family ledger candidate; on a docket where ONLY the template box read the ref, B has nothing to reinstate → A fixes the read at source (Stage 0.5). Traced: `_clip_contained` (template_mapper.py:2544, `_EDGE_CLIP_MIN_INTACT=3`) correctly refuses the 3-scattered-diff garble → the bypass IS required; composes with Gate C/Arc B (Gate C runs only `_absent AND not note` at engine.py:7622 → A sets value-on-page + a note → Gate C inert → B never fires); ref role runs the code family (coerced alphanumeric, engine.py:3029). The neighbour-swallow geometry is SELF-EXCLUDING for an 'L'-clip (left-overhang + right-edge-anchored within [tx2−0.25tw, tx2+g] + row-aligned + ≥0.6 occupancy = only the token the box is clipping) — SOUND replacement for glyph-continuity, PROVIDED C1+C2. CONDITIONS: **C1 (crux copy-paste trap)** — `_snap_union_witness` MUST be fed the INDEPENDENT full-page locate-tier words (`ocr_lines_fn`/`line_cache`), NOT the `[{"words":[pad_box]}]` self-witness the mig-151 path uses at :2680 (else the certifier reconstructs the token from itself = no independent guard); pin it. **C2 (load-bearing, filing-visible)** — BOTH the winner's page-ABSENCE and the recovered value's page-PRESENCE must use Gate C's EXACT tokeniser + RAW hyphenated surface (`rv.casefold() in {t.strip(...).casefold() for t in re.split(\s+,page)}`), NOT `_code_norm` (which strips separators → would adopt `WS73673` for a page `WS-73673`, wrong filename, and Gate C is skipped for a noted field so nothing catches it); adopt the exact printed surface or decline; pin. **C3** read-widen disjointness both-ON (`not read_widened` AND independent shape==1.0). **C4** Phase-1 pin: note + cap ≤87 + method `+leftgrow` → not auto-file-eligible + not soft-cleared (role-field note); Python regression PIN worksheet_07 VS-72672→WS-73673 ON / byte-identical OFF, each guard fail-tested. **C5** Phase 2 (auto-file) HOLD — A stays REVIEW-BOUND permanently unless Phase 2 adds an INDEPENDENT family (keyword/anchor) reading the same value (the mig-143 triangulation), NOT same-pixel geometry alone; own arc + census if pursued. **C6 (build precondition)** re-confirm recovery at product 200 DPI (DONE — WIDE@200 = WS-73673 clean) + max clip < 0.8·bw. **C7** adversarial fixtures (two-code same-row → edges≠L/abstain; same-right-edge different-row → row-band excludes; mis-resolved-supplier shape → bounded by C2+right-anchor), each fail-tested. GATE: 605+Demo OFF-vs-ON RR_APP_ENV=1 OCR_RENDER_DPI=200, fire census pixel-adjudicated, M=0, Phase-1 wouldFile(ON)⊆wouldFile(OFF), zero per-field drop; ZERO fires on 605+Demo ⇒ STAYS DARK (needs a real docket corpus). Switch template_code_left_grow / TEMPLATE_CODE_LEFT_GROW, mig 161, TEST_SWITCH_KEYS→45. Auto-file today = ZERO (DARK). Building Phase 1 with C1-C4+C7.

## 2026-09-12 — DESKEW RETRY FIELD ADOPT (role-note door + field-scoped corroborated adopt when the whole-doc gate refuses; the Ridgeway #358 skew exhibit; gary → Oracle; docs/designs/DESKEW_RETRY_FIELD_ADOPT_2026-09-12.md) — **SIGN OFF WITH CONDITIONS C1-C11 (build DARK, mig 162; flip owner-gated)**
Fork rulings: field-scoped adopt is the RIGHT LAYER (not a truthful-flag fix alone, not a whole-doc comparison loosening, not do-nothing); the S1 half-door is a deliberate population freeze, not incoherent; S2 is REAL and pre-existing; S3's direction is probably wrong but the outcome is still held; S5 is half-true — the note is the SOLE checkpoint. **Added value: YES — caught an undeclared DARK hard-dependency that makes the arc vacuous on its own exhibit, a door that pays a full straighten pass for docs it can never touch, and a "cosmetic" claim that the arc itself falsifies.** PREMISE TRACED + HOLDS: `_needs_review = validator.needs_review(...) or format_anomaly_flagged or _cp_fired or _rc_fired` (engine.py:11856); `validator.needs_review` (validator.py:960-981) reads required-empty + confidence<threshold only, never `validation_note`; Gate C's absent writer (:7726-7732) calls `_note(...)` and sets no flag; the late recompute (:12427-12431) can only CLEAR (the one late SETTER, `_apply_labelless_withheld_notes` :12436, sets it for its own note only) → a role-note-only doc reports `_needs_review=False` → `_deskew_retry_should_run` (process_docs.py:48-51, call :1290-1293) never opens (H3). Status is always `"needs_review"` from Python (:1435) and the handler reads `msg.needs_review` ONLY at the unify-OFF bail (:6684), so the flag's lie never files a doc — it only blinds the retry door. H2 confirmed: `_deskew_retry_adopt` (:70-76) is whole-doc strict-greater; the rescue class is capped (`_TCA_PHASE1_CAP=87`, engine.py:669) while the raw garble sits @95 shape-valid. `_corrob` is built at :12384 AFTER the TCA adopt at :12382 → the mirrored record describes the adopted winner (mapping+keyword, `disagree=[]`) — F3 via `_corrob_licensed_keyword` (:151-169 → engine `_corrob_licensed` :1871-1894) genuinely requires no-disagree + a keyword page-text witness = a different Tesseract invocation (full-page pass vs box crop) on the same straightened raster; same-raster common-mode is the accepted 08-30 standard, backed by F4's shape leg + review-bound. THE FAIL-SAFE VERIFIED TRUE: `isSoftAdvisory` (trust.js:540-541) is False for every role key; `_flaggedSoftAware` (:577-590) builds `COMPANY_KEYS ∪ ref ∪ date` — the same set; `isAutoFileEligible` refuses `flagged` (:1250-1261) mig 142 ON or OFF. NO machine road dissolves the adopted note: the TCA note ("…Please confirm.", engine.py:670) is NOT in class F (`_verification_doubt_note_marks` :1915-1928), NOT in `CLEARABLE_NOTE_MARKS` (classFixService.js:60-65 — that list DOES hold the Gate C absent mark, but only on the human class-fix road, which writes `APPLIED_HOLD_NOTE`), NOT a lane-hold, rank-0 for `composeNote` (never de-duped); `reread_hold_corrob_release` (rereadHolds.js:113-131, DARK) strips only the S3-C5 sentence and only over an UNLICENSED old record — the adopted row's record IS licensed, so it can never release across it. **THE SEAM (three catches):** (1) `template_taught_corrob_adopt` (mig 153) is in `dark_switches.js:136` — on a customer default the straightened pass reads `MS.72672` + `_padcodeflag` → Gate C absent → F4 refuses → the arc is VACUOUS on 358; the measurement mirrored the dev-armed env, so the design's S10 omits its own HARD dep (mig 153, or mig 161 `+leftgrow`, + `template_pad_window_code`) — flip order 153 before/with 162, both gate cells required. (2) S5 is half-true: the arc MIRRORS a LICENSED (mapping+keyword) record onto the very field it capped @82, and `critfield_corrob_floor_relax` (trust.js:602-610, rides the owner-ON master) relaxes the 88 floor for exactly a licensed+shape-matched record — so the 88 floor is NOT a second checkpoint for this arc's outputs; the NOTE is the sole one (fine while it stands; pin it as such, and stop the emitted overall carrying the deposed @95). (3) The door opens on ANY role note incl. `supplier_name`, but the adopt excludes it (S9) and the whole-doc leg stays on the raw flag — a supplier-only-noted doc (name-wordness/nonname/identity-conflict notes are common) pays a FULL straightened re-render + re-OCR of every page for zero possible upside; same for a ref note on an F5-excluded method or with `corrected_to`. S1 RULING: freezing the whole-doc population is right — opening it to note-only docs would also widen the dev-armed `DESKEW_CORROB_AUTOFILE` population (:215-217), the one road that skips a hold. Note the field path ALSO runs on the EXISTING door's population (`_needs_review` True + whole-doc refused) — a behaviour change on docs that today "kept raw"; the census must partition fires by door. S2 REAL + pre-existing: `_DESKEW_CHANGED_NOTE` on an optional non-strict first-fill is text-blind soft to `isSoftAdvisory` → mig 142 ON dissolves a straightening-changed value — a mig-142 design violation ("a soft note never reflects a changed value"); not widened by this arc (role-only writer). S3 DIRECTION LIKELY WRONG: the IMAGELESS PRESERVE (handler.js:1687-1694) keeps a stored taught/image-family read when the text-only fresh value differs → Quick most likely KEEPS `WS-73673`, not regresses; either way held — pin the truth, don't assert. S7 verified (:6684). S9 verified (`_supplier_name` popped from the RAW results at :1341). ANOMALIES: (a) a straightened field carrying only a class-F note (`_EDGE_CUT_NOTE`, `_SHAPE_TRIM_NOTE`, `_REREAD_NOTE_HEAD` — the edge-clip rescue class IS class F) copies a machine-clearable hold; (b) the straightened pass can resolve a different supplier/template (deskewed letterhead heals identity) → a value from another scope's shape/corroboration spliced into the raw doc; (c) a DATE adopt over a Gate B "year isn't printed anywhere on this page" raw note offers `Use "old"` — the ref-only ABSENT rule has no date twin; (d) the "kept raw (not higher)" log prints for note-only docs where the whole-doc leg was never eligible. BLAST RADIUS LOW: pure process_docs helpers, switch-gated, no engine/trust change; cost = OCR time per admitted doc, bounded by C1. LAYER: truthful-flag (any-note ⇒ True) is the eventual honest fix but touches every doc's emit + the unify-OFF bail and does NOT fix H2; whole-doc loosening re-opens the doc-561 class; Arc B is history-dependent (5 human confirms + an on-page keyword candidate) while deskew REMOVES the tilt (007's creed) and the parent is ON live — this closes a hole in a shipped mechanism. Reward is n=1 and unproven → "zero fires ⇒ DARK" is the right stance. **CONDITIONS:** **C1** the note-held door keys on ref/date role fields that already pass F1+F5 (noted, non-authoritative method, no `corrected_to`/`was_corrected`) — never `supplier_name`-only; pin: supplier-only-noted clean doc → door closed; ref note with `corrected_to` → door closed. **C2** declare mig 153 (or 161) + `template_pad_window_code` ON as the exhibit-class HARD dep in S10 + `dark_switches.js`; pin: `TEMPLATE_TAUGHT_CORROB_ADOPT` unset → the 358 fixture (straightened `MS.72672` + absent) refuses. **C3** on any field adopt set `_overall_confidence = min(_oc0, _oc1)` (never above either pass); pin 358 → 79. **C4** stamp/append `_DESKEW_CHANGED_NOTE` when the adopted dict's note is empty OR machine-clearable (`_is_verification_doubt_note` or a `CLEARABLE_NOTE_MARKS` twin) so the surviving hold is always lane-hold or deny-by-default; pin with `_SHAPE_TRIM_NOTE`. **C5** refuse the field adopt when the straightened pass's `_supplier_name` (text_normalise-equal) or `_template_id` differs from the raw pass; pin. **C6** no `corrected_to` when the RAW date note is Gate B's page-absence claim (hoist a `_FILING_SANITY_YEAR_ABSENT_MARK` constant), symmetric with the ref ABSENT rule; pin. **C7** JS both-ON pin: adopted ref-role row (TCA note, licensed mapping+keyword record, @82, shape-ok) with `optional_soft_flag_autofile` + `corroboration_autofile` + `critfield_corrob_floor_relax` ON on a graduated scope → `isAutoFileEligible` = `flagged`; note stripped → eligible (pins the note as the SOLE checkpoint). **C8** pin the actual Quick-reprocess behaviour on an adopted `template_mapping_corrobadopt` row (IMAGELESS PRESERVE keeps the stored read; non-taught key → CONTESTED-held), replacing S3's assertion. **C9** the else-branch log must say "whole-doc adopt not applicable (note-only hold)" for a raw-flag-False doc, and the trace event carries `door` (`flag`|`note`), was/now, fams, conf. **C10** record S2 as a NAMED flip precondition on `optional_soft_flag_autofile` (`_flaggedSoftAware` must treat any `_isLaneHoldNote` row as never-soft) in `pendingfeatures.md` + the mig-142 `dark_switches.js` comment. **C11 GATE** the 605 corpus is REQUIRED (`RR_DB`; the Nordwind skew class lives there), Demo 369 additionally; cells {162 OFF/ON} × {153 OFF/ON} + the dev-armed `DESKEW_CORROB_AUTOFILE=1` cell; realdoc M=0 AND wouldFile(ON)==wouldFile(OFF) set-equality in EVERY cell; fires partitioned by door (note-only vs flag) and role, adjudicated AT THE PIXELS (GT may rubber-stamp the garble); door census = added passes + OCR seconds with supplier-only passes = 0; render-determinism ×3 on 358 as a BUILD precondition; live #358 armed FULL reprocess → `WS-73673` @82 held, confirm writes no correction row; zero field-adopt fires on 605+Demo ⇒ STAYS DARK. RELIES ON (upstream): `_corrob_licensed_keyword` semantics, `_shape_ok` emit (engine.py:12464-12482), `_corrob` built after TCA (:12382-12384), `filing_value_sanity_flags` ON (F4 non-vacuous), mig 153/161 ON (exhibit class), `deskew_review_retry_enabled` ON (parent). DISABLES (downstream): nothing — every output is role-noted, `DESKEW_CORROB_AUTOFILE` is never consulted, the whole-doc population is byte-identical. Auto-file today = ZERO (DARK, seed OFF).

## 2026-09-12 — QUICK FILE (non-OCR intake + management; barry + eric → plan; docs/designs/QUICKFILE_AND_DEPARTMENTS_PLAN_2026-09-12.md §3) — **SIGN OFF WITH CONDITIONS Q-C1…Q-C12 (plan; nothing built)**
The shape — an ordinary `status='confirmed'` documents row with `extraction_method='typed'` rows, `intake='direct'`, never Review, never `reviewService.confirm` — is the right shape and the cheapest one. NOT yet safe as written: the learning invariant rests on two claims and only one is true. (a) "The ONE helper `learningExcludedSql` reaches every learning reader" — TRUE for the READ side (the 17 pinned sites `test_learning_excluded_readers.js:52-71` + `get-auto-file-reason` `:76-83` + `processing/handler.js:4902`; `templateMerge.planBackfill` carrying it keeps a typed Invoice PDF off a supplier's template). (b) "The lane never calls `reviewService.confirm`, and all write-side learning lives there" — TRUE of the lane, FALSE as an invariant: four existing doors take a confirmed row INTO confirm/Review without the lane's consent and none knows `intake`: `documents.deconfirmDocument` (`documents.js:378-384` — the choke point for the chip's Put back, Search/Learning Repair send-back, the class-fix undo; called `settings/handler.js:564`, `processing/handler.js:4979`); the Learning Repair console lists typed rows (`getConfirmedDocsForScope`/`getConfirmedDocsByIds` are NEGATIVE-pinned to never carry the predicate, `test_learning_excluded_readers.js:115-118`); "Edit in Review" re-files through `confirm(allowRefile)` (`reviewService.js:188`, `batchAuditService.js:212`) then runs `saveCorrections`/`saveSupplierIdentifiers` gated only on `!_via` (`:433-447`), `persistConfirmedValues` (`:500-505`), the template/graduation hooks (`:595-623`); `reprocess-document` (`processing/handler.js:3704`) and `requeueConfirmedDocsForScope` (`documents.js:361-369`) would push a `.docx` at Python (silently skipped, `tesseract.py:1178-1180`) or OCR a Quick-Filed PDF and confirm it as a human read. (c) The realdoc harness selects GT by raw `status='confirmed'` SQL (`realdoc_regression.js:177-178,188`) — a Quick-Filed PDF with typed supplier/date/ref becomes a GT row the OCR pipeline is scored against → manufactured M. (d) `{title}` IS in `DEFAULT_PATTERN` (`filename_pattern.js:20`). (e) Stamp (`stampService.js:81 NOT_PDF`) and print (`print/handler.js:220`) already refuse non-PDF. (f) eric's DTO path is `src/services/dto.js`. **THE SEAM:** read-side helper and write-side doors are two predicate FAMILIES; the plan ships one. Exact failure: admin opens Learning Repair for (Acme, Contract), clicks send-to-Review → `deconfirmDocument` → Review renders `[]` pages → Confirm → hints/corrections/confirmed-value rows for a document nobody ever read — and the "never learns" metric still reads ZERO because it counts docs, not rows. Second: `documents.working_path` — the lane must NULL it after filing + unlink the inbox copy exactly as `reviewService.js:507-511`, else `resolveFilePath` prefers `working_path` (`documents.js:804`) and `reconcileHolding` deletes a confirmed doc's inbox copy at next start (`:6252-6257`) → every Quick File preview breaks after the first restart. Third: `previewService` returns the WHOLE file as base64 for any non-`.pdf` (`:155-160`, `:209-221`) — the `isRenderable` guard must sit BEFORE `readFileSync` (a 50 MB `.xlsx` = 67 MB over IPC per click) and leave `.tif/.bmp` on today's branch. ANOMALIES: (1) `getFieldValueSuggestions` carries the exclusion clause (`documents.js:160`) — a Party autocomplete on it never lists a Quick-File-only company; (2) the dashboard "filed automatically" DENOMINATOR (`search/handler.js:60`) counts typed docs; (3) a `'none'` preset via `addPresetTypes` ships `title_aliases` + label overrides (`document_types.js:804,818-822`) → a scanned page headed "AGREEMENT" TYPES as a form-only type via OCR (the Generic design's seam 4); (4) `--quickfile` argv lands in the running instance's session (`main.js:1064`) — refuse with no session/role; (5) `.doc/.xls/.ppt` CAN carry VBA — the honest control is "never executes, hands to the OS like Explorer" + MOTW survival pinned; (6) `renameSupplier` renames typed rows too — desired, say so. OCR/office reality: pypdfium2 + stdlib ZIP/XML licence-clean; `extract_msg` GPL correctly refused; cap PAGES (first 50) as well as chars, off the main thread. CX: fail-toward-review is INVERTED for a typed row — the safe state is REFUSAL with a sentence ("filed without reading — edit it from Quick File"), never a queue entry. GATE: `npm run test:pins` incl. the updated exact-text pins (`test_learning_excluded_readers.js:231-233,331`); realdoc M=0 with mig applied + the harness clause; the OFF control byte-identical on filenames/folders; a Chris sandbox pass that TRIES Learning Repair send-back + Edit-in-Review on a Quick File doc. **CONDITIONS: Q-C1** extend `learningExcludedSql` with the column-guarded NON-switchable clause (emitted even under `LEARNING_EXCLUDE_DOCS=0`) + update the three exact-text pins. **Q-C2** a WRITE-side refusal family pinned by a source-contract test: `deconfirmDocument`, `requeueConfirmedDocsForScope`, `reviewService.confirm`, `reprocess-document`, `batchAuditService` correct, Learning Recovery reassign all refuse `intake='direct'`. **Q-C3** the ONLY road to the OCR lane = an explicit audited "Read this document" that sets `intake=NULL` BEFORE reprocess; pin reprocess without it refuses. **Q-C4** `realdoc_regression.js` (+ any GT-selecting script) gains `AND COALESCE(d.intake,'') <> 'direct'`; pin with a fixture DB. **Q-C5** NULL `working_path` after `commitDocument` + unlink the inbox copy; pin preview survives `reconcileHolding`. **Q-C6** `getDocumentPages/getThumbnail` return `[]`/`null` for non-renderable extensions BEFORE any `readFileSync`, `OCR_EXTS` byte-identical; pin both. **Q-C7** Party autocomplete = a dedicated non-learning reader (never `getFieldValueSuggestions`), later D2-filtered; pin it lists a Quick-File-only company. **Q-C8** `'none'` presets ship `title_aliases=NULL`, no label overrides, and `detect_document_type` excludes `'none'` types; pin a page headed "CONTRACT" does not type as the preset. **Q-C9** the auto-filed DENOMINATOR excludes `intake='direct'`; `getFiledCounts` keeps counting them. **Q-C10** `--quickfile` argv + drag-drop staging refuse with no logged-in admin/edit session; every staged path passes `_realCanonical`, is a regular file, not under `userData`/the output root, ext-allowlisted + size-capped before any copy. **Q-C11** pin MOTW propagation through `copyFileSync` (write a `Zone.Identifier` ADS, copy, read back) or drop the Protected-View claim. **Q-C12** drag-drop stays v1.1 with its own Oracle pass; the shared-preload change pinned byte-identical for every window lacking `[data-intake-drop]`.

## 2026-09-12 — DEPARTMENTS (document visibility gating; eric arch §A → plan §4) — **SIGN OFF WITH CONDITIONS D-C1…D-C12 (plan; nothing built)**
Single nullable `documents.department_id`, NULL = shared, tag + per-type default, two lines in `canAccessDocument`, ONE SQL fragment in every list reader, assign-time route gate — this IS the smallest sound model; no lower layer (SQLite has no per-connection principal; a per-viewer VIEW is a per-request temp view with the same forgotten-reader risk). The reader surface is the real risk and the house owns the mitigation — the source-contract pin pattern that caught the `templates.js` drift (`machine_vias.js:2-6`). PREMISE: F1 TRUE — `gateEnabled()` has no `isPackaged` guard (`accessService.js:37-41`), live at `api/handler.js:505,521,551`, `review/handler.js:82-86`, `print/handler.js:205`; ALREADY PARTIAL — `stampService.js:49` + the workflow handler call `canAccessDocument` unwrapped. Severity today LOW on the desktop (the Windows user who can set the env var owns `docusnap.db` + the output tree) and NIL from the LAN client; its real status = a P1 PRECONDITION for D2, one line with an existing pattern (`processing/handler.js:2286-2294`) + pin (`test_access_service.js:117-127` must keep proving OFF works unpackaged). F6 TRUE (`database/index.js:46`) so `ON DELETE RESTRICT` is real. F3 TRUE but INCOMPLETE — misses `documents.getByIds` (activity strip `processing/handler.js:4946`, auto-filed tile `review/handler.js:527`, quick-check grid `batchAuditService.js:130`), `getConfirmedDocsByIds`, Learning History's `getDocumentsForFieldValue` (`review/handler.js:452` — filenames/ids of other departments' docs). "Byte-identical when empty" TRUE for the data-gated parts, FALSE for the 9→1 count-broadcast refactor and the `learningExcludedSql` text change — own pins. **THE SEAM (three missed):** (1) assign-time gate via the FULL `canAccessDocument(recipient)` REGRESSES routing with empty tables — readonly users are routable today (`workflowService.js:17`, `_validateAssignTarget :185-193` checks only active), `ROUTABLE_STATES` includes `needs_review`/`deferred` (`:29`), and the route-party grant is exactly what lets a readonly recipient see an unconfirmed doc → evaluated BEFORE the route exists it returns `readonly_unconfirmed` → `RECIPIENT_NO_ACCESS` on a route that works today. The gate must evaluate the DEPARTMENT decision only. (2) insert-time tagging by DETECTED type + "re-apply only if NULL" files a wrong-department doc silently (a PO mis-detected as Invoice tagged Finance at insert, retyped in Review, stays Finance) — the plan names `department_set_by` but the model has no such column. (3) backup/restore: `document_types` (with `default_department_id`) + settings are in `TABLES` (`backupService.js:34-37`), `documents`/`user_departments` are not — a restored raw id points at a different/absent department → FK abort or a NEW doc tagged to the WRONG department (a leak by restore). ANOMALIES: (a) the IMPORTING user — reception (HR) imports 20 invoices, the type default tags them Finance, the queue fragment hides them ("I imported them and they vanished") while the import log already streamed every value to that window; (b) disabling `departments_enabled` after tagging leaves docs restricted with no UI to see why; (c) system routes pick by role (`amountRouting :158-163`) → `assign-failed` held + audited only (`:177`) — silently never fires for an outsider; (d) the consistency pin as written FAILS today on the bin (`edit` sees `getDeletedQueue` rows but step 4 denies them, `accessService.js:70`); (e) create-time widening undefined; (f) a dangling id if RESTRICT is ever bypassed → invisible to everyone but admin. Office reality: the sidecar sits BESIDE the file — leaks nothing the file doesn't; D5's `{department}` token is the only honest fix, and it changes folders for NEW filings only (re-tagging never moves files — say so). CX: the importer's vanished batch and the disabled-switch ghost restriction read as "broken" — fixed by TELLING, not widening. GATE: D1+D2+assign gate+pins in ONE slice; `test_department_visibility.js` = source-contract positive list (every reader in F3 + the four missed) + negative list + a runtime matrix fixture (2 depts × admin/edit/readonly/all_departments × confirmed/needs_review/deferred/deleted × route/no-route × desktop/`/v1`); `test_v1_*`/`test_search_contract` unchanged with empty tables; `test_access_service` green (OFF control); realdoc M=0 with mig applied and no departments; a count-broadcast pin; Chris as a NON-admin edit user in a second department importing into a type defaulted to the other. **CONDITIONS: D-C1** the assign-time gate calls `departmentTagDecision(db, recipient, doc)` ONLY; pin routing a `needs_review` doc to a readonly FYI recipient still succeeds with empty tables. **D-C2** add `documents.department_set_by ('rule'|'user')`: defaults write `'rule'`, the dropdown writes `'user'`, confirm re-derives from the CONFIRMED type when `'rule'` (never `'user'`), audited; pin the mis-detected-type case. **D-C3** backup exports departments by slug and remaps `default_department_id` + the folder-default settings through it, NULLing on absence; pin a restore never tags a doc to a foreign id. **D-C4** add `getByIds` (three callers), `getConfirmedDocsByIds`, `getDocumentsForFieldValue`, the autocomplete to the fragment list; pin each. **D-C5** restate the consistency pin over NAMED surfaces with the bin's role rule documented, and prove it FAILS when one fragment is deleted (mutation run). **D-C6** `gateEnabled()` honours the env var only when `!app.isPackaged`; ship NOW as its own commit; pin unpackaged OFF works, packaged OFF ignored. **D-C7** the import summary + Review CTA say "N filed to <Department> (not visible to you)" when the importer's fragment hides their own batch; pin the count. **D-C8** turning `departments_enabled` OFF while any doc is tagged is refused with the count + a "clear all tags (admin)" action, or the OFF state shows a persistent banner; pin. **D-C9** create-time rule: an edit user may tag a NEW doc only with a department they belong to (or the type default if a member); NULL/shared at create = admin/`all_departments` only; pin. **D-C10** system-route refusal for an outsider surfaced in the routing-rules dry-run preview; pin `previewRoutes`. **D-C11** the 9→1 `review-count-changed` collapse = its own commit + pin (one emit site; a fixture exercising confirm/defer/restore/delete/send-back/import observes an emit after each). **D-C12** `departments.delete` refuses while any doc references it (application check + RESTRICT); pin the refusal and that a retired department still restricts. **ORDER:** §6 fix (D-C6) NOW as a standalone commit → Q1 → Q2 → D1+D2 (ONE slice incl. the assign gate + D-C2 + both pins) → D3/D4 → Q3 → Q4 → Q5 → D5 → Q7 (after D2, own Oracle pass — the first body-bearing WRITE endpoint on `/v1`). **REFUSE OUTRIGHT:** `.msg` text · `/v1` download · per-department output roots · version diff/restore UI · deriving departments from folders · macro-enabled OOXML · the watch lane before a holding state · the no-sidecar setting (owner Q10 — DO NOTHING) · any auto-close of routes on membership change in v1. **OWNER Q&A:** Q1 copy-and-leave YES, no "remove original" checkbox in v1 · Q2 YES + Q-C8 · Q3 YES + 50 MB as a setting; "never executes, hands to the OS like Explorer" + Q-C11 · Q4 YES, cap pages too · Q5 YES · Q6 YES · Q7 assign-time gate on the DEPARTMENT decision only; `edit` senders refused to outsiders; an ADMIN sender may route to an outsider (audited, open-route visibility only); system routes refused + surfaced · Q8 v1.1 opt-in YES, new filings only · Q9 YES · Q10 DO NOTHING.

## 2026-09-12 — DESKEW FALSE-ABSENT RE-FLAG / hold-RELEASE arc (mig 163; gary design; the 6 Saltmarsh delivery-note class, mirror of #358; docs/designs/DESKEW_FALSE_ABSENT_REFLAG_2026-09-12.md) — **FORK: SEND BACK the release leg / SIGN OFF WITH CONDITIONS the hold leg**
2026-09-12 deskew_false_absent_reflag — the proposal: when the review-bound straighten retry reads the SAME value for a role field Gate C falsely marked page-ABSENT and the straightened frame keyword-corroborates it, DROP the false note → the auto-file predicate decides. **FORK ruling.** (A) **note-DROP / auto-file "release" leg = SEND BACK.** Fails **H1** (no unconditional hold leg shipped/measured first) and carries two unsound seams: **Q6 overall-stomp** — the release's doc-level `_overall_confidence = st` overwrites mig-162's C3 cap when both fire on the same doc (fix C-Q6: `if _fa: skip`); **Q2 cross-raster overall inheritance (the sharp one)** — F8 (st doc-wide clean) says nothing about the RAW fields that persist; a NON-adopted required field (e.g. the DATE role reading a confident garble @95 on the raw raster, clean, no note) passes the per-field 88 floor (guards ref+date on the field's OWN conf) and rides st's inherited ~100 → **silent wrong-date misfile**; C5 guards only supplier. Fix C-Q2: every REQUIRED field not adopted must read the same VALUE raw-vs-st, else refuse (this parity guard makes field-splice ≡ whole-doc-adopt, also killing the doc-561 risk). **Q3 length-blind residual** — a `_fold_shape` length-blind scope + a position-stable mis-seat on a same-shape shorter token near the label passes F2+F3+C5; the 605 fire-census MUST assert every released value's length ∈ the scope's confirmed length distribution + a constructed adversarial that refuses. **H3-H5:** S_AUTO ≥ 10 DISTINCT docs+values across **≥2 suppliers** (the cohort is one), 0-misfile GT census, M=0 + zero per-field accuracy regression. Right layer (process_docs, mirroring mig 162, routing through `isAutoFileEligible`) — not WRONG LAYER, but premature + unsound as drawn. (B) **Phase-1 HOLD leg = SIGN OFF WITH CONDITIONS (build now).** Replace the FALSE absent note with a TRUTHFUL review-bound note when F2(unchanged)+F3(keyword-corroborated)+F4a(no st absent)+C5(same identity) hold — removes NO checkpoint (still held); kills a lie the software tells; on a ref/date ROLE field `isSoftAdvisory` is False so mig 142 cannot dissolve it (closes the mig-162 S2 side-door for free); it is the H1 precondition. **P1** conditions above; **P2** the note is a lane-hold ("— confirm once.") out of every `CLEARABLE_NOTE_MARKS` / class-F set (bilingual pin), the doc stays flagged/held, `wouldFile(ON)==wouldFile(OFF)` set-equality (removes no filer — a delta is a bug). **Q1 verified sound:** `_corrob_values_agree` (engine.py:1134-1149) is exact-normalized (only widening = a date-fold), so `DN-42798` vs `DN-42` disagrees → F3 refuses the truncation garble; a future prefix-tolerant compare would break it (the S2 near-miss, pinned bilingual). **BUILT DARK 2026-09-12** (Phase-1 only): process_docs helper + call site, mig 163, dark_switches (47), `_reconcileEnv` child, pins `python_backend/tests/test_deskew_false_absent_reflag.py` + `database/modules/test_migration163_deskew_false_absent_reflag.js`. Empirical gate (armed, 6 docs, OFF vs ON): RE-FLAGGED on #130/#139/#140, #136/#144/#145 unchanged, wouldFile deltas = 0. Census `TESTING/_measure/deskew_hold_release_20260912/CENSUS.md`. The release leg NOT built (its conditions recorded in dark_switches.js + pendingfeatures.md).

## 2026-09-12 — DESKEW HOLD-RELEASE, the auto-file RELEASE leg v2 (mig 164; gary v2 folding the v1 conditions) — **RE-VET: SEND BACK again; GO/NO-GO = NO-GO as drawn → DO NOTHING until the ≥2-supplier corpus exists**
2026-09-12 deskew_hold_release v2 — v2 closes C-Q3 (an additive engine `_shape_len_ok` emit from confirmed `value_counts` lengths, fail-closed — right layer, refuses the position-stable shorter-token mis-seat), C-Q6 (double-enforced: call-site `if _hr_release_on and not _fa` + helper early-out — pin the call-site guard as primary), and the F2 case-sensitivity near-miss (the comparator at process_docs.py:121/:472 is `" ".join(v.split())`, no `.lower()` → `dn-42798`≠`DN-42798`). overall=st inheritance is SOUND for the SCORE (`validator.overall_confidence` scores `[required] or [all]`, validator.py:864-866; C-Q2 pins those value-identical). **BUT v2 does NOT clear the send-back — two findings + a layer.** **(1) SHIP-BLOCKER — the `_needs_review`/auto-file seam (§3):** the mutation "don't touch `_needs_review`; the unchanged isAutoFileEligible decides" is self-contradictory on a DEFAULT install — `_maybeAutoFile` gates on `_needs_review`/overall BEFORE `isAutoFileEligible` (handler.js:6675 parks overall<min(userThr,95); :6694 parks at 95-100 if needs_review unless gate-unify/soft-arc ON). The Saltmarsh crop @89 drags overall <95 → the release is INERT at default config (never reaches isAutoFileEligible); worse, it drops the truthful note while leaving needs_review=True → a doc held with NO reason (a regression vs Phase-1); it fires ONLY with gate-unify-ON/soft-arc-ON = an unstated cross-arc dependency, so a default-config census measures an inert arc (the green-test-can't-reproduce trap). gary copied mig-162's set/leave-needs_review pattern without accounting for the release's opposite (file, not hold) intent. **(2) C-Q2 not airtight by its own logic (§2):** the default filename pattern is `{docType}.{date}.{ref}.{title}` (filename_pattern.js:20); `{title}` is OCR-derived (filing/handler.js:140) + `required:0` (document_types.js:746) → a filing-deciding NON-required field NOT in C-Q2's required-only set. It misfiles today ONLY by the coincidence that `title` exists solely on General Document, which is unconditionally auto-file-barred (trust.js:1178 generic-type) — a three-module invariant no test enforces. Fix R2 = WHOLE-DOC value parity (every non-`_` field raw-vs-st, case-sensitive F2 comparator) — which also makes field-splice ≡ whole-doc-adopt (my v1 INTENT; I said "required", meant "whole-doc" — corrected). The v1 confident-wrong-DATE class IS closed (date is a required role). **(3) WRONG-LAYER lean (§6):** the release DECISION belongs in trust.js (a graduated, corroboration-licensed scope may file a doc whose only hold is the specific `_DESKEW_VERIFIED_NOTE`), where note+floor+needs_review are coherent and an ungraduated scope falls through to held-WITH-reason — reusing the corroboration-autofile machinery instead of a Python note-drop that erodes the "a validation_note → never auto-file" invariant (trust.js:1014,1150). **Efficacy reality (§5):** the Saltmarsh straightened doc overall stays <95 (crop @89 drags the mean) → parks at 6675 even ignoring needs_review → the arc can't fire on its OWN exhibit at default config; a census MUST assert ≥1 real fire or it is vacuous. **CONDITIONS TO RE-CLEAR:** R1 resolve the needs_review incoherence (no doc held with its note dropped — either clear needs_review=False + fall-through=the reflag truthful hold, or move the decision to trust.js) — pin: a qualifying default-config doc EITHER auto-files OR stays held WITH `_DESKEW_VERIFIED_NOTE`, never held with no note; R2 whole-doc value parity — pin: a non-generic doc with an optional OCR field (e.g. title) differing raw-vs-st REFUSES; R3 census in the FIRING config with ≥1 real fire + wouldFile(ON)⊇wouldFile(OFF) + every added filer's full filed identity (filename+folder) pixel-correct + M=0 + zero per-field accuracy drop + ≥2 suppliers + adversarials {poisoned-short length, shorter-token same-fold mis-seat, case-variant, confident-wrong non-released required}. **RESTING STATE = DO NOTHING** — the release is corpus-blocked (no ≥2-supplier/605 DB on disk), inert at default config, and needs a trust.js re-layer; Phase-1's truthful hold (mig 163, shipped) already kills the lie and is the correct resting state. NOT to be built even DARK until R1+R2 are designed AND R3 can run. Nothing built for the release leg (design only).

## 2026-09-12 — Suspect-code label re-read (the owner's "position shouldn't matter → locate label, read value" arc; 007+reggie converged; docs/designs/SUSPECT_CODE_LABEL_REREAD_2026-09-12.md) — **SEND BACK → DO NOTHING** (both pillars false at source; main verified)
2026-09-12 suspect_code_label_reread — SEND BACK, likely DO NOTHING. The two load-bearing claims are FALSE, verified at source by main: **(1) the safety-by-construction claim is INVERTED** — `_relocate_and_read` (template_mapper.py:2140-2142) returns `_geometric() or _inline()`: for any taught mapping with a stored offset (`dx or dy`) it runs the axis-aligned box re-crop (tilt-UNSAFE) FIRST and returns it whenever it yields a value; `_inline()` (the tilt-robust line-word read) is only the fallback. So the caller cannot force the inline read, and "the shape-exact + page-present gate auto-selects inline / refuses geometric" is backwards → on a LABEL-ABOVE / stacked-code layout (Order No / Delivery No / Invoice No on adjacent rows) the geometric re-crop drifts onto a NEIGHBOURING row whose code folds to the SAME `@@-#` family (shape_match_score==1.0) and IS page-present → the wrong-row neighbour is ADOPTED. "Can only swap shape-invalid for shape-valid+page-present" is true but INSUFFICIENT (a shape-valid page-present WRONG value satisfies it). **(2) the premise is false** — `VS-72672` is SHAPE-VALID: `_fold_shape` (format_anomaly_checker.py:237-280) folds both `WS-73673` and `VS-72672` to `@@-#` (letter→@ prefix-agnostic, single digit-run length-invariant), and the project's own mig-160 verdict says verbatim "the shape-valid VS-72672"; so the trigger `shape_match_score != 1.0` NEVER FIRES on the motivating exhibit. The Ridgeway ref is the shape-VALID + page-ABSENT left-clip class, ALREADY healed by mig-161 `template_code_left_grow` (first-pass) + mig-162 (review-bound straighten). The class the arc DOES target (a shape-INVALID tilt garble on an inline code field mig-161/162 don't catch) has NO exhibit in 605/Demo → by the project's own "zero fires ⇒ stays DARK" rule, unshippable. **Also:** a RELOCATE (grabs a neighbouring row) is a materially larger blast radius than the row-BOUNDED `_widen_code_read`/`_grow_code_left_read` family it claimed to extend, and placing it at :2901 fires it BEFORE the drift guard + mig-157 override + the registration arbiter → could pre-empt the arbiter (the mig-157 C1 counter-case). **Landing: DO NOTHING** — the motivating exhibit is already covered; no distinct measured exhibit exists; the design had a real wrong-row misfile hole. Conditions to ever revive (Oracle): correct the premise, MEASURE a real distinct shape-invalid-inline-code exhibit at 200 DPI (no exhibit ⇒ do nothing), call the inline read DIRECTLY (never adopt a `_geometric` return), site it AFTER the arbiter (fallback-only), census must include the tilted shape-valid wrong-row-neighbour fixture. NOT built (design only). Process note: 007 reasoned tilt-geometry + reggie reasoned the shape predicate abstractly; NEITHER ran the two strings through `_fold_shape` or reconciled against the mig-160 "shape-valid VS-72672" finding — a premise not verified at source before the consult (the CLAUDE.md junctures #5 grep-prior-art-on-the-mechanism + #6 verify-state-at-source).

## 2026-09-13 — Quick File SUBMIT UI: replace the popup MODAL with an in-page PANE (barry product + eric arch consensus; docs/designs/QUICKFILE_SUBMIT_PANE_2026-09-13.md) — **SIGN OFF WITH CONDITIONS** (pre-FLIP, not pre-merge)
2026-09-13 quickfile_submit_pane — vetted the BUILT result (implemented in parallel under the review; quickfile.js deleted, quickfileView.js + view-quickfile router in place). **Premise holds:** pane-over-modal is owner-directed + product-sound (persistent receipt, future drop target, less ceremony); `.view{display:none}`/`.active{display:flex}` makes the pane pattern native; no reason to keep the modal or both (keeping both doubles the double-fire surface). **OFF inert** (Oracle's honest framing — not literally byte-identical: adds an inert `<section>`, a `data-view` attr, a `VIEWS` entry, one unreachable listener; no user-observable change, no new backend call): nav+btn stay `display:none` un-hidden only in the `enabled` branch, `#view-quickfile` never `.active`, `showView('quickfile')` fail-closes on `!QuickFileView.enabled` (enabled set true BEFORE the nav un-hides → no visible-button-reads-disabled window), the one DOMContentLoaded `quickFileDocTypes()` probe is NOT new (the modal did it). **Seam clean:** traced `delete-document` → `documents.softDelete` only (no deconfirm/learning/reprocess) so Undo=soft-delete is genuinely clear of the Q-C2 write-side refusal family; Department UNWIRED is correct (handler never copies `meta.departmentId` → a half-wired field silently DROPS the tag = fail-toward-wrong); autocomplete UNWIRED correct (Q-C7). **Conditions:** **C1 (APPLIED)** the `#qf-dropzone` must not look droppable while preload swallows drops — was `2px dashed` (false affordance for the deferred Q-C12 drag-drop), now a plain click-to-pick card (id kept). **C2 (APPLIED)** receipt "Open folder" de-pathed to `showDocumentInExplorer(docId)` not raw `showInExplorer(storedPath)` (no filed path into the renderer; survives a re-file; pin updated to enshrine it). **C3 (OWED, pre-flip)** manual ON/OFF click-through + a Chris pane pass before flipping `direct_intake_enabled` for shared/customer use. **C4 (standing)** the nicer pane must NOT trigger an early flip — a Quick-Filed doc is still a confirmed row reachable by the four UNBUILT write-side doors (Put-back/Learning-Repair send-back, Edit-in-Review, reprocess, requeue); do not flip before Q-C1/Q-C2. **Verification gate:** `test_quickfile_pane.js` green (15; goes red if quickfile.js or its `<script>` is ever restored — the double-fire ship-blocker) + `test_direct_intake_service.js` green (backend unchanged) + C3. No corpus/M=0 (no confirm/filing-path code changed). **BUILT + COMMITTED DARK 2026-09-13** `9d57c76` (C1+C2 folded in). Non-blocking nits fixed: stale "checked by quickfile.js" comment. Nit left as-is per Oracle: no Department layout slot in code (cleaner).

2026-09-13 CLIENT SEARCH PARITY + POP-OUT plan (`docs/designs/CLIENT_SEARCH_PARITY_PLAN_2026-09-13.md`, eric-designed) — SIGN OFF WITH CONDITIONS for S0 AS RE-PROPOSED (JS-only extraction into `src/windows/shared/search-ui/`, plain `<script>` globals, a `window.SearchTransport` adapter set BEFORE the shared scripts, caps extended by the IO audit, a COMMITTED generated client copy + hash pin); the plan AS WRITTEN would be SEND BACK on three facts: (1) `src/shared/` sits in a gap — `scripts/harden-js.js:99-103` protects only `src/windows/` and `verify-release-artifact.js:28` PLAINTEXT_FORBIDDEN only flags `src/(modules|services|lib)|database`; (2) the plan's transport silently drops three core actions with NO /v1 backing — `repairDeconfirm` (send-back), `restoreAllDeleted`, and the desktop-local Explorer/Open File/Print trio — plus doc types/entitlement/current-user/settings/push events; (3) the client CSP (`client/renderer/index.html:5-6`, `style-src 'unsafe-inline'` with no `'self'`) blocks a linked shared stylesheet. Fork rulings: location `src/windows/shared/search-ui/` GO; plain globals + adapter GO (the `--smoke-windows` probe `src/main.js:1079` and `test_no_global_collisions` depend on it); MARKUP + CSS sharing REQUIRED by the mandate (JS-only leaves a 38 KB markup fork that rots on the next added button) but NOT in S0 → its own slice S0b, gated by a headless old-vs-new DOM-equivalence + computed-style diff (`#app.outerHTML` + display/flex/height of `#app,#body,#results-pane,#preview-pane,#preview-sidebar`) — a source pin cannot see the `#app{flex column;100vh}`→`#body{flex:1}` height-chain break a `<div id="search-root">` WRAPPER would cause; S0b must land BEFORE S1. Committed copy over symlink/extraResources/bundle (symlinks need Developer Mode — the winCodeSign trap; extraResources lands outside the asar + the navGuard root). Q-F: the pop-out becomes the client's ONLY search; the in-pane search (`client/renderer/renderer.js:985-1224`) reduces to a launcher in S1 (its preview kept for the mailbox decision path until S4) — mounting the shared module in the SAME page as renderer.js is the `_btn` collision class (`confLevel` is declared in search-state AND local at renderer.js:1174). Premise corrections: `_syncPageNav` shows `#match-nav` unconditionally (search-preview.js:78-79) → `caps.find=false` needs a NEW branch, gated so the core (true) path is unchanged; the pop-out CANNOT learn the user's ROLE (no `/v1/auth/me`; the main window learns it only from the login response, renderer.js:296) and the shared UI gates Delete/Restore/Purge/Send-back/bin on `SearchState.role` → S1 needs a `client-current-user` IPC (main caches {role,username,displayName} from login, cleared on logout; null → read-only, fail-safe); the type filter over /v1 is NOT dead (handler.js:510-518 → searchService.js:51 reads docType); /v1 rows carry no `has_file`/`page_count`, `page_count` rides the DETAIL (dto.js:33) which the preview reads from `merged` → fast-first-page works over the wire. SEAMS: (1) smoke probe keeps the exact global names + ADDS `SearchTransport`,`SearchUI`; (2) `test_search_detail_depathed.js:23-24` scans only `/`-less scripts → goes VACUOUS for the shared files — extend it over `search-ui/*.js` + assert positively that `coreTransport.getDocumentDetail` maps to `window.docusnap.getDocumentDetail`; (3) `test_no_global_collisions` survives (buckets `/` paths as shared, still fails cross-file dupes) + needs a CLIENT twin in S1; (4) 9 pins ENOENT on the move — repoint each RED-then-GREEN (`test_preview_error_state:21`, `test_find_in_document_box:33,43,58`, `test_fast_first_page:41`, `test_search_print_action:34` needs a `window.SearchTransport` stub before its require, `src/lib/test_ooxml_grid:60`, `test_purge_and_bin_truth:120`, `test_bin_changed_broadcast:70` keep the `onBinChanged` event name, `test_activity_strip:105` pre-existing red — repoint anyway); (5) `shared/thumbs.js` captures `window.docusnap` at load and owns `window.Thumbs` → the search-ui twin uses a DIFFERENT global `window.SearchThumbs` and the core page DROPS `../shared/thumbs.js` (two scripts assigning `window.Thumbs` = load-order roulette the collision pin cannot see); (6) CSS TOKEN DIVERGENCE (S1 blocker): the search CSS consumes `--sans/--accent-bg/--accent-border/--err-bg/--warn-bg/--ok-bg/--doc-bg`, none defined in the client's hand-copied token sheet (`client/renderer/index.html:16-30`) → highlights/tints/backdrop render TRANSPARENT → add `theme.css` to the sync set + a shared client theme-boot; (7) lifecycle (S1): `client-logout` (main.js:266) clears pageCache but leaves the pop-out alive; `window-all-closed` needs the pop-out closed too → main closes the pop-out on logout AND on `win` 'closed'; connection events broadcast to ALL windows; (8) FIND TIMEOUT TRIPS CONNECTION-LOST (S2 blocker): apiClient.js:101 idle 20 s rejects `connection timed out`, `isNetworkError` (main.js:223) matches `timed?\s*out` → `markConnection(false)`; `pdf_find.py` OCRs up to `_MAX_PAGES=100` × two PSM passes, response not streamed → a slow scanned find shows "Connection lost" → `client-find` NOT `guarded()`, long `timeoutMs`, timeout → a visible "Find took too long on this scan"; server caps the /v1 OCR fallback page count + returns `kind:'partial'`; (9) `/page/:index?scale=` clamps scale (cap 4) + index≥0 server-side (scale 50 = memory DoS); (10) sync + CRLF: `core.autocrlf=true` → the hash pin normalises line endings, hashes the copy minus its banner, MIRRORS (deletes removed files). Adapter semantics: PURE pass-through `(...a) => window.docusnap.x(...a)` — no `.catch`, no envelope, exact positional arity (a "normalised" `{matches:[]}` on failure would turn today's `_clearMatches()` into a visible `no-match` state). CONDITIONS S0: C1 adapter pass-through pinned (source regex + a rejecting stub rejects through); C2 drop `shared/thumbs.js` from search/index.html, twin = `window.SearchThumbs`; C3 repoint the 9 pins red-then-green, extend the de-pathing pin, extend the collision pin over the new set + prove it bites with a planted dupe; C4 `test_search_ui_no_direct_ipc` fails on a planted `window.docusnap`, `test_client_search_sync` fails on a one-byte change (CRLF-normalised, banner-aware, mirror); C5 smoke probe + `htmlAssetProblems` over the new html. GATE S0: `npm run test:pins` same-or-better (only `test_activity_strip` red) + owner live open of core Search (results, preview, page nav, find, xlsx grid, bin, mailbox deep-link, stamp popup). S1 conditions: pop-out CSP `style-src 'self' 'unsafe-inline'`; theme.css in the sync set + shared client theme-boot; `client-current-user`; logout + main-close kill the pop-out; connection broadcast to all windows; client collision-pin twin; in-pane search → launcher; a cap false for a REMEDIABLE reason (contract drift) shows one line "Some tools need a newer ScanFinder on the core PC", inherently-desktop actions just hide. S2: seams 8–9; `FIND_MAX_INFLIGHT` + q≥2; never log `q` (mirror handler.js:514-515); contract 1.2.0→1.3.0 MINOR, `CLIENT_CONTRACT` lockstep; `binRestoreAll` cap or a new endpoint. OCR reality: the only OCR touchpoint is S2's find; boxes are page fractions (frame-correct); the per-doc word-box cache (`pdf_find.py:180`) makes repeat finds cheap — the FIRST find on a 30-page scan is the expensive one (seam 8). No new OCR engine.

## 2026-09-14 — `/v1` contract 1.3.0 → 1.4.0: the search pop-out's last workflow bits (per-doc routes / history, admin cancel, new stamp type, stamped-copy overlay) + the routed banner re-homed in the popup — **SIGN OFF WITH CONDITIONS** (all four built in the same commit)
2026-09-14 v1_workflow_minor_1_4_0 — vetted the UNCOMMITTED diff. **Premise holds:** the four routes (`src/modules/api/handler.js` after the stamping block) mirror their desktop twins (`workflow/handler.js:97-133`) one-to-one — `isWriter` on the reads, admin on the writes PLUS the service's own admin check (`workflowService.js:396`), `_canAccess` on both by-id seams (SEC-03), explicit / SQL projection (no comment, no stamped_path), cancel through `dto.projectRoute`; `WORKFLOW_ROUTE` gates before dispatch; the token never reaches the pop-out; every server string through `esc()`/`Number()`; the client overlay is createElement-only; boot order right (role set → `afterInit` → `refreshCaps`); nothing touches extraction/filing. **Fork ruling:** re-homing the routed banner + admin cancel in the SHARED POPUP is the right layer — `searchActions.js:21-24` declares `_providers` and never iterates it (the loop went with the 2026-08-28 popup redesign `9c50b93`), so the inline `searchWorkflow._provide` surfaces were dead on BOTH apps; do not resurrect the inline provider. **Two seams found:** (1) **OC2 misattribution** — the new popup history row named `from_username` as the actor of EVERY `recalled` row, but three producers share that state (sender recall / delete-close / admin cancel; `workflowService.js:390-392`) → the popup would print the sender as the actor of an admin's cancel; the dead inline block honoured OC2 (never a guessed actor) — Fix B removed a guard Fix A relied on. (2) **cap-flip-on-404 vs hide-existence** — `capGated` flipped `caps.docRoutes`/`workflowHistory` OFF for the session on ANY 404, but the per-doc reads answer 404 for a doc hidden by `accessService` (purged-under-you — real since this morning's purge fix — or department-restricted, fail-closed regardless of the switch) → one hidden doc silently stripped the banner / cancel / history from every later doc until the window reopened; the handshake already decides the cap, so the 404 flip was redundant and harmful. **Non-blocking, logged in pendingfeatures:** the popup shows banners AND the assign form (E1 showed banners INSTEAD → an edit user can stack a second open route; lost at 9c50b93, tolerated by `adminCancelRoute`); `unwrap` words every 403 as "no access to this document" (wrong for a role-refused cancel after a demotion; cosmetic); the S2 preview reads still flip on 404 (pre-existing). **CONDITIONS (BUILT 2026-09-14, same commit):** C1 `searchStamp.js` names the sender on a `recalled` row ONLY when `resolution_comment` is NULL (NULL-ness is the sanctioned discriminator, never the text); otherwise state + comment, NO actor — harness rows for both producers pin it on both apps. C2 the per-doc reads use `docRead`: a 404 yields the empty shape for THAT call and leaves the cap alone (426/402 still flip) — the harness's doc 3 answers 404 before doc 2 is read and the drive asserts the cap survived + the pop-out pin asserts the call order. C3 a behavioural EDIT-role run (`--role edit`) on a 1.4.0 core: no [Cancel route], no "+ New stamp", no permanent delete / Empty bin, and `client-wf-admin-cancel` / `client-wf-stamp-type-create` / purge channels NEVER called — "hidden beats refused" pinned by behaviour, not a source regex. C4 gate: `npm run test:pins` green incl. `test_client_search_sync`, `test_v1_workflow`, the core functional pin, the pop-out pin (A1 parity, A2 lite, A3 workflow, A5 edit, A4 vs a 1.3.0 core); client `npm run dist` in lockstep at 1.4.0.

## 2026-09-14 — Search viewer: the CONTENTS panel (the PDF's bookmarks → click to jump) + viewer SPEED (JPEG for scan pages, a thumbnail concurrency cap) — contract 1.5.0 — **SIGN OFF WITH CONDITIONS** (all built in the same commit); Phase-2 fork ruled
2026-09-14 contents_panel_viewer_speed — vetted the UNCOMMITTED diff. **Premise holds:** `getDocumentPages` and `getThumbnail` never pass `--format` and the array path renders with the hard-coded `fmt='png'` → the Review / teach / OCR-crop renders are untouched; the format changes only the ENCODING, not the bitmap dimensions, so the stamp ghost (fractions of `img.getBoundingClientRect()`) and the find overlay (`%` of the wrapper) are frame-independent; print is server-side; `get_bounds()` exists on the vendored pypdfium2; q90 4:2:0 at 216 DPI is fine for a viewer (a vector page with a full-page background image goes JPEG — cosmetic softening only); pypdfium2 5.10's `get_toc` carries a `seen` set + `max_depth` so a cyclic bookmark tree cannot hang the spawn, and the 500/200 caps are enforced twice; hidden-existence preserved (`_gateDoc` 404 → `docRead` returns `[]`, cap kept). Measurements accepted (spawn 0.26-0.29 s fixed; PNG-encoding a scan page at scale 3 = 0.84 s + 5 MB; JPEG 0.03 s + ~1 MB). **The seam (one real one):** the Contents panel meets the COUNT-UNKNOWN lazy path — for a NULL/0 `page_count` doc (Quick File, pre-mig-37, a separated original) `currentPages = [first]` until the async probe lands; the outline arrives in parallel with live buttons, and `_showPage` returns SILENTLY on `idx >= currentPages.length`; if the probe fails every entry beyond page 1 is a dead button forever — the "looks broken" class. **Anomalies:** the thumbnail queue was FIFO, so a fast scroll / new search queued gone rows AHEAD of the visible ones, `isConnected` was only checked after the spawn, and a skipped job could poison the per-doc cache with null; the Python pin proved "no `--format` → PNG" on a VECTOR page only (PNG under `auto` too — cannot catch a flipped argparse default); strip/band-encoded scans (N images each < 50%) would stay on the slow PNG path (fail-safe). **CONDITIONS (BUILT 2026-09-14):** C1 an outline entry with `page >= currentPages.length` renders UNAVAILABLE (greyed + tooltip "Page N isn't available"), re-evaluated from `_syncPageNav` (which runs after the count probe and every page show), and a click on it explains itself inline — never silent; harness: the count-unknown doc 3 with a bookmark beyond its 2 pages. C2 `searchThumbs.js`: LIFO pump; at dequeue a job whose `<img>` is disconnected is skipped AND `cache.delete(doc.id)` so a re-scroll re-requests; the cache entry is set BEFORE the synchronous pump (else the forgotten null is re-inserted); unit pin `src/windows/shared/test_search_thumbs_queue.js` (queue 10 / 8 gone → 2 calls; LIFO; forgotten rows re-request). C3 the Python pin also proves a RASTER page with no `--format` stays PNG. C4 (optional, done) `_is_raster_page` SUMS image cover for the strip class. **C5 Phase-2 ruling:** build the COALESCED single spawn first — one `--page-info` flag answering `{page, pages, outline}` for the first paint (existing outputs byte-identical; also removes the C1 seam and the wasted outline spawn on the 99% of PDFs with no bookmarks); a persistent worker only later and only for page / thumb / count / outline — never the array render or `pdf_find` (Tesseract seconds; per-request `deps.env`) — relying on the `deps.spawn` seam and passing pages.py's argv verbatim per JSON line; it DISABLES per-call isolation (a hostile LAN-uploaded PDF crashing pdfium hits every viewer) → per-request timeout + kill + restart, spawn-per-call fallback, per-file quarantine after two deaths, explicit `close()` + recycle after N requests, a `will-quit` kill (+ the dev-restart trap). **Pre-existing, not blocking:** no previewService spawn has a timeout; the `/v1` page / count / outline reads lack find's in-flight cap. **Gate:** pins green incl. C1-C3; `sync-client-search` hash pin; `--smoke-windows` 14/14 on the packaged build.

## 2026-09-14 — `--page-info`: ONE render process for the first paint (page + count + bookmarks) + a 3-page READ-AHEAD batch (owner: "prefetch 2 or 3 pages after the one in view") — contract 1.6.0 — **SIGN OFF WITH CONDITIONS** (C1-C4 built, C5 built too)
2026-09-14 page_info_readahead — vetted the UNCOMMITTED diff (the slice ruled for as C5 of the previous entry). **Premise holds:** one spawn for page + count + outline, no `also` on the first paint; blast radius = the search viewer only (`page-info|--also` touches search / preview / api / client / tests; pages.py's default / thumb / count / outline branches unchanged; nothing under processing / extraction / ocr). **Bounded:** desktop ≤ 1+8 (`PAGE_INFO_ALSO_MAX`), `/v1` ≤ 1+4 (`PAGE_INFO_ALSO_MAX_V1`), Python `_ALSO_MAX`, the per-page dimension clamp still precedes every render. **`_pageJobs` seam:** a stale batch cannot write (selection token + jobs-map identity; a new Map per select also catches a same-doc re-select); a hole is never requested twice — BUT the busy flag leaked (C1). **Memory:** the same array that already kept every visited page, +3 pages, dropped on the next select; one ~15 MB structured clone vs the old 37 MB full render — acceptable. **Fallbacks:** desktop hidden doc → `_assertDocAccess` rejects → caught → the per-page path → the existing error state; client 404 → null, cap kept → per-page null → `getDocumentPages` [] → "No preview available"; non-PDF skips both branches. **CONDITIONS (BUILT 2026-09-14):** C1 the busy-flag reset is identity-guarded (`if (jobs === _pageJobs) _prefetchBusy = false`) — a batch landing late from selection A must not free B's latch, else B's next flip spawns a SECOND concurrent batch (index dedup held, the one-process invariant did not); pin = the harness `--race` run on BOTH apps (A = a slow batch, B = an 8-page doc with a far bookmark, jump mid-batch → never more than one read-ahead batch in flight for B; the re-arm is sequential). C2 the stamped ⇄ original swap seam — `_showPage` captures the page array before its await and bails if the SET was swapped (a late render must never overwrite a STAMPED page with the ORIGINAL), and the batch writer is bounded by `i < currentPages.length` (never extends a swapped shorter array). C3 count precedence — the FILE's count wins (`info.pages >= 1`, not `> 1`): a stale row `page_count` above the real count would offer pages that render as duplicates of the last page. C4 the pins count, not test existence: EXACTLY two page-info calls for doc 1 (the first paint, then one batch) on both apps. **C5 (recommended, built):** on a searched document the first read-ahead waits for the search-term find (`_prefetchHold` → `_prefetchRelease` after the find lands) — the find is a Tesseract run on a scan and the highlight is what the user is waiting for; two-core contention. **Gate:** pins + both apps' harness runs (incl. `--race`, A6 vs a 1.5.0 core) green; the Python pin byte-identical on the existing modes.

## 2026-09-14 — DATE FORMS: the test customer's "23rd Aug 2026" refused at confirm → ONE month-name rule on every surface (mig 167 `date_forms_wide` DARK on the engine side; the confirm door + the Review/Teach readers unswitched) — **SIGN OFF WITH CONDITIONS** (C1-C4 built the same afternoon; C5 = the flip gate, recorded)
2026-09-14 date_forms_wide — vetted the UNCOMMITTED diff (reggie's design, main tightened it after the advisor's draft let "Aug 2026" split into 20 + 26). **Premise holds:** the engine (`validator.parse_date`) already read "23rd Aug 2026"; the refusal was the desktop CONFIRM door (`filing/handler.js parseDate` — no ordinal, no day name, no 2-digit year or `-`/`/`/`.`/`\` on month-name shapes) and the shared `validation_patterns.date` (credibility gate / keyword qualification / Review badge — no ordinal, no `D-Mon-YYYY`). Widening ONLY the month-name family is the right cut: the month name is the guard that keeps "3.5.2", "1,234.56", "12-34-5678" out; numeric dates are untouched. **Layering:** the human doors (confirm, Review drawn/typed, Teach) take the wide forms UNSWITCHED (a person's value — refusing it is the bug); the engine half (crop credibility, keyword qualification + the G4 "a date is not a reference" guard, `parse_date`) is DARK behind `date_forms_wide` because a wider substring pattern can also ADMIT a wrong crop as "a credible date" — that is the blast radius and it needs the census. OFF byte-identical verified by the pin (validator/anchor/keyword unchanged with the env unset). **Seams checked:** the digits→digits separator is NON-EMPTY in every reader + the config MDY pattern (else "Aug 2026" reads as a date); the day-first/month-first regexes agree across JS door / Review / Teach / Python by a shared vector file; the JS numeric branches now round-trip (JS `Date` rolled "31/04/2026" to 1 May — strptime never did; the door and the engine must refuse the same values). **CONDITIONS:** C1 the Review drawn-date reader `_matchStrictDate` must ALSO refuse an impossible day (it only checked ≤ 31; "31 Apr 2026" would show as a valid drawn date and then be refused at confirm) — add the calendar round-trip, pin "31 Apr 2026" / "31/04/2026" refused. C2 the Quick-check grid's `_baValPats` reads the raw config and never got the `date_wide` merge — apply it through ONE shared helper with the badge (`ensureValidationPatterns`) so the two readers cannot drift; pin it. C3 the pin only literal-checked the day-first fragment; literal-pin the month-first fragment `(?:(?:st|nd|rd|th)\s*[,./\\-]?\s*|\s*[,./\\-]\s*|\s+)(\d{2}|\d{4})$/i` in all three JS readers (that fragment IS the "digits→digits never glued" rule). C4 delete the now-unreferenced `DATE_FORMATS` in `filing/handler.js` (dead code beside the door invites a future reader to "use" it). C5 FLIP GATE for the engine half: realdoc 605 M=0 + `wouldFile` set-equality OFF vs ON, plus the `VAL_CENSUS_DIR` crop/keyword acceptance census OFF vs ON where EVERY new acceptance is hand-verified against the rendered page (a wider pattern must admit only real dates), run with mig 166 in its SHIPPING state (OFF) so the two date arcs are not measured entangled; then the customer-default flip is the owner's call. **BUILT the same afternoon:** C1 `_realDMY` gates every `_matchStrictDate` return (+ the Teach twin `_parsesAsDate` round-trips too); C2 `_widenDatePatterns(pats)` — one helper, both call sites; C3 + C1/C2 functional pins (the pure renderer functions are LIFTED out of the source and RUN against the shared vectors — `src/windows/review/test_validation_pattern_surfaces.js`, 170 checks); C4 deleted. Pins 363/363 green; Python pin green under `py -3.12` AND `vendor\python`. C5 recorded in `pendingfeatures.md` + `docs/DARK_SWITCH_LEDGER.md`.

## 2026-09-14 — TEACH OVER THE SEARCH CLIENT (docs/designs/TEACH_OVER_CLIENT_2026-09-14.md) — **SIGN OFF WITH CONDITIONS** (C1-C7; owner then added upload-to-teach = B, its own follow-up pass below)
2026-09-14 teach_over_client — vetted the reconciled eric+gary design. Fork: owner-decision A (teach from the core's review queue) is a coherent v1 cut, does not collapse without B; ship A with empty-queue UX + honest copy. **Verified:** `_upsertTemplate` has no `await` before its row writes (review/handler.js:1801-2080) → the create/reuse core + all mappings/fixed/hidden can sit in ONE synchronous `better-sqlite3 db.transaction()`; `saveMapping`/`setFieldFixedValue`/`setHiddenField` all upsert (idempotent); the double `_upsertTemplate` (step-3 CREATE + step-4 `onTaughtConfirm`) is BENIGN (the 2nd reads `doc.template_id` set in-transaction → UPDATE/reuse, no second template, no mapping re-write); premise "no state after step 3 where extraction reads WRONG" HOLDS; `taught_fields:[]` isolation HOLDS — BUT plain /v1 confirm ALREADY mints GRADUATION templates via `onScopeGraduated` (reviewService.js:612, W=10-gated, no drawn mappings), so the grep/`confirm_never_teaches` pins bound DIRECT drawn-mapping teach, not graduation (state precisely). **Premise corrections:** (a) BLAST RADIUS overstated — the guided wizard writes template/(supplier,type)-scoped `template_field_mappings` + `template_fields` + `template_hidden_fields` + `field_label_overrides`, NOT the app-wide `field_anchors` (that is the Stage-2 ⊕ `save-field-anchor` path this design does not expose); (b) render-parity is the DESKEW-FRAME (`expand=False`) + aspect/orientation, NOT scale — mappings are stored NORMALIZED (0-1) so desktop `TEACH_RENDER_SCALE=4.0` vs /v1 `PAGE_SCALE 3/4` yields identical coords; auto-rotate is baked at import; the ONLY real divergence is the interactive deskew frame. **Seams:** double-upsert benign (above); idempotency ledger recorded at step 6 → a crash in [step-3-committed .. step-6] is caught only by write-idempotency, not the ledger short-circuit (fragile-by-implicit → C5). **CONDITIONS C1-C7:** C1 the DB-equivalence gate must compare `{templates, template_fields, template_field_mappings, template_hidden_fields, template_landmarks, field_label_overrides}` (ADD template_fields + template_hidden_fields — fixed/hidden live there; a mis-written fixed value slips a mappings-only gate), drop field_anchors (or keep as a zero-assert). C2 correct the blast-radius text (design lines 138/204/179) + re-target the audit metadata to the real tables. C3 render-parity gate BEFORE S1: `/v1/documents/:id/page-deskew` MUST use `expand=False` + identical `detect_skew` params to desktop `get-page-deskew`; assert identical returned image DIMENSIONS + `measured`/`angle` (do NOT force scale equality — coords are scale-invariant). C4 restate the ack scope honestly: the type-split/identity acks bound wrong-TYPE + wrong-IDENTITY only; a mis-drawn BOX on an already-graduated (supplier,type) auto-files wrong values, bounded by audit + reversibility + admin-auth (desktop parity) — OPTIONAL hardening: a re-taught template on an already-graduated scope holds siblings for one human-agreeing confirm (mirror identity-hold-siblings). C5 crash-replay idempotency pin (step-3 commits, step-4/6 never complete → replay same `teachCommitId` = EXACTLY ONE template + ZERO dup mapping/fixed/hidden rows); PREFER recording `teachCommitId` in a `pending` state INSIDE the step-3 transaction, promoted to `done` at step 6. C6 the wizard payload `sample_deskew_angle`/`angle_measured` MUST be written synchronously in the step-3 transaction (the `sample_deskew_angle IS NULL` guarded UPDATE, handler.js:1693-1708 — the 2026-09-07 placement root-cause fix); only the Python `generateSampleAngle`/landmarks/fingerprint spawns are step-5 best-effort. C7 empty-queue UX ("no documents are waiting to teach — scan one into the main PC first") + owner copy that this teaches CORE-scanned docs. **GATE before S3:** the CORRECTED equivalence pin (C1) + the render-parity assertion (C3) substitute for a realdoc M=0 run ONLY IF /v1/teach/commit re-drives the same functions with the same payload and does not alter `reviewService.confirm`'s taught path (else run `realdoc_regression.js` M=0); PLUS the atomicity pin (failing mapping → ZERO rows) + the crash-replay pin (C5) + `test_v1_teach.js` auth matrix (no-token 401 / edit 403 pins admin-only / unentitled 402 / license-denied 403) + `test_v1_confirm_never_teaches.js` + the handler-grep pin (bounding DIRECT mapping-teach, per §2). S0 (byte-identical shared-module extraction) is unconditioned — none of C1-C7 bite it.

## 2026-09-14 — TEACH OVER THE SEARCH CLIENT, slice B (upload-to-teach) — follow-up — **SIGN OFF WITH CONDITIONS** (C8-C14; A's C1-C7 stand; gates S4, not S0)
2026-09-14 teach_over_client_B — owner reversed the B defer ("we will implement upload to teach too"). Vetted the NEW OCR-import surface B adds. Forks: Q2 in-flight=1 + size + page-count + audit is SUFFICIENT (no per-day quota); Q3 admin-only AFFIRMED. **Premise (Q1) HOLDS:** reusing the import path over /v1 with autoFile:false is sound. autoFile suppression verified clean (`processing/handler.js` :2968 `autoFileRun = opts.autoFile !== false` → :3159 → :6602 `if (autoFileRun) _maybeAutoFile`, gated at the call site — cannot re-arm on a graduated scope). Existing per-file watchdog (`file_timeout_seconds`=300 :2230; JS process-tree kill :3933-3941) frees a wedged worker. Only local-trust assumption a remote trigger stresses is fan-out → closed by `TEACH_STAGE_MAX_INFLIGHT=1`. **New seam (Q4):** the staged `needs_review` row is concurrently confirmable by the core operator (then `/v1/teach/commit` fails its step-2 teachable-row validation) and accumulates invisibly to the remote uploader. **CONDITIONS C8-C14:** C8 the /v1 stage must drive the SAME auto-file gate with `autoFileRun=false` — `process-folder` is an `ipcMain.handle` (uses `requireRole` + `event.sender`), NOT reusable verbatim; extract/invoke the underlying batch path with a no-op progress sink; pin `autoFile:false` via the /v1 route incl. a GRADUATED scope. C9 return the docId captured from the import result (`_handleFileMessage` `msg.db_id`), NOT the desktop filename-pick (`teach/renderer.js:237` — fragile under duplicate names). C10 in-flight decrement + temp-FOLDER cleanup leak-proof on every terminal path (mirror intake's `finish()`+done-guard+`req.on('aborted'|'error')`, handler.js:833-840); clean the temp AFTER the import (it reads from there); the /v1 in-flight counter is separate from the worker watchdog and must release in a `finally`. C11 page-count HARD cap via a cheap `render/pages.py` count PRE-PROBE BEFORE the OCR spawn (reject >cap in ms, not after pegging the slot for the watchdog's full duration). C12 mark stage-origin rows ("staged for teaching by <user> via client") so the core queue distinguishes them + a future janitor can reap abandoned ones (marking cheap, ships with S4; janitor post-v1) + a specific "that document was already filed — re-import it to teach" message on the commit race, not a generic 400. C13 admin-only AFFIRMED (stage writes no learning row, but the commit is admin-only → an edit-only stage is a dead-end half-capability, and stage is the remote OCR-flood surface); record the deliberate desktop(edit)-vs-/v1(admin) asymmetry so a future dev doesn't harmonize it back open. C14 ACCEPTED RESIDUAL: a malicious admin looping sequential 40-page uploads keeps the one OCR slot busy (degrades the core's own throughput) — bounded by in-flight=1 (serialized) + watchdog + audit + the OFF switch; no quota for v1. **S4 GATE:** `test_v1_teach.js` stage pins — auth matrix (no-token 401; edit→403 admin-only; unentitled 402; switch off 409); refusals (non-PDF/OLE 415 via `fileKinds.isUploadIntake`; oversize 413; too-many-pages rejected by the PRE-PROBE before any OCR spawn); happy = exactly ONE `needs_review` row NOT filed + returned docId == created row; in-flight cap (2nd concurrent → 429) + the slot FREES after complete/abort/timeout (a slot-leak pin); `autoFile:false` PINNED even for a graduated (supplier,type) via the /v1 route. NO realdoc M=0 for B (reuses the vetted import path with auto-file suppressed; the teach half is governed by A's corrected DB-equivalence gate C1/C6).

## 2026-09-14 — Simpler client connection setup (SIMPLER_CLIENT_CONNECTION_2026-09-14.md)
VERDICT: SIGN OFF WITH CONDITIONS. Design sound + correctly scoped as a UX layer; settled
principle + the 3 rejections (broadcast / second-port / encrypted-DB fingerprint) all correct;
threat model honest; eric's compare-in-MAIN is the right fix and is STRONGER than today's
import-profile (which pins with no machine compare) + raw window.confirm TOFU.
Fork rulings: pairing "not mandatory" = ACCEPTABLE (it is today's shipped posture, not a new
regression; keep the lever real). mDNS/S4 = CONCUR DEFER (opt-in only if ever built).
SEAM eric couldn't see: a server-cert SAN gap (client dials an address not in managedSans →
ERR_TLS_CERT_ALTNAME_INVALID with an UNCHANGED CA) would fire the "server identity changed"
re-accept alarm → trains users through the one screen that must stay scary. Must branch on
error code (C3).
Conditions (build): C1 verified path fully main-resident, no renderer caPem round-trip;
C2 compare LOCALLY-COMPUTED fingerprint of the exact pinned bytes, never server-reported,
abort if uncomputable; C3 isCertError()-before-isNetworkError + ALTNAME(unchanged CA)≠CA-change
routing, never a rejectUnauthorized bypass; C4 keep pairing lever real + labelled a verification
aid (≥8 alnum or failed-pairing lockout; never mandatory-by-default — breaks env/sandbox enrol);
C5 enable-gate is UI-only, loopback/env/127.0.0.1 stay cert-free and unbroken.
Tests (must fail on the bug): T1 MITM caPem+lied fingerprint → refuse, not pinned; malformed PEM
→ refuse. T2 SAN-gap → "address not covered" (no re-pin) vs genuine CA change → refuse-default
re-accept; TLS-verify classified isCertError not isNetworkError. T3 PIN pairing default
(no-code open, code-set requires match). T4 S4 stays off/opt-in, host+port only, connect never
depends on it.

## 2026-09-16 — SPLIT-SEGMENT "LOOK FIRST" HOLD (the seam the watch_separate soak exposed: a merged multi-page cut reads 100 % clean and auto-files on manual import / can be swept later on watch) — **SIGN OFF WITH CONDITIONS** (C1-C10, all built the same day; mig 176 default ON on the Oracle's advice)
2026-09-16 split_segment_multipage_hold — vetted gary's design (thread the separator's rewrite set to `_handleFileMessage` on BOTH arrival paths; stamp a "— confirm once." lane-hold note on the ref-role row of every MULTI-page segment of a HEURISTIC split; sheet-bounded cuts + 1-page cuts untouched; a reprocess carries the mark). **Premise holds** (watch hold = import-time `autoFileRun=false` only, `watch/handler.js:483`; manual discards rewrites `:3300` and `_maybeAutoFile :6682` files a merged clean cut — measured 12/16 at 100 %). **Overstated:** "put_back_at is wrong because File-All bypasses it" is true only under DARK mig 87 — the note shape is still right, for robustness (`trust.js:1188` skips only put-back; `:1243-1254` counts any note). **Class boundary understated:** the belt sees merged CUTS only — a stack whose only first-page signature is page 1 is never split ('skip') and imports whole; that is separator accuracy (a reading fix, DARK, separate). Traces CONFIRMED: role-note refusal at every door incl. the File-All bypass and the sweep's Tier 2 (`sweepPredicate.js:60` 'stored-flagged' is the ONE Tier-2 guard → pin, C7); merge `:1695/:1746` behaviour → slice 2's post-pass at the ONE production merge site is the right layer (the hold is a property of the FILE, no re-read can verify page 2) → C1 same release; A1 confirmed (no reprocess path feeds stored notes to Python; mig 162's door keys on the engine's own note); `rereadHolds` no-double-note is cosmetic; `applySeparationToTracked` reads only original/segments. Seams gary missed → conditions: **C4** the reason panel said "flagged by a formatting check" (false pointer) → own kind `segment-hold` + page range + "use Split"; **C5** under DARK `note_topic_dedup` the rank-2 hold would swallow a genuine ref advisory → `_refRank` 0 for the mark; **C3** never a row Review can't render, `supplier_name` LAST (issuerSiblingFillService machine-clears it on siblings, C10 source pin); **C6** the Review Split-PDF tool / a user's own `*_split_pA-B.pdf` (no rewrite) never held — pinned. Ruling: **DEFAULT-ON** (hold-only; byte-identical on every non-split import and every 1-page cut — the real_34 one-click flow untouched; the class it prevents is the worst silent one: company B's page bound inside A's filed PDF; an unlisted INSERT OR IGNORE seed is gate-clean, mig-169 precedent) and **apply to the MANUAL path** (one look per genuine multi-page heuristic cut = the same look an ungraduated sender already costs; sheets are the zero-look answer). Separate owner fork, not bundled: with the dangerous population durably marked, the 09-01 import-time hold of 1-page WATCH cuts could be relaxed to match manual. Gate: the three new pins + `test_note_topic_dedup` C5 + the existing gate/readiness/sweep/reprocess pins + the soak replay belt ON on both arrivals (real_34 → 0 marks, 34 ready; every `_split_pA-B` marked + refused; singles untouched; the genuine 2-page doc HELD = the pinned trade; manual OFF-vs-ON multi-page auto-file count recorded → ON = 0). The realdoc 605 harness is structurally blind to this belt (no separation pre-pass) — not the gate. Design record `docs/designs/SPLIT_SEGMENT_HOLD_2026-09-16.md`.

## 2026-09-16 — SEPARATOR ACCURACY: (A) thread the page's own title into the pre-pass (gary's design) + (B) the repeat-letterhead OVER-split found while building his controls — **(A) SIGN OFF WITH CONDITIONS (C1-C7, cascade shape, DARK, flip coupled to (B) slice 2) · (B) CONFIRMED, pre-existing, WIDER than described (three paths) — slice 1 = a self-declared-continuation veto in `segmentation.py` (build FIRST, DARK), slice 2 = supplier-aware identity_change, slice 3 = the orphan-shaped 1-page-cut belt; C8-C11**
2026-09-16 separator — vetted gary's title-threading design against the measured root cause (every missed boundary in the templated stacks = a `type_refused` from `identify_template` because the pre-pass passes no `detected_slug/title_trusted`; **framing corrected: not a coin flip but a systematic bias** — sibling fingerprints are identical (the letterhead words; STOP_WORDS strip the type words) so the stable sort picks the closest logo then `confirmed_count DESC` → the most-confirmed sibling every time → the type-presence veto refuses it whenever that type is armed and its heading is absent → a supplier's LESS-common types are under-split systematically). **A1 ruling: the CASCADE (trusted → untrusted slug → base, each falling back on None / type_refused), not trusted-only** — the untrusted slug reaches only the `:929` matching branch + `_match_by_keywords`' tie-only preference; the veto still runs on the slug-picked sibling; the only added case is a young sibling on a continuation page with a cross-type body mention (census cell). **Env claim holds** (`handler.js:2859` env = process.env + OMP; the DB-bridged switch envs never reach the pre-pass) → argv is the only kill. **"(A) adds no over-split exposure" was over-stated at the time** — the 6 controls all fire via the overlap path in every arm, so they could not show a delta on the identity-change path (S1: base picks the same sibling on p1/p2; the cascade picks type X on p1 and base's M on a logo-only p2 → `M != X` → "different template"). → C5 extended controls (logo-only p2, name-line p2, full-header p2, a "Page 1 of 2" first page, a young-sibling footer mention) with over-splits ≤ base per file; C6 the end-to-end TRUNCATION metric (0 auto-filed docs whose page_count < the GT doc's); C7 the flip waits for (B) slice 2 OR a measured 0 delta on (i)+(ii). **(B) confirmed by trace** (`extract_keyword_fingerprint :1316-1373` harvests the header band → a repeated letterhead reproduces the fingerprint → `fingerprint_overlap` ~1.0; the docstring's promise is false whenever the letterhead repeats; the veto protects only an armed picked type with the heading absent). Two more paths named: (b) same-supplier identity change, (c) `is_document_start` on a full-header continuation (HYPOTHESIS, one control). Proxy fairness: fair and mild — real Sage/Xero/QuickBooks continuation pages repeat the FULL header incl. the heading, number, date, customer block and "Page n of N". **Right layer:** REJECT fingerprint-minus-branding (≈ empty → real_34 collapses to one segment), REJECT doc-start-as-sole-fix (loses worksheets/dockets/alerts; and (c) fires on full headers), DROP the "heading PRESENT" positive twin (redundant with the veto on armed types, nothing on young ones). **Slice 1: `is_continuation_page` — "Page n of N"/"Page n" n ≥ 2 (never n=1), "continued"/"(cont.)"/"continuation", "brought forward"/"B/F" — universal, template-free, precise; OCR failure = today's behaviour or an under-split the mig-176 belt holds.** Slice 2: supplier-aware identity change (census same-supplier identity_change-only boundaries = 0 first). Slice 3: the human checkpoint for the residual (young sibling + full letterhead + no marker + no heading): a post-extraction belt — a HEURISTIC 1-page cut with NO ref AND NO date AND the same supplier as the preceding segment → hold BOTH with the segment-hold sentence. Recovery path (barry/eric): the note must say where the original scan is (`.sf_separated_originals`) or Review gains "Rejoin with previous". S4 named in plain terms: a Sage-class page 2 (ref + date + supplier repeated) orphans as a `…-DUPLICATE.pdf` beside the truncated page 1 — two wrong filed docs, nothing in Review, no way back. Coupling: (A) DARK ships now; (A)'s FLIP waits for slice 2 (or the (i)+(ii) 0-delta measurement); slice 1 is independent and highest value.
**BUILT the same day, both DARK:** mig 177 `segment_continuation_veto` (slice 1) + mig 178 `segment_title_slug` (A, cascade); `docs/designs/SEPARATOR_ACCURACY_2026-09-16.md`. Six-arm census: stacks 72 → 79/95 (0 lost), all 14 controls 0 over-splits (base 10), real_34 34/34; the (i)+(ii) logo-only / name-line controls measured 0 delta in EVERY arm (C7's alternative met). e2e truncation metric: RESULT.md "e2e".

## 2026-09-17 — SEPARATOR ARC 3: a KNOWN-SUPPLIER-NAME identity change (the template-free leg — a non-templated stack imports WHOLE; gary's design over a measured 93/95 signal) — **SIGN OFF WITH CONDITIONS (C1-C9; `band_corrob` YES with a REPAIRED witness + a position bound; DROP unknown→known and PIN it; minConfirms 3; the reader joins the excluded-readers pin) — built the same morning as mig 179 `segment_known_supplier_change`, DARK**
2026-09-17 separator arc 3 — **Premise check, three corrections + one confirmation.** (1) `bundle_05` page 2 is NOT "a supplier with 1 confirm" — `gt.json:150-177`: p1 Thornbury WORKSHEET, p2 Thornbury PURCHASE ORDER, p3 Northgate invoice; page 2 names the SAME issuer, so a name rule is structurally blind to a same-issuer type switch at any minConfirms — `bundle_05` stays 2/3 by design; the lever is a two-sided trusted-title switch (symmetric, asymmetry-safe) → `pendingfeatures.md`, not this arc. (2) "A miss is held by mig 176" is only true when the file gets ≥ 2 segments (`split_plan.js:59`; `segmentHoldPages` reads rewrites only) — a whole file with no cut imports whole and auto-files under page 1 (`RESULT.md:115-118`), so every recall miss of this rule, incl. the prefix miss, is today's behaviour = for a 2-doc stack a silent whole-file auto-file, not a hold; reframe the design doc. (3) The 93/95 was measured with a SUPERSET population (`seg_probe5.py:18-20`: every distinct confirmed name incl. machine confirms ∪ `templates.name` ∪ dominant_supplier, len ≥ 6, no confirm bar) — the number must be re-established with the SHIPPED reader (load-bearing). (4) Confirmed: `process_docs.py:880` strict `parse_args()` → the file must never ride `buildTrainingArgs` args; both callers have a tempFiles array (`handler.js:2974/2989`, `:3112-3114` cleaned `:3340`/`:3444`); `_separationOpts` `:2836-2845` gains the third param at BOTH sites; a 0-template install never reaches the rule (`segmentation.py:219`). **The seam:** veto ordering correct (rule → `if boundary and self_cont`); mig 176 → 179 converts HELD multi-page cuts into UNHELD 1-page cuts on MANUAL import (WATCH holds every fresh segment, `watch/handler.js:483-485`) → a false cut is the S4 silent class on manual import → the gate must be SEGMENT-level (over-splits = 0), not the filed-level truncation alone; `header_band_text` on a CONTINUATION page has no recipient marker → the band is the first 20 raw lines INCLUDING the item table → a line item naming a known supplier ("Print Tracker subscription") becomes the earliest admitted name on a logo-only p2 → cut; the `num` witness does NOT catch it (a Sage-class p2 repeats "Invoice No"/"Date") — the unmeasured false-cut cell → the position bound (C2); an unlabelled window-envelope recipient ABOVE the issuer + mutual B2B (both admitted) → page 1's earliest name = the recipient → `cur_name` wrong → p2's repeated letterhead ≠ → false cut, cascading — controls3 has the LABELLED form only → new control + optional mitigation `cur_names` as the SET of page-1 band names (costs the unlabelled-switch recall; measure both); 178 ↔ 179 coupling if the title-OR witness is adopted (`title_signal` needs `title_ctx`) → 179-ON/178-OFF is a reachable customer state → census that cell. **Q1 `band_corrob`:** right trade, WRONG LIST — bare `"no."` and `"date "` make a "Part No." column / "Registered No." footer / "Delivery date" satisfy it; repair = a LABELLED number marker (`_NUMBER_MARKERS` ∪ reference/ref/job/quote/credit note/delivery note/docket/ticket + our ref/your ref) OR a LABELLED date marker (`_DATE_MARKERS` ∪ "date:"/"dated") OR — only with `title_ctx` — a TRUSTED title (the arm that rescues marker-less worksheets/dockets); a pinned module constant; keep the c/o context exclusion alongside (belt and braces on the S4-silent class). **Q2 population:** Tier A verbatim (`learning.js:219-227`, `learningExcludedSql(db,'')`, `COALESCE(confirmed_via,'') NOT IN (MACHINE_VIAS_SQL)`, the `hasVia` probe, GROUP BY LOWER(TRIM)); Tier B `:231-238`; **minConfirms 3** (the function's own default; 2 admits every two-off test name — "Chris Docs" at 9 passes any threshold and 'docs' is in neither chrome list, harmless unless a band literally prints it, but it shows the population is only as clean as the confirms); readers pin SAFE — add the SITES row, a `known:` snapshot reading (1 → 0 after all stamps; no `template_fields` rows in the fixture), §6 Q-C1 an `intake='direct'` doc never enters; privacy/perf: the same names already ride the templates/hints temp JSONs per pre-pass, one GROUP BY is negligible. **Q3 the DROP:** agree — every narrower form (unknown→known + trusted title) is page-level and asymmetry-prone (a trusted title on p2 is a Sage/Xero/QB continuation's normal header); pin "unknown → known never a boundary". **Q4 prefix tolerance:** suppress-only → can never cause a false CUT; its cost is a miss = today's behaviour (premise 2: possibly a silent whole-file auto-file, NOT a hold); make it symmetric, after suffix-strip, pin the documented miss; widen the suffix list (uk/group/holdings/(uk)). **Q5 blast radius:** fires only inside the pre-pass on a multi-page PDF with ≥ 1 template, i ≥ 1, BOTH sides naming an admitted known supplier, with the witness, DARK, argv-only (emit file + flag together or not at all); on the owner's DB 21 human names at ≥ 2 (junk "Chris Docs" admitted by the 2-token rule, "ME" killed), the two-confirm tail gone at 3. Census cells: (a) ADMISSION — print the admitted list from the SHIPPED reader and eyeball it; (b) SEGMENT-PLAN DIFF OFF vs ON over every multi-page PDF the owner has, every delta reviewed; (c) real_34 34/34. **C1** the repaired witness (labelled markers as a pinned constant; bare "no."/"date " excluded; the title arm only with `title_ctx`). **C2** a position bound — the name within the first N ≤ 6 non-empty band lines (pick N by census; `name_top4` measured free). **C3** minConfirms 3; the c/o/care-of/delivered-by/collected-by/via/attn/fao exclusion on the line + the line above; `same_supplier` symmetric + the wider suffix list. **C4** controls5 measured with the SHIPPED functions (never the probe): (i) a logo-only p2 + a known supplier in a line item + repeated Invoice No/Date, no page number → 0 cuts; (ii) an unlabelled recipient block above the issuer, both known → 0 cuts; (iii) the two-doc non-templated known-known switch → 1 cut; (iv) a blurred p1 letterhead + a clean p2 → 0 cuts; (v) 179-ON/178-OFF over stacks + all controls; (vi) a `first_witness`-less real first page (a docket with only "Docket No.") → cut. **C5** re-establish the stack number with the shipped population: ≥ 91/95, 0 lost vs cascade+veto, over-splits 0 on stacks/real_34/singles/controls1-5. **C6** e2e with the three switches: truncation (auto-filed) = 0 AND `truncatedHeld` = 0 (`soak_e2e_check.js:40-43` counted a held wrong cut as a log line only — make it fail); `bundle_05` = a held 2-page + a 1-page; the auto-file set of every non-split file identical to the two-switch run. **C7** pins: `test_segmentation.py` §9-§12 incl. the two PINs (the veto beats the rule; unknown→known never), `test_split_plan.js` §8 (OFF argv byte-identical; both flags or neither), `test_segment_dark_seeds.js` (mig 179 seed 'false', listed, 48 → 49, no force-ON twin), the readers pin, the `learning.js` docstring naming the third consumer. **C8** commit the guarded-variant runner + outputs (the brief's `probe_name8_*` were not on disk). **C9** correct premises 1-2 in the design doc; log the two-sided trusted-title switch as the `bundle_05` lever in `pendingfeatures.md`. Not slice 2 — agreed (additive under-split fix; the (A) flip stays discharged).
**BUILT the same morning, DARK: mig 179 `segment_known_supplier_change`** to C1-C9 (C2 = 6 lines; the FIRST-PAGE SET mitigation adopted after its control false-cut under the first-name form — and made a WIDE read after Tesseract's right-block order defeated the strict band; a table-header band cut + a money-line exclusion added for the item-table seam and measured on their own controls); `docs/designs/SEPARATOR_ACCURACY_2026-09-16.md` "Arc 3"; census + e2e results in `RESULT.md`. **Follow-up ruling (same morning): the shipped-function census landed at 89/95 → the 91 bar was withdrawn** (derived from the probe's superset population) and replaced by "0 lost vs 177+178 AND every residual miss a named, deliberate fail-safe trade" (all six were); the SET reset on an unnamed cut = the correct call (carrying the name across = the garbled-p1 S4 class one document on — recorded as REJECTED); the third witness OR-arm (recipient block + a real date shape, one definition off `_RECIPIENT_MARKERS`) ACCEPTED if it buys no new over-split — it recovered the two marker-poor dockets (→ 91/95) and bought 0 anywhere. **e2e:** mig 179 adds 0 truncations; `bundle_05` held 2-page + 1-page; whole-file parity identical — but the widened control set exposed the PRE-EXISTING marker-less repeat-letterhead over-split with one page 1 AUTO-FILED (the S4 class caught live) → the slice-3 belt / a suppress-only "same supplier + no witness → never a fingerprint cut" extension = the next vet (pendingfeatures.md 2026-09-17).

## 2026-09-17 — THE S4 SILENT TRUNCATION BELT (a marker-less repeat-letterhead page 2 cut at page 2 → page 1 AUTO-FILED as a complete 1-page document; the arc-3 e2e caught it live, #1255) — gary's HYBRID design (pre-pass boundary CLASS + a post-extraction PAIR check that holds BOTH halves with a durable "— confirm once." note) → **SIGN OFF WITH CONDITIONS (C1-C12); DARK mig 180 `segment_pair_hold`; the customer FLIP waits on a recovery action (C12) + the C9 census cells**
2026-09-17 S4 pair belt — **Premise holds, traced** (#1255 AUTO-FILED + 7 held in `e2e3_result.txt`; `_maybeAutoFile` runs in the deferred tail `handler.js:6714-6753`; the import chip reads `review_hold` and flips on `doc-auto-filed`; Confirm clears every note `reviewService.js:480`; sweep Tier 2 `sweepPredicate.js:60`; watch `autoFileRun=false` for every fresh segment `watch/handler.js:485`; round-robin scatters a pair across shards). **Layer ruling: the HYBRID is right** — the pre-pass owns the boundary CLASS (WEAK = a template-only cut with no doc-start: same-template fingerprint ∨ "no template → template" ∨ a same-SUPPLIER sibling switch; STRONG = any doc-start hit, any known-supplier change, a cross-supplier template switch), the 200-DPI pipeline owns the VALUES (same supplier ∧ later page reads no date ∧ (no ref ∨ the predecessor's ref) → hold both). Pre-pass suppress-only REJECTED, but for a corrected reason: NOT "a suppressed only-cut is a silent whole-file auto-file" (on all four exhibits the suppress would be the RIGHT outcome) — the load-bearing reason is RECALL: 22 of 71 genuine stack boundaries are weak under the class, so a suppress would merge every one (a look + a Split each) and regress what 177/178/179 bought; only the values discriminate. Measured population (main session, `weak_cut_census2.py`, shipped functions): exhibits 4/4 weak, real_34 0/33 weak (every alert page is an EMAIL-header doc-start — the walk's "first-page fingerprint" label hides it by reason precedence, `segmentation.py:378-386`), controls3/5 weak = exactly the 4 exhibits, stacks ~22 weak (8 same-supplier sibling switches + ~14 "None → template"), every one reading its own ref + date → released. **WRONG SOURCE caught:** gary's `name_of = {template_id: templates.name}` — the codebase has ruled twice never to match on the cosmetic name (`template_matcher.py:643-670` `_template_identity`); "Purchase Order Template" vs "Invoice Template" for ONE supplier would make the sibling false cut STRONG and UNHELD, and an admin rename would silently change the class → C4. **Seams found → conditions:** S1 the 'release' re-invoke of `_maybeAutoFile` BYPASSES the watch hold (watch never called it at the original landing) → C1; S2 the release runs in the SUCCESSOR's sync leg before the predecessor's IO tail (working copy/rotate) → `commitDocument` could file an un-rotated source copy → C2; S3 overlapping pairs (p1|p2|p3, three weak cuts) erase each other's hold under idempotence-by-mark → C3; S4 `carrySegmentHold` carries ONE sentence (`.find`) → a row with both 176 + pair sentences loses one on a `used_new` merge → C5. Strict ref equality misses a one-glyph confusable slip on the repeated number (`INV-2959l`) → S4 again → C6. UNMEASURED: real_34's protection rests on the born-digital text layer reading all three of from:/sent:/subject: — a RASTERISED alert bundle at 150 DPI with one garbled marker → weak → p24/p25 (shared serial + date) held → C9(i). The Sage-class full-header repeat WITHOUT a page number is ds True → STRONG → unheld though the values would catch it → the "email-arm-only STRONG" alternative class as a census ARM, C9(iii). CX: the pair reason must NOT say "use Split" (wrong tool); `.sf_separated_originals` + "re-import" is not a recovery for the average person → DARK with the honest note is fine, the customer FLIP needs a one-click "Re-import as one document"/Rejoin (C12). Duplicate-scan trade (same ref + same date held for one look) ACCEPTED. DARK not default-ON: 176's precedent does not transfer — this belt clears what it writes, re-invokes auto-file and changes auto-file TIMING on every weak genuine boundary (C11). The C arm stays in this slice. **Conditions:** **C1** `landed` records `autoFileRun`; 'release' re-invokes `_maybeAutoFile` ONLY when the predecessor's landing had autoFileRun=true; on watch it clears the note and stops (PIN: a watch pair release never auto-files). **C2** 'release' awaits the predecessor's deferred IO before `_maybeAutoFile` (PIN: the rotated working copy files). **C3** the pair sentence names its partner page; stamp idempotence + `clearPairNote` key on the EXACT sentence, never the bare mark (PIN: p1|p2|p3 with (p1,p2) release + (p2,p3) hold → p2 stays marked). **C4** the C arm's identity = `_template_identity(t)`, never `templates.name`; empty → C False → STRONG (PIN: a cosmetic rename does not change `boundary_class`). **C5** `carrySegmentHold` carries EVERY mark-bearing sentence (filter, not find; PIN). **C6** refs compared after text_normalise + separator strip + a digit-confusable fold (O/0, I/l/1, S/5, B/8, Z/2 — hold direction only); the same-document arm needs a NON-EMPTY equal ref; dates via `parseDate`, unparseable = empty (PINs: `INV-2959l` ≡ `INV-29597` → hold; `INV-29598` → release; `25/12/2026` ≡ `25-12-2026`). **C7** reason-panel kind `segment-hold` subkind `pair` with its own copy (no "use Split"); the note reads sensibly when the partner never landed. **C8** pin `segments[k] ↔ made[k]` (pdf_splitter range order); `weak_pages` unconditional-emit pin; mig 180 seed + `TEST_SWITCH_KEYS` 50; commit `weak_cut_census2.py` + outputs. **C9** extra census cells: (i) real_34 RASTERISED at 150/200 DPI → weak count + held = 0 via the values; (ii) a young-install stack (templates for the SUCCESSOR's supplier only) → inconclusive count; (iii) the email-arm-only STRONG alternative class over stacks/real_34/controls; (iv) 178+179+belt ON over controls3/5 → every sibling false cut held. **C10** successor-error PIN; Stop after the predecessor landed → it keeps a durable note; a fresh import of the leftover `_split_p2` → no pair, no note (the filename is never the trigger — the 176 C6 rule). **C11** DARK (mig 180 `segment_pair_hold` 'false', no force-ON twin) with the rationale recorded. **C12** the FLIP gate adds a recovery action on the pair reason ("Re-import as one document" from `.sf_separated_originals`, or Rejoin-with-previous). **Gate before merge (DARK):** pins C1-C8 + gary's list green; exhibits 4/4 held + marked; stacks weak ≈22 → held 0, inconclusive 0, the auto-file SET identical OFF/ON; real_34 born-digital 0 weak AND rasterised held 0; controls1-5 segment-level over-split/lost unchanged; e2e3 `truncatedAutoFiled = 0 ∧ truncatedUnmarked = 0` with `soak_e2e_check.js` made to FAIL on an unmarked truncated half; whole-file parity identical; `bundle_05` unchanged. **Before the customer flip:** C12 + the C9 cells green.
**BUILT the same day, DARK: mig 180 `segment_pair_hold`** to C1-C12 (`docs/designs/SEGMENT_PAIR_HOLD_2026-09-17.md`; `segmentation.boundary_class`/`boundary_classes` + the unconditional `weak_pages` emit; `split_plan.buildPairContext`/`pairSentences`/`pairHoldDecision`/`clearPairSentence`, `hasSegmentHold` widened to either family, `carrySegmentHold` carries every mark-bearing sentence (C5); `handler._pairLanded`/`_pairTriple`/`_clearPairSentence` + the `_ioDone` deferred (C2) + `autoFileRun` recorded per landing (C1) + the exact-sentence stamp/clear (C3) + the C4 identity via `_template_identity`; reason panel subkind `pair` + own copy (C7); pins `test_segmentation.py` §13, `test_segment_pair_hold.js`, `test_segment_pair_stamp.js`, dark seeds, count pins 50; suite 385/385). Census with the shipped functions: exhibits 4/4 weak, real_34 0/33 weak, stacks ~22 weak all genuine (`weak_cut_census2.py` committed, C8). **e2e4** (72 files, the belt + 177/178/179 + 176): truncatedAutoFiled 0, merged 3/3 marked, pairMarkedExact 0, 3 of the 4 exhibit pairs held + marked — the blur pair RELEASED (page 1's number unreadable; the same-number arm needs both) → **follow-up ruling the same hour: SIGN OFF on the added arm + C6b** — the rule is the PRINCIPLE: a weak later page releases only on POSITIVE evidence of its own document (own number AND own date, not the same-document shape); any missing piece → inconclusive → hold both; a repeated value is never own evidence; the ONE release that must stay = an unread earlier number + a COMPLETE later page (a census count, revisit on the number). Restated + pinned ((i) no number + the same date → hold; (ii) a different number + no date → hold). **e2e5** (the blur pair, app restarted): inconclusive → both held + marked, truncatedUnmarked 0 → **all four exhibit pairs held + marked; the gate is green for the DARK merge.** The flip waits on C9 (rasterised real_34, a young-install stack, the email-arm-only class arm, 178+179+belt over controls3/5) + C12 (a one-click "Re-import as one document" / Rejoin).

## 2026-09-17 — Departments D4 (Settings IPC + UI) — SIGN OFF WITH CONDITIONS (build); SEND BACK (flip)
Vetted the eric (IPC/UI) + gary (tests) consensus. Verdict: build D4 while `departments_enabled` stays OFF (low
blast radius); BLOCK the switch from being set ON until the intake lanes fail-closed (a Help caveat is NOT enough —
silent exposure of confidential docs). Per-item conditions, all MET in the 2026-09-17 build:
- C2 audit adapter: nest the whole meta under `metadata` (spread would drop name/slug/departments/from/to →
  near-empty rows); action_category 'settings'. → `_deptAudit` in settings/handler.js; pinned test_department_settings §A/§E.
- C3 renameDepartment: in the SERVICE, slug STABLE (backup remap key), admin-gated + audited. → done; pinned §A.
- C4 updateType `default_department_id`: validate exists AND is_active=1, else drop; route via the existing IPC. → done; pinned §C.
- C5 write-side belt + gating split: switch gates UI + write-side tagging; read keys off DATA (D-C8 honesty). Belt =
  refuse a non-null tag while OFF (OFF can't un-hide). → done; pinned §B.
- C8 disable-with-tagged-docs: allow + persistent honesty banner (refuse traps the admin) linking to the remedy. → done (banner copy in renderer).
- D-C3 backup: departments a slug-keyed parent (upsertParent never DELETEs → M5-safe), default_department_id remap by
  slug (dangling→NULL, never foreign), folder-default settings excluded. → done; pinned test_department_backup_remap.
- item 8 flip gate: `departmentService.INTAKE_GUARDED = false`; `department-set-enabled(true)` refuses `intake_unguarded`.
Before the FLIP: D2b (intake membership rule) + the full denial matrix + mutation pin on a seeded multi-dept DB.

## 2026-09-18 — Departments D7 (multi-department, join tables) — SIGN OFF WITH CONDITIONS
Vetted barry (UX) + eric (schema/gate/IPC) + gary (migration/tests). Design: `docs/designs/DEPARTMENTS_D7_MULTI_2026-09-18.md`.
Model = a `document_departments` join (+ `document_type_departments`); shared = zero rows; visible if in ANY of a doc's
departments. DARK-compatible (byte-identical when no departments); `departments_enabled` stays opt-in. Conditions:
- HARD ITEM (SEND BACK → fixed in the design): the widening SET rule. `(next△old)⊆mine ∧ next≠∅` allows a self/team
  lockout ({Fin,HR}, actor∈{Fin}, next={HR}). FIX: own-slice/immutable-foreign — `effective = ownSelection ∪ (old\mine)`,
  require `ownSelection ⊆ mine` AND `ownSelection ≠ ∅`. Reduces to today's singleton rule; foreign tags preserved.
- #1 scalar: NULL after backfill (eric) — a NULL column can't fire its own RESTRICT; migration MUST be one transaction;
  go-forward-only stated + Help note (a downgrade past mig 184 with departments on exposes tagged docs); add 184 to any
  existing DB-newer-than-code guard; no new tripwire.
- #2 ON DELETE: document_departments.department_id RESTRICT (fail-closed), document_id CASCADE, type_departments
  CASCADE-both. Delete in-use count moves to the join; counts retired members.
- #4 decision doc.department_id→doc.id: keep the SYSTEM_ACTOR/not-configured/admin/all_departments short-circuits BEFORE
  the EXISTS; pin that decision only gets getById-sourced docs.
- #5 empty=shared model↔UX split sound; sole-department leak-warning attaches to the D6 bulk-move path (delete stays
  RESTRICT-blocked).
- #6 PERF is the true blast radius: ~13 correlated-EXISTS readers, corpus M=0 vacuous for latency → a perf gate on a
  departments-populated synthetic corpus (≥10k docs) BEFORE FLIP (not before merge); add index document_departments(department_id).
- #7 doctype_grants.department_id (dept×type deny) untouched — rule separately; drop default_department_id from
  updateType's allowed list when setTypeDefaultDepartments lands.
...
- 2026-09-22 (PP-OCR recognition-only slice fallback for confusable-glyph misreads; oscar ADOPT-WITH-CONDITIONS consensus): **SEND BACK.** Engine sub-decision = **DO-NOTHING** (unwarranted on current evidence); warranted fix = **WRONG-LAYER** (flag-door widen + format/confirmed-literal context). (1) **C1 mis-located / insufficient** — `_corrob_licensed` (engine.py:1938-1970) licenses on `len(fams)>=2 AND any∈_CORROB_PAGE_FAMILIES`; a same-pixel `{crop-Tesseract, ppocr}` pair passes because the co-located crop read supplies page-family membership regardless of whether ppocr is in the set. Real rule is producer-side: `_corrob_record_bucket` (engine.py:2028) must return None/host-family for PP; PP must never enter `_field_candidates`. Ship-blocker. (2) **Blast radius vs reward** — entire measured confusable population is ONE repeat supplier (Pelican PI→P1); the live exhibit supplier is also repeat; ZERO cold-supplier confusables. PP's only non-redundant class (cold, no history) is held-for-review regardless, so PP degrades from "prevents silent wrong-file" to "polishes the suggestion on an already-held doc" — 55MB ONNX runtime unjustified; C9 census returns 0 distinct cold suppliers by construction. (3) **C3 verified + worse** — ref_confusable_class_outlier (format_anomaly_checker.py:950-1012) Rule A range(1,len-1) excludes pos-0/last, Rule B needs digit-for-letter; exhibit `I625802868` (leading letter-for-digit) trips NEITHER; ref_confusable_flag is customer-ON (mig 173) → the exhibit silently auto-files TODAY and PP (gated to the flag) never fires on it. The reggie widen is a hard prerequisite and, format-context-gated, is the actual fix. C2×C3 seam: overwrite-permitted on a flag-door-blind field = silent wrong PP auto-file → PP must act only under a forced hold. (4) Note must not assert PP's value (rubber-stamp risk on the 1 common-mode-wrong). Redirect: fix flag door (context-gated) + lean on confirmed-literal/format lever; only revisit PP after a corpus with real cold-supplier confusables shows a non-zero PP delta, under corrected C1 + C2⊆C3 + neutral note. Gate adds: a pin that `_corrob_licensed`=False on the same-pixel `{crop,ppocr}` record (must fail on the naive design); wouldFile(ON)⊆wouldFile(OFF); pixel-adjudicated M=0/BROKE=0; determinism release-gated on vendor/python.

- 2026-09-22 (PP-OCR slice fallback — RE-RULE on the corrected live Print Tracker exhibit; supersedes the same-day SEND-BACK): **SIGN OFF WITH CONDITIONS.** Corrected evidence (coordinator, pixel-adjudicated doc00822, 34pp): the real exhibit is INTERIOR `1G25802868` (G@pos2→6 collapsing to a valid all-digit `1625802868`, Tess conf 14 vs Paddle 1.00), and p7 `RFH0738865` (O→0) at conf 76 is a demonstrated SILENT auto-file wrong on a heterogeneous UNIQUE serial. Verified in code: `ref_confusable_class_outlier` (format_anomaly_checker.py:1002-1011) is structurally silent on an all-digit read AND on a no-alpha-prefix serial (the `0` at RFH**0**738865 has alpha-left/digit-right → not isolated in an alpha run) — so the format-widen redirect AND confirmed-literal (mig 204) both provably cannot catch this class. DO-NOTHING does not survive: only an architecturally-independent second reader's DISAGREEMENT flags p7, the worst failure class (confident/silent/permanent/self-concealing, -DUPLICATE can't save a unique filename). 44MB clears the bar on RISK (not size) for the high-volume unique-serial supplier class the owner files. CONDITIONS: (C1 UNCHANGED, #1 ship-blocker) PP never enters `_field_candidates`/`_corrob_record_bucket` returns None/host-family — no second family can license `_corrob_licensed`; (TRIGGER REDESIGNED) gate PP to PP-vs-Tesseract DISAGREEMENT on a scanned ref-role OCR read, NOT the blind format flag — AGREE→do nothing (preserves current auto-files), DISAGREE→force hold (cap≤69+note+roleKeys), NEVER auto-file/overwrite-and-file; (neutral note) show PP as a suggestion, human reads the glyph; (no corrected_to/learning/trust-number/hold-lift); DARK+TEST_SWITCH_KEYS+byte-identical-off. GATE: 605+Demo+PrintTracker ON-vs-OFF, wouldFile(ON)⊆wouldFile(OFF) (provable), catch-set>0 incl. p7+p11, false-hold rate published, agree-but-wrong≈0 sampled, C1 pin (must fail on naive design), determinism two-run byte-identical release-gated on vendor/python, trade-lock pins (disagree+right HELD not filed; disagree+wrong still HELD).

- 2026-09-23 (arithmetic-witness single-digit total misread flag, the #464 class — gary design; recorded post-hoc from HANDOVER_2026-09-23_EVENING.md): **SIGN OFF WITH CONDITIONS.** Premise verified at source: `net_misread_total_flag` (ON since mig 81) needs total≈subtotal and provably cannot catch a correct-magnitude gross (#464 Nordwind £2,363.76 read £2,368.76, balancing only through the 2% tolerance) — the sole wrong-value money auto-file in 1,076 corpus docs. Six conditions, ALL applied in the build (mig 209 `recon_singlechar_misread_flag`, DARK): neutral SYMMETRIC note (components can be the misread ones — never asserts the total is wrong); a DISTINCT mark pinned ABSENT from the class-F doubt-clear set; require BOTH subtotal AND tax present; credit-note abstention identical to net-misread; the one-note-per-field guard; env `== '1'`. Predicate `validator.arith_witness_misread_total` reuses the pinned `suffix_reconcile.digit_substitution_diff`; ≥10p apart (rounding-proof). FLIP GATE: the #464-exact unit test must assert `isAutoFileEligible` flips eligible→flagged WITH `auto_file_threshold` set so conf-90 ≥ floor (the universal blocker is trust.js:1311, not :1064 — else the test proves nothing); OFF-arm md5 identity; realdoc M=0; a false-hold census (multi-line/multi-rate VAT, uncaptured line, misread-COMPONENT) — narrow min_delta to ≥£1 if real docs false-hold in [10p,£1); an owner live reprocess of #464.

- 2026-09-23 (PP-OCR second reader — the paddle-RESOLVER re-ruling on the dual-reader census; recorded post-hoc from HANDOVER_2026-09-23_EVENING.md + memory): the coordinator ran a DB-free label-anchored dual-reader census (TESTING/_measure/dual_reader_census/, results Desktop\TEMPTEST_dual_reader\): 1,474 scanned docs, 3,718 ref/date/total fields, Tesseract vs PP-OCR — **97.4% agree, 0 common-mode in the 120 hardest confusable-heavy confident-agrees** (pixel-adjudicated), the 98 disagreements Tesseract-wrong/Paddle-right incl. 3 silent-wrong totals 5→9. Oracle verified the census at source (crop pad = a small fixed px quiet zone, value-only; the earlier 35%-of-width pad had bled the label in and manufactured false disagreements). RULING: **DOWNGRADE (paddle-AGREE → re-word the confusable soften note to confident copy, KEEP the hold, auto-file byte-identical): SIGN OFF, ship DARK (mig 210 `glyph_confusable_resolve`).** D1 note non-empty; D2 keep the REF_ADVISORY (soften) mark, never the ABSENT mark; D3 key on the UNIQUE phrase "look like another on a scan" (NOT the shared `_FILING_SANITY_SOFTEN_MARK`, which the reinstate note also carries) as the SOLE note; D4 hard dep glyph_fallback_enabled; D5 prerequisite — confirm the paddle AGREE branch actually FIRES on real anchor reads (the `no_box` geometry question). **RELEASE (paddle-AGREE → CLEAR the note → auto-file): the 09-22 SEND-BACK is RETIRED → approved in principle, DARK, flip-gated on ONE more census** — the FILTERED `_absent` population (ref crop value absent from the whole-page text as a whole token + a one-glyph confusable page form exists = the soften-firing set, enriched for CLIPPED crops, the one place PP-on-the-same-clipped-crop could share a loss): pixel-adjudicate crop-vs-page form; among crop-WRONG cases count how many PP ALSO got wrong; **0 → flip**. A synthetic same-pixel-mirror pin was REJECTED as vacuous. R1 hard dep on the disagreement-hold; R2 confident-fired PP only; R3 ref-role only; **R4 the veto-fallthrough seam — engine.py G1 (~:12731) re-adds a note AFTER the glyph hold on a fall-through doc; pin BOTH branches**; R5 producer discipline (`_confusable_soften` has no page witness of its own — PP is its only independent axis); R6 the pixel-adjudicated flip gate above. Separate positive lead: extend the DISAGREEMENT-hold to the TOTAL role (the 3 silent-wrong 5→9 totals). BUILT 2026-09-23 evening: the DOWNGRADE (mig 210, 12 pins; D5 confirmed live — 40/40 `glyph_check` AGREE on the owner's Print Tracker diag, 0 `no_box`); the filtered `_absent` census tool (`census_absent.py`, reuses the engine's own `_nearest_confusable_page_token` / `_one_digit_letter_confusable` + the product's `reconstruct_page_text`). RELEASE not built — awaits the census + a re-vet with R1-R6 restated verbatim.

- 2026-09-23 NIGHT (the PADDLE CORROBORATION ARC — 007 + oscar design assembled in `docs/designs/PADDLE_CORROBORATION_ARC_2026-09-23.md`, with the 727-doc second-reader measurement): **per slice — S0 geometry hygiene (built `a0c25f2`) SIGN OFF W/COND (INCOMPLETE: it fixed the frame, not the PAD PARITY — C1); S1 slice_integrity SIGN OFF W/COND / partly WRONG LAYER (9 of the 16 false holds are a reader-RECT mismatch fixable at the capture site, not a placement defect: Part A v1 = word-snap + row-band only, the ink sensor DETECT-only, C2); S-B1 weak-glyph index SIGN OFF; S2 wire the healed slice SIGN OFF W/COND (`unhealed` = abstain-all, NO note, NO cap in v1, C3); S3 read-set capture SEND BACK on one point (the `pp_line` family BREACHES C1 — verified: `_corrob_licensed` engine.py:1964-1972 and trust.js:687-689 count ANY agreeing family, so {crop} ∪ {pp_line} licenses; PP reads live in `self._glyph_readset` + trace only, C4); S-B3 totals hold SIGN OFF W/COND (C11); S4 Part C SEND BACK (five holes C5-C9); the 82-doc ledger-write route = RIGHT LAYER, own vet (C8); PP det model DO NOTHING.** PREMISE: "727 confirmed = GT" overstated — 51% machine confirms (371 scope_sweep + 119 auto), partly circular where the confirm came from the same read (partition by provenance, C9); "common-mode 0/358" is the wrong headline — 4 PP-agree-and-box-WRONG via the RESOLVE branch (`RFH0738865` vs confirmed `RFHO…`) → **4/362 on pipeline crops, one O/0 family**; the mig-207 hold does NOT catch its founding exhibit p7 (PP reads `RFH0` at 0.962 on the anchor rect) — PP's demonstrated value on this font is the S/5 class (3 true catches `HS71Y07217`→`H571…` 0.999 via RESOLVE), the sans O/0 class is PP-BLIND (4/4 wrong on `W2E8X06407`) → reachable only by page-family disagreement or history; **the 5 "clipped taught boxes" are a PAD-PARITY bug (verified: `anchor._crop_and_ocr` reads the box ±20 px, anchor.py:3975-3976; the hold handed PP ±0.15·h ≈ 4 px)** — and `_grow_code_left_read` returns (surface, conf) only, so a converged grown winner's geometry is the pre-grow box (007's H is FACT); vertical bleed = a rec-only-model effect (no line segmentation) → a line-tight rect, not a grow rule. SEAMS: (1) `pp_line` × `_corrob_licensed` (above, ship-blocker); (2) any new suppression list for the 82 must be refused by BOTH licence twins like `suppressed_taught_role` (engine:1961, trust.js:686) BEFORE the census; (3) "box+page AGREE → PP dissent trace-only" must mean a POSITIVE page-family `agree` entry — page-SILENT + PP ≥0.90 on a clean slice → HOLD, else the no-page-witness class the hold was built for is thrown away (C5); (4) `+corrected`/`+snapped`/`+confirmed_adopt` winners (the 5 RFH0 docs: Stage 2.5b `ocr_corrector.correct_extraction` rewrote the string) — comparing PP to a history-rewritten string is PP-vs-history; abstain or compare to the retained raw read (C6); (5) Part A heal × the left-grow/pad-window rect — capture the grown rect (C1) or the clip survives. RULINGS: Q6 the tie-breaker re-scope is correct and loses nothing measurable — with C5; Q7 H must exist before any clear on a serial with a `_CONFUSE_TO_DIGIT` position (human-confirm `value_counts` dominant ≥0.90 + the count-gated shape), trace the 5 RFH0 docs' 2.5b change first; Q8 trace-only; Q9 mandatory re-census with the 0.90 floor FIXED and labelled in-sample. CONDITIONS C1-C14 (C1 pad parity at capture; C2 Part A v1 scope; C3 no note/cap on unhealed; C4 PP never in the record; C5 positive page agree; C6 abstain on rewritten winners; C7 H from human confirms only, tie-break only, never against a taught+page-contained read, no H on a scope with a rival confusable confirm — pin the RFH0 4:1 case; C8 the 82 own vet + both-twin refusal pinned first; C9 restate 4/362 + provenance partition + in-sample floor; C10 mig-207 flip gate = the 727 re-census after C1+casefold+floor: false holds ≤1 on a contact sheet, would-file lost ≤1 adjudicated, the 3 HS71Y catches kept, the 5 clip + 4 bleed docs read right, wouldFile(ON)⊆OFF, M=0, OFF md5, two ON runs identical; C11 totals pins incl. `E2,363.76` OOV strip + two-amount abstain; C12 R2 hold+note, no stored-row write; C13 rewrite the contradictory §5 sentence; C14 Part A abstains on `_line_cache is None`). CX: demoted PP lone dissents must still be COUNTED (`glyph_lone_dissent`); the flip headline = the 6 filing docs the hold falsely held, recovered. **BUILT the same night after the vet:** C1 pad parity + the C10 PP floor 0.90 + the Seam-1 mechanism pin (`e28a9d6`) · every hold exit traced (`e08c7db`) · **S1 `extraction/slice_integrity.py` (mig 212 `glyph_slice_integrity`, `005b22b` → `feaa1a4` snap the BARE box first + C6 on the DOWNGRADE → `625ccc0` the value-text filter)**. **C10 RESULT (`RESULT_C10.md`):** the C1 parity rect alone turned the clips into 105 silent length abstains (PP read neighbouring ink — found only because every exit was traced); S1 v3 on the 727: false holds **1** (serif O/0 `SO-99174` 0.918) · would-file lost **1** · the 3 `HS71Y` catches kept · agree 419 · length abstains 30 · `rewritten_winner` 32 (all five `RFH0738865` docs — the identity-limit family no longer agrees or rewords) · the 5 clips 4 agree + 1 safe abstain, the 4 bleeds 3 agree + the 1 hold · Print Tracker `W2E8X06407` sans 0/O agrees at 0.999 on the word-tight rect · true common-mode **0** (the 18 harness rows are the Castellan `CJB-` vs `JB-` GT artefact). **Bar MET**; owed: OFF md5 identity + two ON runs byte-identical. Recommendation: migs 207 + 212 (+210) flip-ready on this evidence; mig 211 stays DARK.

- 2026-09-23 EVENING (confusable RELEASE `glyph_confusable_release` mig 211 — gary design → Oracle re-vet with the filtered census + `docs/designs/CONFUSABLE_RELEASE_SEAMS_2026-09-23.md`): **SIGN OFF WITH CONDITIONS for a DARK build; two FLIP blockers, one of which can collapse it to DO NOTHING.** PREMISE CHECK: **P1 "the soften note is the ONLY blocker" is FALSE as a class claim** — trust.js `_pageFamilyDisagrees` (:659-671) refuses a role field whose record has a page-family entry in `disagree ∪ discounted` (sub-100 :1073-1075 `trust_role_disagreement_refuse`; at-100 `role_disagree_refuse_at100`, both ON), and `text_normalise.normalise_for_tokens` has NO O/0 fold, so wherever keyword.py captured the `_near` form the record carries `disagree:[{keyword,'S0-…'}]`; only the mig-191 suppression (`_suppress_taught_ref_disagree_record`, applied :12959-12981) lifts it, and only for an AUTHORITATIVE Stage-0.5 winner on a scope WITH a learned shape. Yield = {taught winner + learned shape} ∪ {keyword captured nothing} — UNMEASURED by the DB-free census. P2: on a doc trust.js holds anyway, a RELEASE swaps the DOWNGRADE's confident copy for the bare "reads it two ways" gate reason (worse CX, zero gain). P3: R2 "confident-fired PP" is NOT met by a 0.95 MEAN (one glyph near 0.6 hides in an 8-glyph mean) — per-glyph min moves to slice 1. P4: census geometry (word boxes) ≠ pipeline geometry (taught/relocated boxes, mapped-template docs only — F3 `_s05_pages`); accept for slice 1, the realdoc run is the bridge. SEAMS: S1 RELEASE×G1 (re-note on fall-through, fine, pin both); S2 RELEASE×boost×fc → the gate-free 100 path (pop → fc −12 gone → boost → clamps 100 → trust.js at-100 regime whose only page-family belt is `role_disagree_refuse_at100`) — a NAMED precondition; S3 an in-engine mirror of the disagreement check at :12198 runs BEFORE the :12959 suppression → must apply the mig-191 predicate itself; S4 poison amplification closed today by `autofile_gate_unify` + `learning_exclude_machine_confirms` (machine files never feed `value_counts`; learning.js:1587 names the hole with gate-unify OFF); S5 A4 geometry: `page_idx` from the ref's MAPPING row else 0 → an anchor-won ref on a multi-page doc without a ref mapping crops PAGE 0 → clip evidence misattributed. ANOMALIES: bare containment admits a glued neighbouring digit (`…2SO82482`); `_crop_padded` clamps silently at the page edge (wide read == narrow read → false containment); cold-scope trailing-letter blind spot (`WS-1493S`→`WS-14935` by both on a clipped S) = the named residual; gary's gate `wouldFile(ON) − wouldFile(OFF) == released` would FAIL on a non-bug → use ⊆ + a logged reason per released-not-filed doc. RULINGS: Q1 confusable-soften ONLY; Q2 containment YES but BOUNDARY-GUARDED (`_page_presence_corroborated` on the RAW wide read); Q3 YES re-invoke `_flag_ref_confusable_ambiguous` INSIDE the release path (never by reordering); Q4 keep `anchor_inline`; Q5 accept F3; Q6 accept A4 suppress-only + a page-KNOWN condition; Q7 NO — per-glyph min is slice 1. CONDITIONS: **C0 (FLIP blocker; run first if cheap)** on the realdoc run count, among soften-firing docs, (i) page-family disagree AFTER suppression / (ii) taught+on-shape / (iii) neither; yield = (ii)+(iii); **yield ≈ 0 → DO NOTHING, retire the key** (re-scope to a confusable-aware `disagree` fold, own vet). **C1** `_ctc_decode` → `(text, mean, min_glyph)`; release needs mean ≥ 0.95 AND min ≥ T, T set from a census re-run over the on-disk slices (publish the 14/4/clip mins). **C2** in-engine page-family parity: build the record via `_build_corroboration_emit`, apply the mig-191 predicate itself, abstain `page_family_disagrees` on any remaining page family; parity pin vs `_pageFamilyDisagrees`. **C3** containment = `_page_presence_corroborated(committed, ppw_raw)`; pins `…2SO82482` held / `PO No SO-82482` released / `5O80228` held / None held. **C4** wide crop clamped by the page bounds → abstain `page_edge`. **C5** re-invoke the ambiguous flag after the pop. **C6** release only when the crop page is KNOWN (ref's own mapping row or a 1-page doc); trace `geom_src`, `page_idx`. **C7** `_soften_meta` written only on `_note()==True`, per-run reset; release needs `('confusable', near)` AND `_one_digit_letter_confusable(committed, near)`. **C8** named preconditions in the switch copy (`trust_role_disagreement_refuse` + `role_disagree_refuse_at100`; `autofile_gate_unify` + `learning_exclude_machine_confirms`, ref `test_machine_confirm_learning.js`). **C9** trace every abstain reason; released dict keys == {value, confidence, method}; no `_field_candidates` write. **C10** gate: `wouldFile(ON) − wouldFile(OFF) ⊆ released`; every released-but-unfiled doc has a logged reason; every NEW file pixel-adjudicated; classify (a) already ≥ floor / (b) fc lift / (c) boost-only; M=0; OFF md5 identity with FALLBACK+RESOLVE=1; two ON runs byte-identical on vendor/python. **C11** R4 pins: unit `_fallthrough_critical_corroborated(released, [keyword S0-…], page, False) is False`; extract()-level both branches; source-order pin. Gate to merge (DARK): pins + run-pins green + OFF byte-identical. Gate to FLIP: C0 yield > 0 AND C10 AND the abstain histogram AND T set — back to the Oracle with numbers. **C0 TARGETED RESULT (same evening, owner's live-DB copy, 5 census docs confirmed there):** the soften fired on 1/5 (#45 Copperfield `PO-22954`, the Chris exhibit) → its keyword `P0-22954` sits in `suppressed_taught_role` (mig 191) → trust.js would NOT hold it; reason `below-floor` at 81 = the note's own −12 fc penalty → **category (ii), yield 1/1**; the 3 Vellum `SO-` docs carry NO note today (history-backed paths already resolved them, suppressed `S0-` competitors); #504 Pelican keyword winner clean. **C0 FULL RESULT (same night, 727 confirmed docs, `TESTING/_measure/release_c0_20260923/RESULT.md`): 9 soften-noted refs → (i) 8 still page-family-held — ALL Print Tracker crop-WRONG/page-RIGHT (`RFH0738865` for printed `RFHO738865` ×5, `HS71Y07217` for `H571Y07217` ×3) → the note is a TRUE positive there and the RELEASE can never fire (PP disagrees; C2 abstains) · (ii) 1 = #45 · (iii) 0 → YIELD 1 of 9 (1 of 727). Built DARK (mig 211, 19+1 pins, `3fd83ab`); recommendation: do NOT flip on this evidence alone — the DOWNGRADE already fixes the copy on all 9; the P1 premise catch was load-bearing. Broader: `disagreeing-read` holds 82/727 on reprocess — the page-family gate, not the soften, is the dominant Print Tracker hold (separate arc: a confusable-aware `disagree` fold).**


**2026-09-24 — Chris teach-a-doc round (reggie R1 · gary G1/G2/G3 · eric E1/E2/E3; full designs + conditions in `docs/designs/CHRIS_ROUND_FIXES_2026-09-24.md`).** R1 SIGN-OFF-W/COND C1-C4 — bug fix in the mig-70 parent, env kill only, no mig; **premise catch:** "⊆" was false (the helper runs on the challenger too) and R1-as-designed removed the only hold on a code-led clipped total (currency has no JS corroboration consumer) → exemption moved to the call site and gated on `money_cents` agreement with the challenger. G1 SIGN-OFF-W/COND C5-C8 (DARK mig 213; rename — collides with `keyword.value_is_caption`; sentinel reuse verified clean at `_crosscheck_witness_bucket`). G2 CENSUS FIRST (no code). G3 SIGN-OFF-W/COND C9-C10 (15 dead-pin files, not 5). E1 C11 (in-flight no-op). E2 C12 (lockstep bump; consented File-N state). E3 C13 (boolean + single-doc only; presentation, not audit). DO NOTHING: `_CHROME`+='customer', code edit-distance repair. Order: E1a → G3 → E3 → E2 → E1b → R1 → G1 → G2 census. NOTHING BUILT — owner's go pending.

### 2026-09-24 — taught_name_disagree_refuse (gary Slice 2, mig 214) — SIGN OFF W/COND (build DARK) — BUILT the same afternoon
Census 727: 24 catch / 1 false (Fembank) with ≥2-token witness guard → accepted; `OR ≥N chars` rejected (re-admits the label class).
Premise fixes: trailing punct already folded by _cmp_norm _EDGE_RE (internal punct is the real disagreement); template_fixed = memory family (engine.py:2032, docstring :5504 stale); required===0 excludes REQUIRED non-role names, not roles.
C1 has(k)+Number(required); C2 names only, no address branch; C3 no JS fold, Python pin + alnum-equal count; C4 keep roleDisagreeOnly name, pin OFF-parity; C5 action cue in copy; C6 pins Siemens/shadow/&-vs-and; C7 FLIP gate: (supplier,field) flood table, ≥3 confirmed-as-is = NO-GO.
Gate: JS-twin census SET ⊆ Python-30 = 25, buyer_issued split; realdoc+HardSet+sandbox OFF/ON md5-identical, M=0, wouldFile(ON)⊆OFF, deps asserted 'true' in the copy (at-100 leg has no env override, trust.js:664).
**BUILD RECORD:** `trust._disagreeRefusesRow` at both sites, mig 214 seed OFF, count pins 18, Review copy cue, Python C3 pins; JS-twin census = 24 holds on the 727 (the 25 minus #229 — JS nameQuality lacks COMMON_WORDS, "Construction" 4-consonant run → a missed catch, pinned), 6/6 on Chris's sandbox, lone-word witnesses 0, no flood group. Gate (b) owed before any flip.

**2026-09-24 — R1 C3 gate READ (`TESTING/_measure/r1_code_agree_gate_20260924/RESULT.md`):** on the owner's 727, K (env kill) md5 == P (pre-R1 tree) — byte-identical; N (ON) vs K: yield fires 1→1, 0 removed / 0 added, 0 `format_fail_code_agree` events, wouldFile 517→517, M=0 — the exemption is INERT on this data (no taught money box captures an ISO code); efficacy rests on the Chris-36 quick gate (OFF 0 file / ON 22 file, 14 held with real reasons). Hard Set P: wrong+would-file 0 both renditions; PRE-EXISTING observation — digital cold SUPPLIER 0 % (EMPTY, held) in 9/10 classes vs scan 60-100 % — logged for the owner, not an R1 effect. R1 stays LIVE.
...
## 2026-09-25 — two Review-window designs (owner hit both live): (B) reconcile the four status counts · (A) "suggested teach" auto-draw. Consensus: 007+barry+eric. VERDICTS: B = SIGN OFF WITH CONDITIONS · A = SEND BACK (re-scope; the picker is right-layer, the blind single pre-fill is not)
- IDEA B premise HOLDS (verified): quietLane.js:510 job.total recomputes to the STARTING population each chunk; doc_done emits total:job.total (:615) + renderer keeps j.total=ev.total||j.total (renderer.js:10509) → the "18" tracks the frozen population, never the queue; _renderAutofileCheckBar shows done of total (:10502) on its OWN element #autofile-check-bar; issuer_fill cumulative (reviewService.js:764); prior-collision comment real (:2261). Deleting the denominator is RIGHT layer — the quiet population (_candidates, sender's eligible held docs minus viewers) is legitimately wider than the queue.
- B SEAM (most doubted): the in-view countdown creates its OWN #inview-countdown (renderer.js:10591), hides #reprocess-autofile-bar (:7783), runs on _inviewCd and FILES ON EXPIRY with no click (:10608). CONDITION B1 (ship-blocker): reducer must not write #reprocess-autofile-bar while _inviewCd is live. B2: phase-suppression scoped to the same sender + defer/replay a suppressed offer, never drop (unrelated reprocess offers must not be delayed). B3: the labeled countdown starts from the possibly-wrong 18 → ship barry's NO-denominator indicator first, or verify the start scope by live reproduce. B4: fold the three auto-hide timers into one (clear on doc-change + unload). GATE (renderer-only): event-interleaving harness (3 IPC streams adversarial orders + mid-scan confirm + active countdown) asserting one live element, countdown never hidden by a reducer re-render, no silent file, job_deferred through the one writer + a PIN reproducing the ready-vs-checking clash that FAILS on today's code.
- IDEA A CORRECTED premise verified: _field_read_geom populated only from _winning_read_geom (anchor box/taught_box; engine.py:6903-6928,:11559-11569), EMPTY for keyword reads, read only by DARK glyph arms, trace-only at :5703 → the untaught-sender case has NO box, must be reconstructed. (Minor: the ":2640 _measure_candidates" cite is actually val_census; the winner-only extract_fields claim holds.)
- A SEND BACK: Slice 1 internally contradictory — 007's "located-only" gate fires only where the field is ALREADY taught (inert on the untaught case it targets); the only path that fires is eric's reconstruction, which 007's gate won't bless (mis-locates on repeated tokens/totals). CLAUDE.md invariant: a rubber-stamped mis-placed pre-fill becomes a HIGH-AUTHORITY all-supplier anchor (shape_mode='ignore') → poisons the scope; blind single pre-fill removes placement attention while keeping full authority = WRONG LAYER for safety. Owner's real prize = the multi-label PICKER (Slice 2, candidate retention + per-candidate boxes) — right layer, preserves attention. RE-SCOPE: drop the blind pre-fill; build the reconstruction-geometry CENSUS first (population = keyword-read fields on the untaught-sender corpus; "wrong box" = reconstructed box whose live-frame re-read fails to reproduce the committed value or overlaps another field; bar = measurably SAFER than hand-draw); then the picker with 007 re-read-verify, "confirm the VALUE" copy, host surface decided first, authority never exceeding a hand-draw. Biggest risk A: a mis-located rubber-stamped box poisons every future doc across all suppliers.
- 2026-09-25 — Suggested-teach PICKER (Slice 1) + staging split — SIGN OFF WITH CONDITIONS. Split CORRECT (Slice-1 renderer-only covers 100% of census MULTIPLE = same-value repeats; reggie engine change deferred behind its own census). SEAM CAUGHT: the picker path (openResolveOverlay/resolveCandidatePick) stages display-frame boxes as raw with NO deskew back-transform + draws markers on raw pageImages while boxes are display-frame → silent mis-registered authoritative anchor on deskewed statements (the modal MULTIPLE case). Conditions C1-C7 (C1 frame back-transform on picker path; C3 frame-guard; C4 tight fresh RAW re-read for verify-a; C5 position-only no-value-mutate/no-server-flag-clear; C6 supplier-scope single writer; C7 glyph degrade). Gate before FLIP: byte-identical OFF + false-suggestion IoU census incl. deskew-active statements (must FAIL on the seam if unfixed) + M=0/no accuracy drop on confirmed-supplier siblings. Premise fixes: saveAnchor sweep IS supplier-scoped (CLAUDE.md AN stale); onnxruntime now vendored so glyph_reader generally available.
- 2026-09-25 — Suggested-teach PICKER Slice 1 BUILT DARK (mig 220 `suggested_teach_enabled`; main session, not the Oracle): shared `suggestTeach.js` reducer (unique/multiple/none over ValueLocate) + the renderer hook `suggestTeachBoxes` on enterZoneMode. C1 (deskewFinalizeAnchor back-transform display→raw before openResolveOverlay), C3 (drop on frame change), C4 (fresh RAW tight-crop re-read via getRawPageBase64→ocrRegion, rejects a crop reading >1.6× the value = half-box offset), C5 (UNIQUE stages via captureAnchorContext — never writes the value / clears the flag), C6 (same saveFieldAnchor writer, supplier-scoped) all wired + pinned (test_suggest_teach.js WIRING + test_migration220). ⚠ C7 (glyph second reader) DEFERRED: the renderer has no glyph IPC; the auto-staged UNIQUE path is protected by the tight fresh-RAW re-read + the human Confirm (persist-on-Confirm, box visible), and merged-column ambiguity surfaces as MULTIPLE→picker (human picks off raw markers), so no SILENT wrong-anchor path — but the false-suggestion IoU census before FLIP must include a merged-column/statement class, and glyph is the natural Slice-1b hardening. Byte-identical OFF (renderer gate + no Python touched). Still DARK; flip gate owed.

## 2026-09-25 — Suggested-teach RE-HOSTED to the guided Teach wizard (mig 220), eric DROPS Review C4 — SIGN OFF WITH CONDITIONS (C-A reveal + C-B race guards; flip gate below).
- C4 drop SAFE on the wizard: seen==committed VERIFIED (store():1785→_teachBackBox:662 shared with hand-draw; showValueConfirm draws the STORED box; doCommit:2249 commits it). No frame half-box possible (no new coord path). canAdvance:135-137 BLOCKS advancing while any field is pending → no rubber-stamp/bulk-confirm; per-field showValueConfirm+rb-yes is mandatory. valueLocate is strict-exact word-geometry → wrong value misses or is user-correctable. Authority=Stage-0.5 saveTemplateMapping = hand-draw, NARROWER than the Review ⊕ all-supplier anchor.
- C-A (ship-blocker): UNIQUE auto-draw must tzReset+emphasiseBox+scrollIntoView before showValueConfirm (:1584 only scrollIntoView) — else "Looks right" approves the value text without the box in view (weak same-value corroboration). Reuse showLocatedPick's reveal.
- C-B (ship-blocker): build+pin the async-race guards (suggestOffered marked pre-await; bail if results[key]/drag/doc|page|angle/curField changed — mirror Review :5803-5808) so a late suggest can't clobber the user's own box.
- Issuer EXCLUDED = correct (identity blast + direct-commit path :1288). Import-value seed: no safety seam; efficacy note prefer raw_value as locate target. MULTIPLE=showLocatedPick, never auto-picks (:1267). /v1 clean no-op (getDocumentWithExtractions core-only; absent in coreTeachTransport.js today).
- FLIP GATE: byte-identical OFF + suggestion-fidelity IoU census (incl. deskew-active statements + wrong-value-caption class; must FAIL on a mis-registered/cross-field box; bar >= hand-draw, zero cross-field bleed) + C-A reveal pin + C-B race pin + a canAdvance-blocks-pending pin. Human checkpoint = canAdvance (mandatory per-field terminal decision).
- BUILT the same day (main session, wizard re-host): the ⊕-Review Slice 1 wiring + the Review-only SuggestTeach reducer REMOVED; the setting `suggested_teach_enabled` (mig 220) REPURPOSED to the wizard. Reuse of the wizard's shipped locateTypedValue/showLocatedPick/useLocatedBox (Oracle-signed typed-locate machinery); new = maybeSuggestField + import-value fetch + the transport pass-through. C-A + C-B built + pinned.

## 2026-09-26 — FLIP mig 220 `suggested_teach_enabled` (DARK → default-ON): reframe of the A2 IoU gate — SIGN OFF WITH CONDITIONS.
- Ruling on the owner's reframe: the IoU-vs-GT-box census I named in the flip gate is (a) UNCOMPUTABLE — stress_test/corpus/ground_truth.json is values-only, no box coords — and (b) mis-scoped onto Slice 1, which NEVER silently places. Substitute ACCEPTED for Slice 1: uniqueness-by-construction (ValueLocate string-match on the page's own words) + the mandatory per-field human confirm (canAdvance case-3 blocks any pending field; every status→non-pending transition is a human click). Traced store(pending=true)@teach.js:1944 → canAdvance:141 → rb-yes:1884 / doTyped:1824 / etc.; doCommit reachable only past the gate. No silent-placement path exists.
- Premise verified at source: suggested_teach_enabled is ABSENT from all python_backend/ (pure JS/DB gate) → flipping it cannot change any read → realdoc M=0 sound on the mechanism. Byte-identical OFF pinned (maybeSuggestField early-return, test_teach_suggest.js:39). Reconstruction census (teach_reconstruction_census_20260925/RESULT.md): ref 77% / date 87% UNIQUE, NONE=0 clean, MULTIPLE 13-23% entirely statements + repeat-ref → the picker's whole purpose; covers deskew-active statements. Deskew round-trip = shipped _teachBackBox (no new coord path).
- SEAM / the find: doCommit's mapping loops (teach.js:2400 desktop, ~2338 batch) do NOT exclude status==='pending' — safe only because canAdvance is airtight. Reframe stakes ALL safety on that single interlock, so defense-in-depth is owed.
- CONDITIONS before flip: C1 (ship-blocker) doCommit asserts-and-refuses on any pending field (fail-toward-review), above the batch/desktop branch so both paths covered. C2 add a pin that goes RED if C1 or the canAdvance-pending block is removed. C3 re-assign the full IoU/cross-field-bleed/silent-placement/bar≥hand-draw census to the FUTURE MUSTER (auto-places unique boxes with no per-field confirm → owes it in full before its own flip); record in DARK_SWITCH_LEDGER mig 220.
- NOT required for the flip: a broadened scan-corpus census — the scan tail degrades to NONE→manual (safe); it's an efficacy measure, not a safety gate. Watch-item: the 'read' path's "Looks right →" invites a rubber-stamp on a coincidentally-unique WRONG read — mitigated (not silent) by the "Check what I read for" heading + value-correction row + drawn/ringed box.
- BUILT the flip the same day (main session): C1 doCommit pending-refusal + C2 pin in test_teach_suggest.js; @DEFAULT_FLIP 221 (UPSERT 'true') + DELISTED from dark_switches.js (TEST_SWITCH_KEYS 18→17); pin database/test_default_flip_221.js; C3 recorded in the ledger. run-pins green.

---
## 2026-09-26 — Teach-locate page-words SPEEDUP (oscar diagnosis → Oracle) — SIGN OFF WITH CONDITIONS
**Ask:** the guided-teach auto-draw's first box takes ~7-9s; measured the chain render 1.5s → straighten 1.6-2.5s → **page-words 4.3-4.7s** (serial). page-words = two full-page passes (PSM-3 + PSM-6) on the 288-DPI teach canvas, called dpi=None (Tesseract guesses ~70).
**Verdict: SIGN OFF W/COND, DECOUPLE into two commits.** (1) parallel passes = ship free; (2) downscale+told-dpi = ship behind kill switch + gate; (5) import-geometry reuse = pendingfeatures (right eventual target, not a detour).
- **C1** split: `DS_OCR_PARALLEL_FULLPAGE='1'` on both page-words spawns (byte-identical merge) as its own commit — a gate-trip on the downscale can't cost the free parallel win. **BUILT `ddb5605`**, pin `test_page_words_parallel.js`.
- **C2** kill the within-region.py seam BY CONSTRUCTION: REASSIGN `img = img.resize(...)` so the emitted {w,h} and the OCR read the SAME frame (locate normalises by w/h → no silent box shift). No `img_small`. Placement-invariant pinned.
- **C3** told-dpi tracks the real frame; DPI-AWARE target clamp(importDpi, 150, 288) — a 200-install reads at 200 (locate-recall == import-recall), a 300/unset install reads native 288 but now TELLS 288 (vs guess-70), no recall trade.
- **C4** kill switch `TEACH_PAGE_WORDS_DOWNSCALE=0` restores native (288, dpi=None) byte-for-byte; pinned.
- **SHIP GATE (proportionate — UX-convenience, locate never commits; miss → manual draw):** locate-recall A/B on 30 corpus docs spanning ref/date/supplier. **PASS** (`TESTING/_measure/teach_pagewords_recall_20260926/RESULT.md`): recall 90/90 OLD==NEW (0 regressions), 0 false-locate on wrong text, median IoU 0.975, ~1.5× on light docs (more on heavy scans). The one unique IoU-0 (doc2058 "PI/26/7656") verified BENIGN — both boxes on the real value text at two printed instances of the ref (valueLocate.js:118 only returns runs equal to the value → a moved box is a different TRUE occurrence, never wrong text; human Accept/Redraw picks the instance).
- **Premise corrections oscar caught (both strengthen the case):** page-words is TWO passes not one; dpi was NEVER told (guess-70) → the fix is a recall improvement, not just speed.

**2026-09-26 PM follow-up (real-scan regression → default OFF):** the downscale (lever 2) broke the auto-draw on the owner's live GRAINY SCAN — lower DPI misreads the value's digits/slashes (a date read fine at 288, garbled at 200) so the locate string-match FAILS → no box. Word COUNT was ~unchanged (107 vs 109), so the corpus recall gate (cleaner docs) missed it; the loss is per-VALUE legibility. **Downscale is now OPT-IN (TEACH_PAGE_WORDS_DOWNSCALE=1), default OFF**; the parallel passes (lever 1) stay on — a safe ~2×. Re-enable needs a real-SCAN legibility gate (does the located value TEXT still equal the import value at the lower DPI), not just word recall.
