...
**S1 BUILT 2026-09-14 (the client teach pop-out; full pin gate 366/366; core + `--client` harness both green).**
Read routes over `/v1` (contract 1.7.0): `ocr-region-boxes` / `ocr-page-words` (own OCR-flood cap) /
`page-deskew` (Oracle C3 — reuses region.py `--deskew` `expand=False`, so the straightened frame is identical) /
`teach/config`; new pin `test_v1_teach.js`. `teach.css` extracted from the wizard so both apps link one
stylesheet; `scripts/sync-client-teach.js` + `test_client_teach_sync.js` mirror `teach-ui` + the 9 shared
sub-scripts to committed client copies. Shared wizard gained `window.TeachState.docId` (so the injected
transport builds the doc-scoped OCR URLs) + `_cap()`/`_applyTeachCaps()` (client sets `caps.import=false` →
teach the queue + the empty state points at the main PC, C7; `caps.review=false` → the follow-up "Check in
Review" hides) — byte-identical on the core (caps absent = true). Client: `apiClient.teach.*` + the preload
bridge + `client/main.js teachWin` (a pop-out cloned from search — token in main, closed on logout /
main-close; the 401 report + close cover both pop-outs) + `client/renderer/teach/{index.html (generated by
scripts/gen-client-teach-index.js), clientTeachTransport.js, clientTeachHost.js}`. The `--client` harness
proves the pop-out boots + reads (queue / doc types / page / deskew) over the transport with zero errors.
**Deferred from S1 (do NOT ship the client teach as user-facing until these):** the admin-only entry gate + a
main-window button + the `teach_over_client_enabled` switch land with S3; SAVING (promote/mapping/confirm) is
S3, doc-type CREATE/EDIT over /v1 is S2 — those transport methods throw a clear "coming soon". KNOWN drift
risk: `client/renderer/teach/index.html` is GENERATED once (not synced/pinned), so a change to the core teach
