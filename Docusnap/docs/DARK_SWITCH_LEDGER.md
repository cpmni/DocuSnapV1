...
- **anchor_code_left_grow** (mig 192) — ✅ **GRADUATED to a customer default by mig 193 (2026-09-21, owner go).** The Stage-2 twin: on a taught-box ref disagreement it re-reads the clipped box with the same left-slack window; if that independently matches the full-page read it commits the correct value CLEAN (kills the needless "please verify" click AND lets it auto-file at its own confidence). Non-convergence falls to today's flag (fail-toward-review). Refs only. Gate met: safety census M=0 + the synthetic efficacy injection FIRES + adversarial ABSTAINS. Delisted from TEST_SWITCH_KEYS.
