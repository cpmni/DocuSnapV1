...
console.log('1. a truly fresh DB seeds the 90 bar + the OFF switches');
{
  ...
  check('ref_prefix_confusable_adopt seeded OFF', getSetting(db, 'ref_prefix_confusable_adopt') === 'false');
  check('raw_witness_vacuous_suppress seeded OFF', getSetting(db, 'raw_witness_vacuous_suppress') === 'false');
  db.close();
}
