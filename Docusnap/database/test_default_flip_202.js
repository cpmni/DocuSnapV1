'use strict';
/*
 * test_default_flip_202.js — migration 202: ref_confusable_history_disarm (seeded OFF by mig 201) GRADUATES
 * to a customer default (2026-09-22, owner go — "minimise messages").
 *
 *   The S/5-type ref-confusable nag now disarms on the sender's confirmed HEAD convention (length-agnostic),
 *   with the required rival-head poison guard (gary+reggie → Oracle SIGN-OFF-W/COND). Efficacy is proven by the
 *   7 targeted pins in python_backend/tests/test_ref_confusable_history_disarm.py (the synthetic corpus can't
 *   build a variable-length mixed-prefix repeat sender — the 700-corpus census was byte-identical, a SAFETY
 *   flip). HARD dep ref_confusable_flag ON (default since mig 173). Same flip-home contract as
 *   test_default_flip_191.js. Run: ELECTRON_RUN_AS_NODE=1 ./node_modules/electron/dist/electron.exe database/test_default_flip_202.js
 */
...
const key = 'ref_confusable_history_disarm', seed = 201, flip = 202;
