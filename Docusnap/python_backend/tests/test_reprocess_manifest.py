...
print("doc_overrides:")
# doc_overrides returns a 5-tuple: (enhance, known_template_id, known_doc_slug, cached_text,
# known_doc_slug_authority) — the authority slot is pinned by tests/test_reprocess_type_flip.py.
# No manifest -> globals (byte-identical to today)
e, k, s, ct, _ = doc_overrides({}, "a.pdf", enhance={"x": 1}, known_template_id=5, known_doc_slug="invoice")
...
e, k, s, ct, _ = doc_overrides(man, "a.pdf", enhance=None, known_template_id=None, known_doc_slug=None)
...
e, k, s, ct, _ = doc_overrides(man, "b.pdf", enhance=None, known_template_id=7, known_doc_slug="invoice")
...
e, k, s, ct, _ = doc_overrides(man2, "a.pdf", enhance={"g": 1}, known_template_id=None, known_doc_slug=None)
...
_, _, _, ct, _ = doc_overrides(man3, "a.pdf", cached_text="GLOBAL TEXT")
check("manifest ocr_text wins over global cached_text", ct == "PER DOC TEXT")
_, _, _, ct, _ = doc_overrides(man3, "other.pdf", cached_text="GLOBAL TEXT")
check("absent file -> global cached_text", ct == "GLOBAL TEXT")
_, _, _, ct, _ = doc_overrides({}, "a.pdf")
