...
tail-bound arc, Oracle C6). Pins 431 files green (`test_ref_class_fix.js` flaky under load, green alone). **TEST INSTALLER for the owner's fresh-DB
test on another PC: `dist\ScanFinder Setup 2.0.0-r20260925-0926-c7e0cc0-TEST.exe` (371 MB) = `HARDEN_JS=1 HARDEN_JS_STRINGS=1
TEST_BUILD=1 npm run build`; verifier OK (run it with `TEST_BUILD=1` or it refuses a test build); packaged second-reader
smoke passed; arms all 14 `TEST_SWITCH_KEYS` incl. mig 216 on first launch. ⚠ A build WITHOUT `HARDEN_JS=1` is REFUSED by
`scripts/verify-release-artifact.js` and renamed `*.REFUSED.exe` — the hardened recipe is the only shippable one.** Both
sandboxes were closed for the build (the owner's sandbox DB keeps Credit Note / Delivery Note / Statement installed).
**"Carry on" (late morning, `99297fd`):** "Set aside all" asks once · the redetect's DURABLE activity-strip receipt (kind
`recognised`; lane dep `recordEvent`) · `src/lib` added to `run-pins.js` (its `test_review_events.js` had been red unseen
since 09-07). **TWO DESIGNS Oracle-SIGNED, NOT BUILT, owner go/no-go:** card 5 sender split →
`docs/designs/SIBLING_DOMINANT_HOLD_2026-09-25.md` (Tier C converging-siblings hold, DARK `issuer_sibling_dominant_hold`,
C1 teach-ask + C2 confirmed-siblings BLOCKING) · Ironclad statements typed Invoice →
`docs/designs/TYPE_OWNER_UNINSTALLED_BLOCK_2026-09-25.md` (uninstalled shipped name = blocking owner, DARK
`type_owner_uninstalled_block`, C1 alias exclusion BLOCKING, Hard Set `statement_layout` = flip gate). **Design 2 BUILT DARK the same day (owner
"go"): mig 217 `type_owner_uninstalled_block` (`TEST_SWITCH_KEYS` 15; engine env `TYPE_OWNER_UNINSTALLED_BLOCK`, a child
of the fold; pins 36/36 + the fold pin's red-first twin + `test_migration217_*`; census `TESTING/_measure/type_owner_block_
20260925/RESULT.md`: Chris copy → exactly {46, 52}, the owner's 758 live texts → 0 changes installed + ablated, old-vs-new
engine identical with the block off). Flip gate owed: realdoc ON arm + Hard Set `statement_layout` class (C9).** **Design 1 BUILT DARK too (owner "go"):
mig 218 `issuer_sibling_dominant_hold` (`TEST_SWITCH_KEYS` 16; JS-only, env `ISSUER_SIBLING_DOMINANT_HOLD` explicit
'1'/'0'): `learning.findDominantSiblingIdentity` (fold-first, queued + human-confirmed converging siblings, pair budget)
at the confirm gate after a Tier A/B miss, in the teach ask on both roads (`docId`), and the Review grouping ("check
sender" chip); `/v1` review-confirm 400 carries `nearMatch` → contract **1.10.0** (lockstep: `client/apiClient.js` +
`test_v1_preview_reads.js`). Pins `test_near_match_siblings.js` / `test_reviewservice.js` Tier C / `test_migration218_*`;
census `TESTING/_measure/sibling_dominant_census_20260925/RESULT.md` (Chris copy exactly {62, 69}, replay 0; live copy
0/0). Flip owed: a live client confirm of doc 69 + a Chris round. Next mig = 220 (mig 219 = DARK `keyword_label_tail_bound`, Chris card 4, commit `8b17704`).** **Dev-inspector `7057b87`:** the SFDEV trace console now shows the STRAIGHTENED re-read (the retry's
second `engine.extract` was untraced → "no candidate" while the panel showed the adopted value); `pass: "straightened"`
events + `deskew_adopt` / `deskew_pass`; not in the `-c7e0cc0-TEST` installer.
...
SIGN-OFF-W/COND, 6 conditions applied) — the ARITHMETIC-WITNESS catch for **#464** (the ONLY wrong-value money
auto-file in 1,076 docs: `£2,363.76` read `2,368.76`; balances subtotal+tax only via the 2% tol + a single-digit
substitution ≥10p → FLAG+hold, never adopt; `validator.arith_witness_misread_total` +
`engine._flag_singlechar_reconcile_misread`; `test_arith_witness_misread.py` 24 pins). (3) SFDEV Review console shows
the paddle 2nd-reader row + agree trace. **DUAL-READER CENSUS (`TESTING/_measure/dual_reader_census/`, results
`Desktop\TEMPTEST_dual_reader\`):** 1,474 SCANNED docs, 3,718 ref/date/total fields, Tesseract vs PP-OCR — **97.4%
agree, 0 common-mode in the 120 hardest** confusable-heavy confident-agrees; the 98 disagreements are
Tesseract-wrong/Paddle-right (incl. 3 silent-wrong totals 5→9). This **flipped Oracle** on the paddle-resolver:
DOWNGRADE **SIGN OFF (ship)**, RELEASE **approved in principle**, DARK + flip-gated on a filtered `_absent`-population
census. **NEXT (owner "do BOTH"):** build the filtered `_absent` census (unlocks RELEASE) + the DOWNGRADE
`glyph_confusable_resolve` (mig 210). **⚠ CORRECTED by checking source (the new rule): `net_misread_total_flag` (ON
since mig 81) does NOT catch #464 (net-line case only); "reprocess never files" is WRONG — reprocess-ALL auto-files
via `consume-reprocess-completion`, single reprocess doesn't.** **Also: CLAUDE.md collapsed 1609→1200** (the
09-04→09-21 blocks moved to `docs/session-log.md` "2026-09-23 compaction" + one-liners) **+ a HARD RULE "VERIFY,
DON'T ASSUME"** added to Working-rules (owner ask: verify load-bearing facts at source; never trust a small sample or
an advisor/Oracle claim; bar scales with blast radius).

## ⏭ 2026-09-23 MORNING (PP-OCR "second reader" confusable hold built S0→S2 DARK + anchor-read coverage; security review; Chris vet):
...
- **09-21 EVENING** → `HANDOVER_2026-09-21_EVENING.md` — opt-in auto-split **mig 194** `auto_separate_enabled`
  ...
  DARK `issuer_undetected_blank`** (JS↔Python twin `issuer_read_looks_implausible`) + **Part B the Review "Not
  recognised" tab** (`shared/notRecognised.js`); all 6 Chris-vet fixes.
- **09-20** → `HANDOVER_2026-09-20.md` + `AUTO_SESSION_2026-09-20.md` — hardened RELEASE installer + the 20 new-user
  tutorial videos (plan + scripts in `tools/video_tutorials/scripts/`); NO app/engine code touched.
- **09-18** → `HANDOVER_2026-09-18.md` (+`_NIGHT.md`) — **DEPARTMENTS D4→D7**, opt-in `departments_enabled` DARK;
  **mig 184** replaces the scalars `documents.department_id` / `document_types.default_department_id` (**RETIRED —
  read the `document_departments` JOIN, never the scalar**) with join tables; perf PASS at 50k; NIGHT by-id
  serve/mutate hardening (`fc2443d`+`b29f64c`); `ref_badge_verify_state` **mig 185 DARK**. Chris security vet = watertight.
- **09-17 AFTERNOON** → `HANDOVER_2026-09-17.md` / `_C12_AND_SWITCHES.md` — **mig 180 `segment_pair_hold` DARK**
  (weak/strong cut pair-hold belt; owed C12 Rejoin + C9 cells); the `vinny-video` agent created.
- **09-16 DAY / 09-17** → `HANDOVER_2026-09-16_FLIPS.md` — separator-accuracy arc + the DARK-switch flip census;
  the switches **`name_role_nonname_flag` (mig 172) · `ref_confusable_flag` (173) · `template_pad_date_adopt` (174) ·
  `watch_separate_enabled` (175) · `split_segment_multipage_hold` (176) · `segment_continuation_veto` (177) ·
  `segment_title_slug` (178) · `segment_known_supplier_change` (179)** were graduated toward customer-default
  (state authoritative in `dark_switches.js`). **`deskew_corrob_autofile` stays OFF (owner's call).**
- **09-15/16 NIGHT** → `HANDOVER_2026-09-16_VIDEOS.md` — the tutorial-video tool `tools/video_tutorials/` + `/makescript`
  skill + 5 recorded tutorials in `Desktop\Tutorials`.
- **09-15 EVENING** → `HANDOVER_2026-09-15_EVENING.md` — Departments D2 + D-C11; date-year validator
  `database/modules/date_parse.js`; the core `client_api_host`/`port` LAN bug (dedicated protected-settings writer);
  client startup-crash fix; **mig 171**.
- **09-15 DAY** → `HANDOVER_2026-09-15_DAY.md` — **mig 169** enforces the two /v1 security fixes by default
  (`v1_force_password_change` + `backup_import_seat_only`); **mig 170** `teach_over_client_enabled` ON.
- **09-14 EVENING** → `HANDOVER_2026-09-14_EVENING.md` — DATE FORMS **mig 167 `date_forms_wide`** (month-name /
  ordinal dates accepted at every human door — confirm/Review/badge/Teach; engine half DARK under `DATE_FORMS_WIDE`).
- **09-13 EVENING / overnight** → `HANDOVER_2026-09-13_EVENING.md` / `_NIGHT.md` — **mig 166
  `template_date_invalid_yield_lowconf` DARK**; CLIENT Quick File (`/v1/documents/intake`, contract 1.2.0); the
  QuickFile+Departments FOUNDATION; flip-corpus censuses (migs 159/156/143 flip-ready). **⚠ census baseline is
  `RR_APP_ENV=1`; the separation switches' lever is SHELL ENV, not a DB write.**
- **09-12** → `HANDOVER_2026-09-12*.md` — the flip-test corpus PIPELINE; **mig 162 `deskew_retry_field_adopt` DARK ·
  mig 163 `deskew_false_absent_reflag` DARK**; the QuickFile+Departments PLAN; 2 arcs Oracle DO-NOTHING.
- **09-11** → `HANDOVER_2026-09-11.md` — the customer_name postcode-drift ROOT CAUSE **mig 157
  `template_drift_override_guard`** (+ migs 154-156, 158 DARK); LIVE-confirmed `CH1 2HU`→correct name. **⚠ live DB
  `%APPDATA%\ScanFinder\docusnap.db` is BLOCKED to Claude's script tools — copy `.db`+`-wal`+`-shm` to scratch to inspect.**
- **09-09 / 09-08** → `HANDOVER_2026-09-09.md` / `_2026-09-08*.md` — 09-08 night run; the **`--smoke-windows`**
  packaged-binary verifier layer; the FABRICATED-fingerprint backup-restore bug (Chris card 7); **mig 137 resets the
  DARK test switches** + `database/build_arming.js` (**`TEST_BUILD=1` = the one arming knob**) +
  `scripts/check-release-migrations.js` (release gate); SOURCE PROTECTION + MSIX/Store path; the pre-deployment AUDIT
  (`docs/PRE_DEPLOYMENT_AUDIT_2026-09-08_RERUN.md`).
- **09-07** → `HANDOVER_2026-09-07.md` — **PLACEMENT ROOT CAUSE**: `templates.sample_deskew_angle` NULL was read as
  0.00° AND the packaged build compiled `ocr/detect_angle.py` away → every taught box landed a glyph aside; fixed
  `13cfcf9`+`7c61317` (angle at commit + startup backfill). DARK migs 129-133; TEST force-ON via migs 134/136.
- **09-06 / 09-04** → `HANDOVER_2026-09-06.md` / `_2026-09-04_LATE.md` — the log-review fix plan (**mig 122** seeds 3
  DARK arcs OFF); **`confusion_precedence` mig 119 DARK · `format_class_join` mig 120 DARK · the reference-flag family
  migs 107-118** (all review-bound, DARK). ⚠ These were TEST force-ON in migs 123/124/126/134/136 for the customer
  test builds — the old "revert before a customer build" list is RETIRED (mig 137 + the KEY-based release gate replaced it).
...
**HARD RULE — VERIFY, DON'T ASSUME (owner rule, made explicit 2026-09-23; the app is now large enough that a
plausible-sounding but unchecked premise is the main way a wrong action gets taken).** Before you DECIDE on any
action, check the load-bearing facts AT THE SOURCE. Specifically:
- **Never treat something as correct because it came from a small or partial set of information** — one screenshot,
  one trace line, one log excerpt, one field, one document. A narrow observation is a lead to verify, never a
  conclusion. When a number or claim is surprising, that surprise is the signal to widen the check, not to file it
  as mild confirmation of what you already believed.
- **Never treat something as correct because an ADVISOR/agent said so** — not gary, oscar, reggie, eric, 007, iris,
  herald, Chris, and NOT the Oracle. They start cold, read a subset, and reason from it; they make premise mistakes
  (2026-09-23: the Oracle asserted "totals are overwhelmingly keyword reads → no located box" without checking the
  teach flow — a taught template DOES log a box for the total; the owner caught it in one line). Their job is to
  raise candidates and catch seams; YOURS is to verify the load-bearing claims in the code/DB/data before acting on
  them. When you brief an adversarial reviewer, FRONT-LOAD the facts it would otherwise have to rediscover, so it
  vets the design instead of guessing the premise.
- **The bar scales with the blast radius.** A cheap, reversible edit needs a light check; a migration, a switch
  flip, a customer build, a delete/overwrite, or a diagnosis a whole fix rests on needs the fact confirmed at the
  source (query the DB, read the code, list the table/dir/switch state) BEFORE you commit to it. "It's probably
  fine / an agent said so / the summary says so" is not verification. Current switch/migration state especially
  lives in `database/dark_switches.js` + `docs/DARK_SWITCH_LEDGER.md`, never in a prose summary that can go stale.
