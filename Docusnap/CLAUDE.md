...
**HEAD = `6f735bb` + the docs commit; origin CURRENT (all pushed).** **TEST INSTALLERS (11:39, on `6f735bb` = contract
1.6.0; supersede every earlier pair today):** core `dist\ScanFinder Setup 2.0.0-r20260914-1039-6f735bb-TEST.exe` (`npm run
build:test` — all 50 DARK keys arm, incl. Quick File + Departments + mig 166; packaged smoke-boot identity testBuild:true +
smoke-windows 14/14) + client `client\dist\ScanFinder Search Client Setup 1.0.2-r20260914-1043-6f735bb.exe`. **NOTHING RUNNING at wrap** — every repo Electron
