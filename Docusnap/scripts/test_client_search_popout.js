...
console.log('B. source — the Oracle S1 conditions');
{
  ...
  check('apiClient: CLIENT_CONTRACT 1.8.0 in lockstep; find carries a LONG idle timeout', /CLIENT_CONTRACT = '1\.8\.0'/.test(apiC) && /\/find\?\$\{q\}`, \{ withAuth: true, timeoutMs: 180000 \}/.test(apiC));
  ...
  check('server: API_CONTRACT_VERSION 1.8.0 (lockstep with the client)', /API_CONTRACT_VERSION = '1\.8\.0'/.test(srvH));
