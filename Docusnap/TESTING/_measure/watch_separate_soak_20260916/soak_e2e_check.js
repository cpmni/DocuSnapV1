...
const marks = new Set(db.prepare("SELECT DISTINCT document_id FROM extractions WHERE document_id > ? AND validation_note LIKE '%were cut from a multi-document scan%'").all(minId).map(r => r.document_id));
...
const T = { docs: 0, unmatched: 0, exact: 0, wrongCut: 0, truncatedAutoFiled: 0, truncatedHeld: 0, merged: 0, mergedMarked: 0, mergedUnmarked: 0, onePageAuto: 0, onePageAutoIncomplete: 0, whole: 0 };
const bad = [];
for (const d of docs) {
  T.docs++;
